...
parameter_list|)
block|{
comment|// The API doesn't allow checked exceptions here, so propagate up the stack. This is unwrapped
comment|// in getAntlrParseTree.
throw|throw
operator|new
name|RuntimeException
argument_list|(
...
expr_stmt|;
block|}
comment|/** Check Method signature for compatibility. */
...
parameter_list|)
block|{
comment|// check that the Method is public in some public reachable class:
specifier|final
name|MethodType
name|type
decl_stmt|;
...
name|method
operator|+
literal|" is not accessible (declaring class or method not public)."
argument_list|)
throw|;
block|}
...
block|}
block|}
comment|/** Cross check if declaring class of given method is the same as    * returned by the given parent {@link ClassLoader} on string lookup.    * This prevents {@link NoClassDefFoundError}.    */
DECL|method|checkFunctionClassLoader
specifier|private
specifier|static
name|void
name|checkFunctionClassLoader
