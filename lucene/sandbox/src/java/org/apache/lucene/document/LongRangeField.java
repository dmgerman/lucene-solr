...
begin_comment
comment|/**  * An indexed Long Range field.  *<p>  * This field indexes dimensional ranges defined as min/max pairs. It supports  * up to a maximum of 4 dimensions (indexed as 8 numeric values). With 1 dimension representing a single long range,  * 2 dimensions representing a bounding box, 3 dimensions a bounding cube, and 4 dimensions a tesseract.  *<p>  * Multiple values for the same field in one document is supported, and open ended ranges can be defined using  * {@code Long.MIN_VALUE} and {@code Long.MAX_VALUE}.  *  *<p>  * This field defines the following static factory methods for common search operations over long ranges:  *<ul>  *<li>{@link #newIntersectsQuery newIntersectsQuery()} matches ranges that intersect the defined search range.  *<li>{@link #newWithinQuery newWithinQuery()} matches ranges that are within the defined search range.  *<li>{@link #newContainsQuery newContainsQuery()} matches ranges that contain the defined search range.  *</ul>  */
end_comment

begin_class
DECL|class|LongRangeField
specifier|public
class|class
name|LongRangeField
extends|extends
name|Field
block|{
comment|/** stores long values so number of bytes is 8 */
DECL|field|BYTES
specifier|public
specifier|static
specifier|final
...
name|BYTES
decl_stmt|;
comment|/**    * Create a new LongRangeField type, from min/max parallel arrays    *    * @param name field name. must not be null.    * @param min range min values; each entry is the min value for the dimension    * @param max range max values; each entry is the max value for the dimension    */
DECL|method|LongRangeField
specifier|public
name|LongRangeField
parameter_list|(
name|String
...
argument_list|(
literal|"LongRangeField does not support greater than 4 dimensions"
argument_list|)
throw|;
block|}
name|FieldType
...
return|;
block|}
comment|/**    * Changes the values of the field.    * @param min array of min values. (accepts {@code Long.MIN_VALUE})    * @param max array of max values. (accepts {@code Long.MAX_VALUE})    * @throws IllegalArgumentException if {@code min} or {@code max} is invalid    */
...
return|;
block|}
comment|/**    * Create a query for matching indexed ranges that intersect the defined range.    * @param field field name. must not be null.    * @param min array of min values. (accepts {@code Long.MIN_VALUE})    * @param max array of max values. (accepts {@code Long.MAX_VALUE})    * @return query for matching intersecting ranges (overlap, within, or contains)    * @throws IllegalArgumentException if {@code field} is null, {@code min} or {@code max} is invalid    */
