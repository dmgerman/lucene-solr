...
argument_list|()
decl_stmt|;
comment|// TODO: would be nice to be able to assert silly duplicated tokens are not created, but a number of cases do this "legitimately": LUCENE-7622
name|ts
operator|.
name|reset
argument_list|()
...
name|assertTrue
argument_list|(
literal|"posLength must be>= 1; got: "
operator|+
name|posLengthAtt
operator|.
name|getPositionLength
...
return|;
block|}
comment|/**    * Enumerates all accepted strings in the token graph created by the analyzer on the provided text, and then    * asserts that it's equal to the expected strings.    * Uses {@link TokenStreamToAutomaton} to create an automaton. Asserts the finite strings of the automaton are all    * and only the given valid strings.    * @param analyzer analyzer containing the SynonymFilter under test.    * @param text text to be analyzed.    * @param expectedStrings all expected finite strings.    */
DECL|method|assertGraphStrings
specifier|public
specifier|static
name|void
...
block|}
block|}
comment|/**    * Enumerates all accepted strings in the token graph created by the already initialized {@link TokenStream}.    */
DECL|method|assertGraphStrings
specifier|public
specifier|static
name|void
...
name|assertTrue
argument_list|(
literal|"Analyzer created unexpected string path: "
operator|+
name|s
operator|+
literal|"\nexpected:\n"
...
block|}
block|}
comment|/** Returns all paths accepted by the token stream graph produced by analyzing text with the provided analyzer.  The tokens {@link    *  CharTermAttribute} values are concatenated, and separated with space. */
DECL|method|getGraphStrings
specifier|public
specifier|static
name|Set
argument_list|<
...
block|}
block|}
comment|/** Returns all paths accepted by the token stream graph produced by the already initialized {@link TokenStream}. */
DECL|method|getGraphStrings
specifier|public
specifier|static
name|Set
...
return|;
block|}
comment|/** Returns a {@code String} summary of the tokens this analyzer produces on this text */
DECL|method|toString
specifier|public
specifier|static
name|String
