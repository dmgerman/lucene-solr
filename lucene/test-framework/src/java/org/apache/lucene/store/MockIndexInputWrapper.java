...
name|closed
decl_stmt|;
comment|// Which MockIndexInputWrapper we were cloned from, or null if we are not a clone:
DECL|field|parent
specifier|private
specifier|final
name|MockIndexInputWrapper
...
argument_list|)
expr_stmt|;
comment|// If we are a clone then our parent better not be a clone!
assert|assert
name|parent
operator|==
literal|null
...
parameter_list|()
block|{
comment|// TODO: not great this is a volatile read (closed) ... we should deploy heavy JVM voodoo like SwitchPoint to avoid this
if|if
condition|(
name|closed
condition|)
...
name|RuntimeException
argument_list|(
literal|"Abusing clone of a closed IndexInput!"
argument_list|)
throw|;
block|}
block|}
