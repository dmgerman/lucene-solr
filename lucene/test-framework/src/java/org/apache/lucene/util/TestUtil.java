...
comment|//
block|}
comment|/**     * A comparator that compares UTF-16 strings / char sequences according to Unicode    * code point order. This can be used to verify {@link BytesRef} order.     *<p>    *<b>Warning:</b> This comparator is rather inefficient, because    * it converts the strings to a {@code int[]} array on each invocation.    * */
DECL|field|STRING_CODEPOINT_COMPARATOR
specifier|public
specifier|static
specifier|final
name|Comparator
...
block|}
block|}
comment|// used by TestUtil.checkReader to check some things really unrelated to the index,
comment|// just looking for bugs in indexreader implementations.
DECL|method|checkReaderSanity
specifier|private
specifier|static
name|void
name|checkReaderSanity
...
control|)
block|{
comment|// reader shouldn't return normValues if the field does not have them
if|if
condition|(
operator|!
name|info
...
block|}
block|}
comment|// reader shouldn't return docValues if the field does not have them
comment|// reader shouldn't return multiple docvalues types for the same field.
switch|switch
condition|(
name|info
operator|.
...
block|}
block|}
comment|/**    * Returns a randomish big integer with {@code 1 .. maxBytes} storage.    */
DECL|method|nextBigInteger
specifier|public
specifier|static
name|BigInteger
name|nextBigInteger
...
return|;
block|}
comment|/** Returns true if VirusCheckingFS is in use and was in fact already enabled */
DECL|method|disableVirusChecker
specifier|public
specifier|static
name|boolean
...
return|;
block|}
DECL|method|enableVirusChecker
specifier|public
specifier|static
name|void
