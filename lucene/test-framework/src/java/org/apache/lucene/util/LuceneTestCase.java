...
expr_stmt|;
block|}
comment|/** Returns to the default {@link IndexWriter#MAX_DOCS} limit. */
...
argument_list|()
decl_stmt|;
comment|// this should always be the case, diff each line
if|if
condition|(
name|previousLines
operator|.
...
block|{
comment|// but just in case of something ridiculous...
name|diff
operator|.
name|append
argument_list|(
...
expr_stmt|;
block|}
comment|// its possible to be empty, if we "change" a value to what it had before.
if|if
condition|(
name|diff
operator|.
...
name|println
argument_list|(
literal|"NOTE: LuceneTestCase: randomly changed IWC's live settings:"
argument_list|)
expr_stmt|;
name|System
operator|.
...
name|IllegalArgumentException
argument_list|(
literal|"value must be String or BytesRef"
argument_list|)
throw|;
block|}
block|}
DECL|field|availableLanguageTags
specifier|private
specifier|static
specifier|final
name|String
index|[]
name|availableLanguageTags
init|=
name|Arrays
operator|.
name|stream
...
DECL|method|localeForLanguageTag
specifier|public
specifier|static
name|Locale
name|localeForLanguageTag
...
annotation|@
name|Before
DECL|method|overrideTestDefaultQueryCache
specifier|public
name|void
name|overrideTestDefaultQueryCache
parameter_list|()
block|{
comment|// Make sure each test method has its own cache
name|overrideDefaultQueryCache
argument_list|()
expr_stmt|;
block|}
annotation|@
name|BeforeClass
DECL|method|overrideDefaultQueryCache
specifier|public
specifier|static
name|void
name|overrideDefaultQueryCache
...
annotation|@
name|BeforeClass
DECL|method|setupCPUCoreCount
specifier|public
specifier|static
name|void
name|setupCPUCoreCount
parameter_list|()
block|{
comment|// Randomize core count so CMS varies its dynamic defaults, and this also "fixes" core
comment|// count from the master seed so it will always be the same on reproduce:
name|int
name|numCores
init|=
name|TestUtil
...
name|ConcurrentMergeScheduler
operator|.
name|DEFAULT_CPU_CORE_COUNT_PROPERTY
argument_list|,
name|Integer
operator|.
name|toString
...
annotation|@
name|AfterClass
DECL|method|restoreCPUCoreCount
specifier|public
specifier|static
name|void
name|restoreCPUCoreCount
parameter_list|()
block|{
name|System
operator|.
...
parameter_list|()
block|{
comment|// Randomize IOUtils.spins() count so CMS varies its dynamic defaults, and this also "fixes" core
comment|// count from the master seed so it will always be the same on reproduce:
name|boolean
name|spins
init|=
...
name|info
operator|+
literal|": left numeric doc values for field=\""
operator|+
name|field
operator|+
literal|"\" are not null"
argument_list|,
name|leftValues
operator|==
literal|null
...
name|info
operator|+
literal|": right numeric doc values for field=\""
operator|+
name|field
operator|+
literal|"\" are not null"
...
expr_stmt|;
block|}
comment|// naive silly memory heavy uninversion!!  maps docID -> packed values (a Set because a given doc can be multi-valued)
...
parameter_list|)
block|{
comment|// We pretend our query shape is so hairy that it crosses every single cell:
return|return
name|PointValues
operator|.
name|Relation
...
return|;
block|}
DECL|method|assertPointsEquals
specifier|public
name|void
name|assertPointsEquals
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// same bytes per dimension?
name|assertEquals
argument_list|(
name|info
argument_list|,
...
block|}
block|}
comment|// make sure FieldInfos2 doesn't have any point fields that FieldInfo1 didn't have
for|for
control|(
name|FieldInfo
name|fieldInfo2
...
argument_list|)
expr_stmt|;
comment|// we don't need to uninvert and compare here ... we did that in the first loop above
block|}
block|}
block|}
comment|/** A runnable that can throw any checked exception. */
annotation|@
...
throw|;
block|}
comment|/**    * Checks that specific wrapped and outer exception classes are thrown    * by the given runnable, and returns the wrapped exception.     */
DECL|method|expectThrows
specifier|public
specifier|static
parameter_list|<
...
name|TO
argument_list|>
name|expectedOuterType
parameter_list|,
name|Class
argument_list|<
name|TW
argument_list|>
name|expectedWrappedType
parameter_list|,
name|ThrowingRunnable
name|runnable
parameter_list|)
...
name|AssertionFailedError
argument_list|(
literal|"Unexpected wrapped exception type, expected "
operator|+
name|expectedWrappedType
operator|.
name|getSimpleName
...
name|AssertionFailedError
argument_list|(
literal|"Unexpected outer exception type, expected "
operator|+
name|expectedOuterType
operator|.
name|getSimpleName
...
name|AssertionFailedError
argument_list|(
literal|"Expected outer exception "
operator|+
name|expectedOuterType
operator|.
name|getSimpleName
...
name|assumeTrue
argument_list|(
literal|"runWithRestrictedPermissions requires a SecurityManager enabled"
argument_list|,
name|System
operator|.
...
argument_list|)
decl_stmt|;
comment|// if collation keys don't really respect collation order, things are screwed.
name|assumeTrue
argument_list|(
