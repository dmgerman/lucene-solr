...
name|runDFA
decl_stmt|;
comment|// TODO: this is copied from SimplePatternTokenizer, but there are subtle differences e.g. we track sepUpto an tokenUpto;
comment|// find a clean way to share it:
...
DECL|method|SimplePatternSplitTokenizer
specifier|public
name|SimplePatternSplitTokenizer
parameter_list|(
name|String
...
condition|)
block|{
comment|// a token separator just possibly started; keep scanning to see if the token is accepted:
name|int
...
condition|)
block|{
comment|// record that the token separator matches here, but keep scanning in case a longer match also works (greedy):
name|lastAcceptLength
operator|=
name|sepUpto
expr_stmt|;
...
block|{
comment|// we matched one token separator immediately after another
name|offsetStart
operator|=
name|offset
expr_stmt|;
...
block|{
comment|// false alarm: there was no token separator here; push back all but the first character we scanned
name|pushBack
argument_list|(
name|sepUpto
operator|-
