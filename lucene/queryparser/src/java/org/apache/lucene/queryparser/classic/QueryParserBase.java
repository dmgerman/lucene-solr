...
condition|)
block|{
comment|// we have a graph query that has at least one phrase sub-query
comment|// re-build and set slop on all phrase queries
name|List
argument_list|<
name|Query
argument_list|>
...
return|;
block|}
comment|/**    * Rebuild a phrase query with a slop value    */
DECL|method|addSlopToPhrase
specifier|private
name|PhraseQuery
name|addSlopToPhrase
parameter_list|(
...
parameter_list|)
block|{
comment|// best effort to not pass the wildcard characters and escaped characters through #normalize
name|Matcher
...
expr_stmt|;
block|}
comment|//append the matched group - without normalizing
name|sb
operator|.
name|append
argument_list|(
...
name|ParseException
block|{
comment|// We need to pass the whole string to #normalize, which will not work with
comment|// custom attribute factories for the binary term impl, and may not work
comment|// with some analyzers
name|BytesRef
name|term
init|=
name|getAnalyzer
