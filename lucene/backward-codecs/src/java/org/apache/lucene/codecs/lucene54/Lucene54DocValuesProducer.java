...
begin_comment
comment|/** reader for {@link Lucene54DocValuesFormat} */
end_comment

begin_class
DECL|class|Lucene54DocValuesProducer
specifier|final
class|class
...
argument_list|()
decl_stmt|;
DECL|field|directAddressesMeta
specifier|private
specifier|final
name|Map
argument_list|<
...
DECL|method|Lucene54DocValuesProducer
name|Lucene54DocValuesProducer
parameter_list|(
name|Lucene54DocValuesProducer
name|original
...
expr_stmt|;
block|}
DECL|method|readSortedSetFieldWithTable
specifier|private
name|void
name|readSortedSetFieldWithTable
parameter_list|(
name|FieldInfo
name|info
parameter_list|,
...
name|IOException
block|{
comment|// sortedset table = binary + ordset table + ordset index
if|if
condition|(
name|meta
operator|.
...
condition|)
block|{
comment|// sparse bits need a bit more metadata
name|entry
operator|.
...
name|CorruptIndexException
argument_list|(
literal|"Number type can only be 0 or 1, got="
operator|+
name|numberType
argument_list|,
name|meta
...
throw|;
block|}
comment|// now read the numeric entry for non-missing values
specifier|final
name|int
name|fieldNumber
init|=
...
name|CorruptIndexException
argument_list|(
literal|"Field numbers mistmatch: "
operator|+
name|fieldNumber
operator|+
literal|" != "
...
name|CorruptIndexException
argument_list|(
literal|"SORTED_SET_TABLE cannot have more than 256 values in its dictionary, got="
operator|+
name|totalTableLength
argument_list|,
name|meta
...
condition|)
block|{
comment|// +1 because of the empty set
throw|throw
operator|new
name|CorruptIndexException
argument_list|(
literal|"SORTED_SET_TABLE cannot have more set ids than ords in its dictionary, got "
operator|+
name|totalTableLength
operator|+
literal|" ords and "
...
block|{
return|return
name|getSparseNumericDocValues
argument_list|(
name|entry
argument_list|)
return|;
...
name|namedAccountables
argument_list|(
literal|"direct addresses meta field"
argument_list|,
name|directAddressesMeta
argument_list|)
argument_list|)
...
return|return
operator|new
name|SparseNumericDocValuesRandomAccessWrapper
argument_list|(
name|values
argument_list|,
name|missingValue
...
name|hiDoc
decl_stmt|;
comment|// gallop forward by exponentially growing the interval
comment|// in order to find an interval so that the target doc
comment|// is in ]lo, hi]. Compared to a regular binary search,
comment|// this optimizes the case that the caller performs many
comment|// advance calls by small deltas
do|do
block|{
name|hiIndex
operator|=
...
block|}
block|}
DECL|class|SparseNumericDocValuesRandomAccessWrapper
specifier|static
class|class
name|SparseNumericDocValuesRandomAccessWrapper
extends|extends
...
name|missingValue
decl_stmt|;
DECL|method|SparseNumericDocValuesRandomAccessWrapper
name|SparseNumericDocValuesRandomAccessWrapper
parameter_list|(
name|SparseNumericDocValues
name|values
...
block|}
block|}
DECL|method|getLegacyBinary
name|LegacyBinaryDocValues
name|getLegacyBinary
parameter_list|(
name|FieldInfo
name|field
parameter_list|)
...
condition|)
block|{
comment|// TODO: use .nextSetBit here, at least!!
break|break;
block|}
block|}
return|return
...
case|:
return|return
name|getSortedSetWithAddresses
argument_list|(
name|field
argument_list|)
return|;
...
block|}
block|}
DECL|method|getSortedSetWithAddresses
specifier|private
name|SortedSetDocValues
name|getSortedSetWithAddresses
parameter_list|(
...
return|return
operator|new
name|LegacySortedSetDocValuesWrapper
argument_list|(
operator|new
name|LegacySortedSetDocValues
argument_list|()
block|{
name|long
name|startOffset
...
block|}
block|}
DECL|method|getSparseNumericDocValues
specifier|private
name|SparseNumericDocValues
name|getSparseNumericDocValues
parameter_list|(
...
name|count
decl_stmt|;
comment|/** monotonic meta */
DECL|field|monotonicMeta
specifier|public
name|DirectMonotonicReader
operator|.
name|Meta
...
index|[]
decl_stmt|;
comment|/** for sparse compression */
DECL|field|numDocsWithValue
name|long
name|numDocsWithValue
decl_stmt|;
DECL|field|nonMissingValues
name|NumericEntry
name|nonMissingValues
decl_stmt|;
...
name|addressesEndOffset
decl_stmt|;
comment|/** meta data for addresses */
