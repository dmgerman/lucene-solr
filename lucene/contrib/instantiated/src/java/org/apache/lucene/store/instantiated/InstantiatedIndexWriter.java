...
argument_list|()
expr_stmt|;
comment|// update deleted documents bitset
if|if
condition|(
name|index
operator|.
...
condition|)
block|{
comment|// TODO: this is a simple workaround to still work with tokens, not very effective, but as far as I know, this writer should get removed soon:
specifier|final
name|Token
name|token
init|=
...
parameter_list|)
block|{
comment|// ignore unsupported attributes,
comment|// this may fail to copy some attributes, if a special combined AttributeImpl is used, that
comment|// implements basic attributes supported by Token and also other customized ones in one class.
block|}
block|}
name|tokens
operator|.
