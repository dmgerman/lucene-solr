...
begin_comment
comment|// NOTE: This is a copy of TestNRTThreads, but swapping in
end_comment

begin_comment
comment|// NRTManager for adding/updating/searching
end_comment
...
argument_list|)
expr_stmt|;
comment|//System.out.println("TEST: conf=" + writer.getConfig());
specifier|final
name|ExecutorService
name|es
init|=
...
argument_list|)
decl_stmt|;
comment|//final int NUM_INDEX_THREADS = 1;
comment|//final int NUM_SEARCH_THREADS = 1;
if|if
condition|(
name|VERBOSE
condition|)
...
name|NUM_INDEX_THREADS
operator|+
literal|" index threads; "
operator|+
name|NUM_SEARCH_THREADS
operator|+
literal|" search threads"
argument_list|)
expr_stmt|;
block|}
specifier|final
...
condition|)
block|{
comment|//System.out.println(Thread.currentThread().getName() + ": cycle");
try|try
block|{
comment|// Occassional longish pause if running
comment|// nightly
if|if
condition|(
name|LuceneTestCase
...
argument_list|)
expr_stmt|;
comment|/*                       // non-atomic:                       nrt.deleteDocuments(new Term("packID", delSubDocs.packID));                       for(Document subDoc : docsList) {                         nrt.addDocument(subDoc);                       }                       */
block|}
else|else
block|{
if|if
...
argument_list|)
expr_stmt|;
comment|/*                       // non-atomic:                       for(Document subDoc : docsList) {                         nrt.addDocument(subDoc);                       }                       */
block|}
name|doc
operator|.
name|removeField
...
expr_stmt|;
block|}
comment|// randomly verify the add/update "took":
if|if
condition|(
name|random
operator|.
...
condition|)
block|{
comment|//System.out.println(Thread.currentThread().getName() + ": verify");
specifier|final
name|IndexSearcher
name|s
init|=
...
argument_list|)
decl_stmt|;
comment|//System.out.println(Thread.currentThread().getName() + ": got s=" + s);
try|try
block|{
name|assertEquals
argument_list|(
...
expr_stmt|;
block|}
comment|//System.out.println(Thread.currentThread().getName() + ": done verify");
block|}
if|if
condition|(
name|random
...
name|println
argument_list|(
literal|"TEST: start search threads"
argument_list|)
expr_stmt|;
block|}
for|for
