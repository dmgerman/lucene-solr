...
name|tokenStream
decl_stmt|;
comment|//set subsequent to getWeightedSpanTerms* methods
...
block|}
block|}
DECL|method|isQueryUnsupported
specifier|protected
name|boolean
name|isQueryUnsupported
parameter_list|(
...
parameter_list|)
block|{
comment|// spatial queries do not support highlighting:
if|if
condition|(
name|clazz
operator|.
...
name|startsWith
argument_list|(
literal|"org.apache.lucene.spatial."
argument_list|)
condition|)
block|{
return|return
...
return|;
block|}
comment|// spatial3d queries are also not supported:
if|if
condition|(
name|clazz
operator|.
...
name|startsWith
argument_list|(
literal|"org.apache.lucene.spatial3d."
argument_list|)
condition|)
block|{
return|return
...
operator|)
decl_stmt|;
comment|// If it's from term vectors, simply wrap the underlying Terms in a reader
if|if
condition|(
name|tokenStream
operator|instanceof
...
block|}
block|}
comment|// Use MemoryIndex (index/invert this tokenStream now)
if|if
condition|(
name|internalReader
operator|==
...
expr_stmt|;
block|}
comment|//Now wrap it so we always use a common field.
name|this
operator|.
name|internalReader
operator|=
...
return|;
block|}
comment|/** Returns the tokenStream which may have been wrapped in a CachingTokenFilter.    * getWeightedSpanTerms* sets the tokenStream, so don't call this before. */
...
expr_stmt|;
block|}
comment|/** A threshold of number of characters to analyze. When a TokenStream based on    * term vectors with offsets and positions are supplied, this setting    * does not apply. */
