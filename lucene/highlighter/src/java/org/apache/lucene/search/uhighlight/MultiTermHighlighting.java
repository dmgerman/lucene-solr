...
block|}
block|}
comment|/**    * Return a TokenStream un-inverted from the provided Terms, but filtered based on the automata. The    * Terms must have exactly one doc count (e.g. term vector or MemoryIndex).    */
comment|//TODO: Alternatively, produce a list of OffsetsEnums from the Terms that match the automata.
DECL|method|uninvertAndFilterTerms
specifier|public
specifier|static
name|TokenStream
...
literal|0
assert|;
comment|//Note: if automata were plain Automaton (not CharacterRunAutomaton), we might instead use
comment|// TermsEnum.intersect(compiledAutomaton).  But probably won't help due to O(N) TV impl so whatever.
name|FilterLeafReader
operator|.
name|FilterTerms
name|filteredTermsIndex
init|=
operator|new
name|FilterLeafReader
operator|.
...
argument_list|()
decl_stmt|;
comment|//reuse only for UTF8->UTF16 call
annotation|@
name|Override
specifier|protected
name|AcceptStatus
...
return|;
block|}
comment|/**    * Returns a simple automata that matches the specified term.    */
DECL|method|makeStringMatchAutomata
specifier|public
specifier|static
name|CharacterRunAutomaton
