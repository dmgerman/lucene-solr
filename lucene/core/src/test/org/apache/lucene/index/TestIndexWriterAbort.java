...
begin_comment
comment|/**   * Creates aborting exceptions and checks that files are cleaned up  */
end_comment
...
literal|"SimpleText"
argument_list|)
DECL|class|TestIndexWriterAbort
specifier|public
class|class
name|TestIndexWriterAbort
extends|extends
name|LuceneTestCase
block|{
...
name|Exception
block|{
comment|// use an explicit RAMDirectory, we will act-like windows,
comment|// but no virus scanner (real or fake).
comment|// TODO: use MDW's check instead of TestIW.assertNoUnreferencedFiles,
comment|// then turn it all back on
name|MockDirectoryWrapper
name|dir
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// assert that we actually delete files.
...
argument_list|)
decl_stmt|;
comment|// wrap the actual codec with one that throws random exceptions
name|Codec
name|codec
init|=
operator|new
...
block|}
block|}
comment|// test that there are no unreferenced files: any deletes we make should have succeeded
comment|// TODO: don't use this method, use the MDW one!!!!
...
name|dir
argument_list|,
literal|"unreferenced files remain after close()"
argument_list|)
expr_stmt|;
comment|// checkindex
name|dir
