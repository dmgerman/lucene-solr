...
name|IOException
block|{
comment|// use ramdir explicit, as we do evil things like try to generate broken files, deletes must work.
try|try
init|(
name|BaseDirectoryWrapper
name|dirCopy
...
block|{
comment|// Iterate until our randomly generated bytes are indeed different from the first bytes of the file ... the vast majority of the
comment|// time this will only require one iteration!
while|while
condition|(
literal|true
condition|)
...
condition|)
block|{
comment|// We successfully randomly generated bytes that differ from the bytes in the file:
name|out
operator|.
name|copyBytes
argument_list|(
...
block|}
block|}
comment|// we have to try again, delete the first attempt and retry the loop
name|dirCopy
operator|.
name|deleteFile
argument_list|(
