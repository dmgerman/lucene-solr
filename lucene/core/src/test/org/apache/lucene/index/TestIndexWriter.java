...
argument_list|)
decl_stmt|;
comment|// NOTE: cannot use MMapDir, because WindowsFS doesn't see/think it keeps file handles open?
name|FSDirectory
name|dir
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// setCommitData with no other changes should count as an NRT change:
name|DirectoryReader
name|r2
init|=
name|DirectoryReader
...
argument_list|()
expr_stmt|;
comment|// setCommitData and also commit, with no other changes, should count as an NRT change:
name|DirectoryReader
name|r2
init|=
name|DirectoryReader
...
expr_stmt|;
block|}
comment|// nocommit turn test on once we have VirusCheckingFS
comment|/*   public void testWithPendingDeletions() throws Exception {     try (FSDirectory dir = FSDirectory.open(createTempDir())) {       IndexWriterConfig iwc = new IndexWriterConfig(new MockAnalyzer(random()));       IndexWriter w = new IndexWriter(dir, iwc);       w.commit();       IndexInput in = dir.openInput("segments_0", IOContext.DEFAULT);       w.addDocument(new Document());       w.close();       assertTrue(dir.checkPendingDeletions());       iwc = new IndexWriterConfig(new MockAnalyzer(random()));       try {         w = new IndexWriter(dir, iwc);       } catch (IllegalArgumentException iae) {         assertEquals("Directory still has pending deleted files", iae.getMessage());       }     }   }   */
