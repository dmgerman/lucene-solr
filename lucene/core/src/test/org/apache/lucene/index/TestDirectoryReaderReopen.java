...
expr_stmt|;
block|}
comment|/** test reopening backwards from a non-NRT reader (with document deletes) */
DECL|method|testNRTMdeletes
specifier|public
name|void
name|testNRTMdeletes
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
expr_stmt|;
comment|// make sure all index metadata is written out
name|Document
name|doc
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// This reader will be used for searching against commit point 1
name|DirectoryReader
name|oldest
init|=
name|DirectoryReader
...
expr_stmt|;
block|}
comment|/** test reopening backwards from an NRT reader (with document deletes) */
DECL|method|testNRTMdeletes2
specifier|public
name|void
name|testNRTMdeletes2
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
decl_stmt|;
comment|// This reader should not see the deletion:
name|assertEquals
argument_list|(
literal|2
argument_list|,
...
expr_stmt|;
block|}
comment|/** test reopening backwards from a non-NRT reader with DV updates */
DECL|method|testNRTMupdates
specifier|public
name|void
name|testNRTMupdates
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
comment|/** test reopening backwards from an NRT reader with DV updates */
DECL|method|testNRTMupdates2
specifier|public
name|void
name|testNRTMupdates2
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
comment|// LUCENE-5931: we make a "best effort" to catch this abuse and throw a clear(er)
comment|// exception than what would otherwise look like hard to explain index corruption during searching
DECL|method|testDeleteIndexFilesWhileReaderStillOpen
specifier|public
name|void
name|testDeleteIndexFilesWhileReaderStillOpen
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// Creates single segment index:
name|w
operator|.
name|close
argument_list|()
...
argument_list|)
decl_stmt|;
comment|// Abuse: remove all files while reader is open; one is supposed to use IW.deleteAll, or open a new IW with OpenMode.CREATE instead:
for|for
control|(
name|String
name|file
...
argument_list|)
expr_stmt|;
comment|// Writes same segment, this time with two documents:
name|w
operator|.
name|commit
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Writes another segments file, so openIfChanged sees that the index has in fact changed:
name|w
operator|.
name|close
argument_list|()
