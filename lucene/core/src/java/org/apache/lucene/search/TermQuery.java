...
argument_list|(
literal|"termStates are required when scores are needed"
argument_list|)
throw|;
block|}
name|this
...
block|{
comment|// we do not need the actual stats, use fake stats with docFreq=maxDoc and ttf=-1
specifier|final
name|int
name|maxDoc
init|=
...
condition|)
block|{
comment|// TermQuery either used as a Query or the term states have been provided at construction time
assert|assert
name|termStates
operator|.
name|topReaderContext
...
block|{
comment|// TermQuery used as a filter, so the term states have not been built up front
name|Terms
name|terms
init|=
name|context
...
block|{
comment|// do not compute the term state, this will help save seeks in the terms
comment|// dict on segments that have a cache entry for this query
name|termState
operator|=
literal|null
expr_stmt|;
