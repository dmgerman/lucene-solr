...
name|cacheSize
decl_stmt|;
comment|/**    * Expert: Create a new instance that will cache at most<code>maxSize</code>    * queries with at most<code>maxRamBytesUsed</code> bytes of memory, only on    * leaves that satisfy {@code leavesToCache};    */
...
expr_stmt|;
block|}
comment|/**    * Create a new instance that will cache at most<code>maxSize</code> queries    * with at most<code>maxRamBytesUsed</code> bytes of memory. Queries will    * only be cached on leaves that have more than 10k documents and have more    * than 3% of the total number of documents in the index.    * This should guarantee that all leaves from the upper    * {@link TieredMergePolicy tier} will be cached while ensuring that at most    *<tt>33</tt> leaves can make it to the cache (very likely less than 10 in    * practice), which is useful for this implementation since some operations    * perform in linear time with the number of cached leaves.    */
DECL|method|LRUQueryCache
specifier|public
name|LRUQueryCache
parameter_list|(
...
DECL|class|MinSegmentSizePredicate
specifier|static
class|class
name|MinSegmentSizePredicate
implements|implements
...
name|minSizeRatio
decl_stmt|;
DECL|method|MinSegmentSizePredicate
name|MinSegmentSizePredicate
parameter_list|(
name|int
name|minSize
...
return|;
block|}
comment|/**    * Default cache implementation: uses {@link RoaringDocIdSet} for sets that    * have a density&lt; 1% and a {@link BitDocIdSet} over a {@link FixedBitSet}    * otherwise.    */
...
condition|)
block|{
comment|// FixedBitSet is faster for dense sets and will enable the random-access
comment|// optimization in ConjunctionDISI
return|return
name|cacheIntoBitSet
argument_list|(
name|scorer
argument_list|,
name|maxDoc
...
return|;
block|}
DECL|method|cacheIntoRoaringDocIdSet
specifier|private
specifier|static
name|DocIdSet
name|cacheIntoRoaringDocIdSet
...
block|}
block|}
comment|/** Check whether this segment is eligible for caching, regardless of the query. */
...
expr_stmt|;
block|}
comment|// TODO: should it be pluggable, eg. for queries that run on doc values?
specifier|final
name|IndexReader
operator|.
name|CacheHelper
...
condition|)
block|{
comment|// this segment is not suitable for caching
return|return
name|in
operator|.
name|scorerSupplier
...
return|;
block|}
comment|// Short-circuit: Check whether this segment is eligible for caching
comment|// before we take a lock because of #get
if|if
condition|(
name|shouldCache
argument_list|(
...
return|;
block|}
comment|// If the lock is already busy, prefer using the uncached version than waiting
if|if
condition|(
name|lock
operator|.
