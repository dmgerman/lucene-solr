...
parameter_list|)
function_decl|;
comment|/**    * Compute an index-time normalization value for this field instance.    *     * @param length the number of terms in the field, optionally {@link #setDiscountOverlaps(boolean) discounting overlaps}    * @return a length normalization value    */
...
condition|)
block|{
comment|// the norms only encode the length, we need a translation table that depends on how lengthNorm is implemented
name|normTable
operator|=
name|idfstats
operator|.
...
block|{
comment|// the norm is directly encoded in the index
name|normTable
operator|=
name|OLD_NORM_TABLE
expr_stmt|;
