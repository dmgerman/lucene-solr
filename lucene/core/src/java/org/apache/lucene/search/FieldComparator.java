...
block|}
block|}
comment|/** Returns negative result if the doc's value is less    *  than the provided value. */
...
condition|)
block|{
comment|// reverse of FloatComparator
return|return
literal|1
return|;
block|}
...
return|;
block|}
comment|// nocommit remove null from FC DocTerms/Index as an
...
block|}
block|}
comment|// Used per-segment when docToOrd is null:
...
condition|)
block|{
comment|// -1 ord is null for all segments
assert|assert
name|ords
index|[
name|bottomSlot
...
name|termsIndex
decl_stmt|;
comment|/* Comparator for comparing by value.        @lucene.internal */
DECL|field|comp
name|Comparator
argument_list|<
name|BytesRef
...
argument_list|()
decl_stmt|;
DECL|method|TermOrdValDocValuesComparator
specifier|public
name|TermOrdValDocValuesComparator
parameter_list|(
name|int
...
return|;
block|}
comment|// TODO: would be nice to share these specialized impls
comment|// w/ TermOrdValComparator
comment|/** Base class for specialized (per bit width of the      * ords) per-segment comparator.  NOTE: this is messy;      * we do this only because hotspot can't reliably inline      * the underlying array access when looking up doc->ord      * @lucene.internal      */
DECL|class|PerSegmentComparator
specifier|abstract
class|class
...
DECL|class|AnyPackedDocToOrdComparator
specifier|private
specifier|final
class|class
name|AnyPackedDocToOrdComparator
extends|extends
name|PerSegmentComparator
block|{
DECL|field|readerOrds
...
name|docBase
decl_stmt|;
DECL|method|AnyPackedDocToOrdComparator
specifier|public
name|AnyPackedDocToOrdComparator
parameter_list|(
name|PackedInts
...
block|}
block|}
comment|// Used per-segment when DV doesn't use packed ints for
comment|// docToOrds:
DECL|class|AnyOrdComparator
specifier|private
specifier|final
...
argument_list|()
decl_stmt|;
DECL|method|TermValDocValuesComparator
name|TermValDocValuesComparator
parameter_list|(
name|int
name|numHits
