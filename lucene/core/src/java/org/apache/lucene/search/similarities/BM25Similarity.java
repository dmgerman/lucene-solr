...
name|idf
argument_list|,
literal|"idf, computed as log(1 + (docCount - docFreq + 0.5) / (docFreq + 0.5)) from:"
argument_list|,
name|Explanation
operator|.
name|match
...
literal|0d
decl_stmt|;
comment|// sum into a double before casting into a float
name|List
argument_list|<
name|Explanation
argument_list|>
...
name|norms
decl_stmt|;
comment|/** precomputed cache for all length values */
DECL|field|lengthCache
specifier|private
specifier|final
name|float
index|[]
...
name|field
decl_stmt|;
comment|/** precomputed norm[256] with k1 * ((1 - b) + b * dl / avgdl)      *  for both OLD_LENGTH_TABLE and LENGTH_TABLE */
DECL|field|oldCache
DECL|field|cache
specifier|private
specifier|final
...
operator|)
argument_list|,
literal|"tfNorm, computed as (freq * (k1 + 1)) / (freq + k1) from:"
argument_list|,
name|subs
argument_list|)
return|;
...
operator|)
argument_list|,
literal|"tfNorm, computed as (freq * (k1 + 1)) / (freq + k1 * (1 - b + b * fieldLength / avgFieldLength)) from:"
argument_list|,
name|subs
argument_list|)
return|;
