...
return|;
block|}
comment|/**        * Create a visitor that clears documents that do NOT match the range.        */
specifier|private
name|IntersectVisitor
name|getInverseIntersectVisitor
parameter_list|(
name|FixedBitSet
name|result
parameter_list|,
...
condition|)
block|{
comment|// This dim is not in the range
return|return
name|Relation
operator|.
name|CELL_INSIDE_QUERY
...
condition|)
block|{
comment|// If all docs have exactly one value and the cost is greater
comment|// than half the leaf size then maybe we can make things faster
comment|// by computing the set of documents that do NOT match the range
specifier|final
name|FixedBitSet
name|result
init|=
