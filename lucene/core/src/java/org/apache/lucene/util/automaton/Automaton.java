...
expr_stmt|;
block|}
comment|/**    * Constructor which creates an automaton with enough space for the given    * number of states and transitions.    *     * @param numStates    *           Number of states.    * @param numTransitions    *           Number of transitions.    */
DECL|method|Automaton
specifier|public
name|Automaton
parameter_list|(
...
return|;
block|}
comment|/** How many transitions this automaton has. */
...
index|]
assert|;
comment|// Make sure transitions are in fact sorted:
assert|assert
name|transitionSorted
argument_list|(
name|t
argument_list|)
assert|;
...
condition|)
block|{
comment|// Transition isn't initialzed yet (this is the first transition); don't check:
return|return
literal|true
return|;
block|}
...
return|;
block|}
comment|// Min is equal, now test max:
if|if
condition|(
name|nextMax
operator|>
...
return|;
block|}
comment|// Max is also equal, now test dest:
if|if
condition|(
name|nextDest
operator|>
...
return|;
block|}
comment|// We should never see fully equal transitions here:
return|return
literal|false
return|;
block|}
...
argument_list|)
expr_stmt|;
comment|//System.out.println("  t.nextTrans=" + t.transitionUpto + " t=" + t);
assert|assert
name|t
operator|.
name|max
...
literal|0
decl_stmt|;
comment|/** Default constructor, pre-allocating for 16 states and transitions. */
DECL|method|Builder
specifier|public
name|Builder
parameter_list|()
...
expr_stmt|;
block|}
comment|/**      * Constructor which creates a builder with enough space for the given      * number of states and transitions.      *       * @param numStates      *           Number of states.      * @param numTransitions      *           Number of transitions.      */
DECL|method|Builder
specifier|public
name|Builder
parameter_list|(
...
parameter_list|()
block|{
comment|// Create automaton with the correct size.
name|int
name|numStates
init|=
name|nextState
...
expr_stmt|;
block|}
comment|// Create all transitions
name|sorter
operator|.
name|sort
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// Copy all states
name|copyStates
argument_list|(
name|other
argument_list|)
expr_stmt|;
comment|// Copy all transitions
name|Transition
name|t
init|=
operator|new
...
block|}
block|}
comment|/** Copies over all states from other. */
DECL|method|copyStates
specifier|public
name|void
name|copyStates
parameter_list|(
...
parameter_list|()
block|{
comment|// TODO: BitSet RAM usage (isAccept.size()/8) isn't fully accurate...
return|return
name|RamUsageEstimator
operator|.
