...
begin_comment
comment|/**  * Estimates the size of Java objects using a simple memory model  * for primitive size information.  *   * @lucene.internal  */
end_comment
...
decl_stmt|;
DECL|field|useUnsafe
DECL|field|isSupportedJVM
specifier|private
specifier|final
specifier|static
name|boolean
...
expr_stmt|;
block|}
comment|// updated best guess based on reference size:
name|objectHeader
operator|=
name|Constants
operator|.
...
block|{
comment|// it is not safe to use Unsafe.objectFieldOffset(),
comment|// as it may be scaled (see "cookie" comment in Unsafe), better use defaults
comment|// and conventional size estimation:
name|objectFieldOffsetM
operator|=
literal|null
expr_stmt|;
...
parameter_list|)
block|{
comment|// on exception ensure useUnsafe will be set to false later:
name|objectFieldOffsetM
operator|=
literal|null
expr_stmt|;
...
decl_stmt|;
block|}
comment|/**     * Returns true, if the current JVM is supported by {@code RamUsageEstimator}.    * If this method returns {@code false} you are maybe using a 3rd party Java VM    * that is not supporting Oracle/Sun private APIs. The memory estimates can be     * imprecise then (no way of detecting compressed references, alignments, etc.).     * Lucene still tries to use sensible defaults.    */
...
block|}
block|}
comment|/**    * Returns the shallow instance size in bytes an instance of the given class would occupy.    * This works with all conventional classes and primitive types, but not with arrays    * (the size then depends on the number of elements and varies from object to object).    * Use the array-instance methods instead.    *     * @throws IllegalArgumentException if {@code clazz} is an array class.     */
...
condition|)
block|{
comment|// no need to recurse to superclasses, as all fields are
comment|// added at the end, so we won't find any larger offset
break|break;
block|}
name|clazz
operator|=
...
return|;
block|}
comment|/**    * Recursive descend into an object.    */
...
parameter_list|)
block|{
comment|// this should never happen as we enable setAccessible()!
throw|throw
operator|new
name|RuntimeException
argument_list|(
literal|"Cannot reflect instance field: "
operator|+
name|f
operator|.
...
block|}
block|}
comment|/**    * Return the deep size of an<code>array</code>, including    * sub-objects if there are any.    *     * @param seen A set of already seen objects. If<code>null</code> no references    *      are followed and this method returns shallow size.    */
DECL|method|measureArraySize
specifier|private
specifier|static
name|long
name|measureArraySize
