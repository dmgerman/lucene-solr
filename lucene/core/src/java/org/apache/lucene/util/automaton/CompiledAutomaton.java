...
name|finite
decl_stmt|;
comment|/** Which state, if any, accepts all suffixes, else -1. */
...
expr_stmt|;
block|}
comment|/** Returns sink state, if present, else -1. */
DECL|method|findSinkState
specifier|private
specifier|static
name|int
name|findSinkState
...
block|{
comment|// NOTE: this is a very costly operation!  We should test if it's really warranted in practice... we could do a fast match
comment|// by looking for a sink state (which means it has no common suffix).  Or maybe we shouldn't do it when simplify is false?:
name|BytesRef
name|suffix
init|=
name|Operations
...
name|automaton
expr_stmt|;
comment|// TODO: this is a bit fragile because if the automaton is not minimized there could be more than 1 sink state but auto-prefix will fail
comment|// to run for those:
name|sinkState
operator|=
name|findSinkState
argument_list|(
