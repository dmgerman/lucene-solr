...
name|exec
decl_stmt|;
DECL|field|partitionsInRAM
specifier|private
specifier|final
name|Semaphore
name|partitionsInRAM
decl_stmt|;
...
name|IllegalArgumentException
argument_list|(
literal|"maxPartitionsInRAM must be> 0; got "
operator|+
name|maxPartitionsInRAM
argument_list|)
throw|;
...
expr_stmt|;
block|}
comment|/** Holds one partition of items, either loaded into memory or based on a file. */
...
name|fileName
decl_stmt|;
comment|/** A partition loaded into memory. */
...
block|}
block|}
comment|/** Read in a single partition of data, setting isExhausted[0] to true if there are no more items. */
...
return|;
block|}
comment|/** Sorts one in-memory partition, writes it to disk, and returns the resulting file-based partition. */
DECL|class|SortPartitionTask
specifier|private
class|class
name|SortPartitionTask
implements|implements
...
DECL|class|MergePartitionsTask
specifier|private
class|class
name|MergePartitionsTask
implements|implements
...
name|segmentsToMerge
decl_stmt|;
DECL|method|MergePartitionsTask
specifier|public
name|MergePartitionsTask
parameter_list|(
name|Directory
