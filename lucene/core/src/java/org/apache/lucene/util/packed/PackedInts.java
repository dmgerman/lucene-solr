...
literal|0
decl_stmt|;
comment|// PackedInts were long-aligned
DECL|field|VERSION_BYTE_ALIGNED
specifier|public
specifier|final
specifier|static
name|int
...
return|;
block|}
comment|/**    * Try to find the number of bits per value that would    * read from disk the fastest reader whose overhead is less than    *<code>acceptableOverheadRatio</code>.    *</p><p>    * The<code>acceptableOverheadRatio</code> parameter makes sense for    * random-access {@link Reader}s. In case you only plan to perform    * sequential access on this stream later on, you should probably use    * {@link PackedInts#COMPACT}.    *</p><p>    */
DECL|method|fastestDirectBits
specifier|public
specifier|static
name|int
...
name|acceptableOverheadPerValue
decl_stmt|;
comment|// first see if we can upgrade to byte
name|int
name|byteAlign
init|=
operator|(
...
return|;
block|}
comment|// otherwise try to upgrade to a nibble boundary (for numbers< 32)
name|int
name|nibbleAlign
init|=
operator|(
...
name|byteCount
decl_stmt|;
comment|// Some consumers of direct readers assume that reading the last value
comment|// will make the underlying IndexInput go to the end of the packed
comment|// stream, but this is not true because packed ints storage used to be
comment|// long-aligned and is now byte-aligned, hence this additional
comment|// condition when reading the last value
return|return
operator|new
