...
block|}
block|}
comment|// TODO: parameterize the FST type<T> and allow passing in a
comment|// comparator; eg maybe your output is a PairOutput and
comment|// one of the outputs in the pair is monotonic so you
comment|// compare by that
...
block|}
block|}
comment|/** Starting from node, find the top N min cost (Long    *  output) completions to a final node.    *    *<p>NOTE: you must share the outputs when you build the    *  FST (pass doShare=true to {@link    *  PositiveIntOutputs#getSingleton}). */
