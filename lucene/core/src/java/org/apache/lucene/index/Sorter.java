...
return|;
block|}
comment|/** Returns the native sort type for {@link SortedSetSortField} and {@link SortedNumericSortField},    * {@link SortField#getType()} otherwise */
...
block|}
block|}
comment|/** Wraps a {@link SortedNumericDocValues} as a single-valued view if the field is an instance of {@link SortedNumericSortField},    * returns {@link NumericDocValues} for the field otherwise. */
DECL|method|getOrWrapNumeric
specifier|static
...
block|}
block|}
comment|/** Wraps a {@link SortedSetDocValues} as a single-valued view if the field is an instance of {@link SortedSetSortField},    * returns {@link SortedDocValues} for the field otherwise. */
DECL|method|getOrWrapSorted
specifier|static
...
return|;
block|}
DECL|interface|NumericDocValuesSupplier
interface|interface
name|NumericDocValuesSupplier
block|{
DECL|method|get
name|NumericDocValues
name|get
...
function_decl|;
block|}
DECL|interface|SortedDocValuesSupplier
interface|interface
name|SortedDocValuesSupplier
block|{
DECL|method|get
name|SortedDocValues
name|get
...
function_decl|;
block|}
comment|/** We cannot use the {@link FieldComparator} API because that API requires that you send it docIDs in order.  Note that this API    *  allocates arrays[maxDoc] to hold the native values needed for comparison, but 1) they are transient (only alive while sorting this one    *  segment), and 2) in the typical index sorting case, they are only used to sort newly flushed segments, which will be smaller than    *  merged segments.  */
DECL|method|getDocComparator
specifier|static
name|DocComparator
name|getDocComparator
