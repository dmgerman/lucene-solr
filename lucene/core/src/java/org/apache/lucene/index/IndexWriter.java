...
block|}
block|}
comment|/** Confirms that the incoming index sort (if any) matches the existing index sort (if any).    *  This is unfortunately just best effort, because it could be the old index only has unsorted flushed segments built    *  before {@link Version#LUCENE_6_5_0} (flushed segments are sorted in Lucene 7.0).  */
...
condition|)
block|{
comment|// Flushed segments are not sorted if they were built with a version prior to 6.5.0
throw|throw
operator|new
name|CorruptIndexException
argument_list|(
literal|"segment not sorted with indexSort="
operator|+
name|segmentIndexSort
argument_list|,
name|info
...
name|IllegalArgumentException
argument_list|(
literal|"cannot update docvalues field involved in the index sort, field="
operator|+
name|field
operator|+
literal|", sort="
...
operator|+
literal|"by a different Lucene version. The current index was generated by Lucene "
operator|+
name|segmentInfos
operator|.
name|getIndexCreatedVersionMajor
argument_list|()
operator|+
literal|" while one of the directories contains an index that was generated with Lucene "
operator|+
name|sis
operator|.
name|getIndexCreatedVersionMajor
...
return|;
block|}
DECL|method|validateMergeReader
specifier|private
name|void
name|validateMergeReader
parameter_list|(
name|CodecReader
name|leaf
parameter_list|)
...
name|IllegalArgumentException
argument_list|(
literal|"Cannot merge a segment that has been created with major version "
operator|+
name|segmentMeta
operator|.
name|getCreatedVersionMajor
argument_list|()
operator|+
literal|" into this index which has been created by major version "
operator|+
name|segmentInfos
operator|.
name|getIndexCreatedVersionMajor
...
name|IllegalStateException
argument_list|(
literal|"Indexes created on or after Lucene 7 must record the created version major, but "
operator|+
name|leaf
operator|+
literal|" hides it"
...
argument_list|)
decl_stmt|;
comment|// We set the min version to null for now, it will be set later by SegmentMerger
name|SegmentInfo
name|info
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// we must do this outside of the commitLock else we can deadlock:
if|if
condition|(
name|doMaybeMerge
index|[
...
expr_stmt|;
block|}
comment|/**    * Sets the commit user data iterator, controlling whether to advance the {@link SegmentInfos#getVersion}.    *    * @see #setLiveCommitData(Iterable)    *    * @lucene.internal */
DECL|method|setLiveCommitData
specifier|public
specifier|final
specifier|synchronized
...
expr_stmt|;
block|}
comment|/**    * Returns the commit user data iterable previously set with {@link #setLiveCommitData(Iterable)}, or null if nothing has been set yet.    */
...
name|ROOT
argument_list|,
literal|"%.1f sec%s to merge segment [%.2f MB, %.2f MB/sec]"
argument_list|,
name|sec
argument_list|,
name|pauseInfo
