...
name|segDocValues
decl_stmt|;
comment|/** True if we are holding RAM only liveDocs or DV updates, i.e. the SegmentCommitInfo delGen doesn't match our liveDocs. */
DECL|field|isNRT
specifier|final
name|boolean
name|isNRT
...
argument_list|)
expr_stmt|;
comment|// We pull liveDocs/DV updates from disk:
name|this
operator|.
name|isNRT
operator|=
...
expr_stmt|;
block|}
comment|/** Create new SegmentReader sharing core from a previous    *  SegmentReader and using the provided liveDocs, and recording    *  whether those liveDocs were carried in ram (isNRT=true). */
DECL|method|SegmentReader
name|SegmentReader
parameter_list|(
name|SegmentCommitInfo
...
return|;
block|}
comment|/** Wrap the cache helper of the core to add ensureOpen() calls that make    *  sure users do not register closed listeners on closed indices. */
DECL|field|coreCacheHelper
specifier|private
specifier|final
name|IndexReader
operator|.
