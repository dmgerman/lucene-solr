...
argument_list|()
decl_stmt|;
comment|// first apply segment-private deletes/updates
name|delCount
operator|+=
...
argument_list|)
expr_stmt|;
comment|// ... then coalesced deletes/updates, so that if there is an update that appears in both, the coalesced updates (carried from
comment|// updates ahead of the segment-privates ones) win:
if|if
condition|(
name|coalescedUpdates
operator|!=
...
argument_list|)
expr_stmt|;
name|applyDocValuesUpdatesList
argument_list|(
name|coalescedUpdates
operator|.
name|numericDVUpdates
...
return|;
block|}
DECL|method|applyDocValuesUpdatesList
specifier|private
specifier|synchronized
name|void
name|applyDocValuesUpdatesList
...
name|IOException
block|{
comment|// we walk backwards through the segments, appending deletion packets to the coalesced updates, so we must apply the packets in reverse
comment|// so that newer packets override older ones:
for|for
control|(
name|int
name|idx
