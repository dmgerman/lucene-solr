...
begin_comment
comment|/**  * A per-document set of presorted byte[] values.  *<p>  * Per-Document values in a SortedDocValues are deduplicated, dereferenced,  * and sorted into a dictionary of unique values. A pointer to the  * dictionary value (ordinal) can be retrieved for each document. Ordinals  * are dense and in increasing sorted order.  *  * @deprecated Use {@link SortedSetDocValues} instead.  */
end_comment

begin_class
annotation|@
name|Deprecated
DECL|class|LegacySortedSetDocValues
specifier|public
specifier|abstract
class|class
...
DECL|method|LegacySortedSetDocValues
specifier|protected
name|LegacySortedSetDocValues
parameter_list|()
block|{}
