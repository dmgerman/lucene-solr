...
begin_comment
comment|/**  * A collection of segmentInfo objects with methods for operating on those  * segments in relation to the file system.  *<p>  * The active segments in the index are stored in the segment info file,  *<tt>segments_N</tt>. There may be one or more<tt>segments_N</tt> files in  * the index; however, the one with the largest generation is the active one  * (when older segments_N files are present it's because they temporarily cannot  * be deleted, or a custom {@link IndexDeletionPolicy} is in  * use). This file lists each segment by name and has details about the codec  * and generation of deletes.  *</p>  *<p>  * Files:  *<ul>  *<li><tt>segments_N</tt>: Header, LuceneVersion, Version, NameCounter, SegCount, MinSegmentLuceneVersion,&lt;SegName,  * HasSegID, SegID, SegCodec, DelGen, DeletionCount, FieldInfosGen, DocValuesGen,  * UpdatesFiles&gt;<sup>SegCount</sup>, CommitUserData, Footer  *</ul>  * Data types:  *<ul>  *<li>Header --&gt; {@link CodecUtil#writeIndexHeader IndexHeader}</li>  *<li>LuceneVersion --&gt; Which Lucene code {@link Version} was used for this commit, written as three {@link DataOutput#writeVInt vInt}: major, minor, bugfix  *<li>MinSegmentLuceneVersion --&gt; Lucene code {@link Version} of the oldest segment, written as three {@link DataOutput#writeVInt vInt}: major, minor, bugfix; this is only  *   written only if there's at least one segment  *<li>NameCounter, SegCount, DeletionCount --&gt;  * {@link DataOutput#writeInt Int32}</li>  *<li>Generation, Version, DelGen, Checksum, FieldInfosGen, DocValuesGen --&gt;  * {@link DataOutput#writeLong Int64}</li>  *<li>HasSegID --&gt; {@link DataOutput#writeByte Int8}</li>  *<li>SegID --&gt; {@link DataOutput#writeByte Int8<sup>ID_LENGTH</sup>}</li>  *<li>SegName, SegCodec --&gt; {@link DataOutput#writeString String}</li>  *<li>CommitUserData --&gt; {@link DataOutput#writeMapOfStrings  * Map&lt;String,String&gt;}</li>  *<li>UpdatesFiles --&gt; Map&lt;{@link DataOutput#writeInt Int32},  * {@link DataOutput#writeSetOfStrings(Set) Set&lt;String&gt;}&gt;</li>  *<li>Footer --&gt; {@link CodecUtil#writeFooter CodecFooter}</li>  *</ul>  * Field Descriptions:  *<ul>  *<li>Version counts how often the index has been changed by adding or deleting  * documents.</li>  *<li>NameCounter is used to generate names for new segment files.</li>  *<li>SegName is the name of the segment, and is used as the file name prefix  * for all of the files that compose the segment's index.</li>  *<li>DelGen is the generation count of the deletes file. If this is -1, there  * are no deletes. Anything above zero means there are deletes stored by  * {@link LiveDocsFormat}.</li>  *<li>DeletionCount records the number of deleted documents in this segment.</li>  *<li>SegCodec is the {@link Codec#getName() name} of the Codec that encoded  * this segment.</li>  *<li>HasSegID is nonzero if the segment has an identifier. Otherwise, when it is 0  * the identifier is {@code null} and no SegID is written. Null only happens for Lucene  * 4.x segments referenced in commits.</li>  *<li>SegID is the identifier of the Codec that encoded this segment.</li>  *<li>CommitUserData stores an optional user-supplied opaque  * Map&lt;String,String&gt; that was passed to  * {@link IndexWriter#setLiveCommitData(Iterable)}.</li>  *<li>FieldInfosGen is the generation count of the fieldInfos file. If this is  * -1, there are no updates to the fieldInfos in that segment. Anything above  * zero means there are updates to fieldInfos stored by {@link FieldInfosFormat}  * .</li>  *<li>DocValuesGen is the generation count of the updatable DocValues. If this  * is -1, there are no updates to DocValues in that segment. Anything above zero  * means there are updates to DocValues stored by {@link DocValuesFormat}.</li>  *<li>UpdatesFiles stores the set of files that were updated in that segment  * per field.</li>  *</ul>  *   * @lucene.experimental  */
end_comment
...
argument_list|>
block|{
comment|/** Adds the {@link Version} that committed this segments_N file, as well as the {@link Version} of the oldest segment, since 5.3+ */
DECL|field|VERSION_53
specifier|public
specifier|static
specifier|final
name|int
...
literal|6
decl_stmt|;
comment|/** The version that added information about the Lucene version at the time when the index has been created. */
DECL|field|VERSION_70
specifier|public
specifier|static
specifier|final
name|int
...
name|id
decl_stmt|;
comment|/** Which Lucene version wrote this commit. */
DECL|field|luceneVersion
specifier|private
name|Version
name|luceneVersion
decl_stmt|;
comment|/** Version of the oldest segment in the index, or null if there are no segments. */
DECL|field|minSegmentLuceneVersion
specifier|private
name|Version
name|minSegmentLuceneVersion
decl_stmt|;
comment|/** The Lucene version major that was used to create the index. */
...
decl_stmt|;
comment|/** Sole constructor.    *  @param indexCreatedVersionMajor the Lucene version major at index creation time, or 6 if the index was created before 7.0 */
...
argument_list|(
literal|"indexCreatedVersionMajor is in the future: "
operator|+
name|indexCreatedVersionMajor
argument_list|)
throw|;
...
name|IllegalArgumentException
argument_list|(
literal|"indexCreatedVersionMajor must be>= 6, got: "
operator|+
name|indexCreatedVersionMajor
argument_list|)
throw|;
...
condition|)
block|{
comment|// TODO: should we check indexCreatedVersion instead?
throw|throw
operator|new
name|IndexFormatTooOldException
argument_list|(
name|input
argument_list|,
literal|"this index is too old (version: "
operator|+
name|luceneVersion
operator|+
literal|")"
...
block|{
comment|// else leave as null: no segments
block|}
name|long
name|totalDocs
init|=
...
name|input
argument_list|,
literal|"Segment is from Lucene 4.x"
argument_list|)
throw|;
block|}
else|else
...
name|CorruptIndexException
argument_list|(
literal|"segments file recorded minSegmentLuceneVersion="
operator|+
name|infos
operator|.
name|minSegmentLuceneVersion
...
name|CorruptIndexException
argument_list|(
literal|"segments file recorded indexCreatedVersionMajor="
operator|+
name|infos
operator|.
name|indexCreatedVersionMajor
...
name|CorruptIndexException
argument_list|(
literal|"segments infos must record minVersion with indexCreatedVersionMajor="
operator|+
name|infos
operator|.
name|indexCreatedVersionMajor
...
parameter_list|)
block|{
comment|// give better error messages if we can, first check if this is a legacy codec
if|if
condition|(
name|unsupportedCodecs
operator|.
...
condition|)
block|{
comment|// We should only get here on pre-5.3 indices, but we can't test this until 7.0 when 5.x indices become too old:
assert|assert
name|unsupportedAllowed
assert|;
name|IOException
...
throw|;
block|}
comment|// or maybe it's an old default codec that moved
if|if
condition|(
name|name
operator|.
...
name|IllegalArgumentException
argument_list|(
literal|"Could not load codec '"
operator|+
name|name
operator|+
literal|"'.  Did you forget to add lucene-backward-codecs.jar?"
argument_list|,
name|e
argument_list|)
throw|;
...
block|{
name|Version
name|minSegmentVersion
init|=
literal|null
decl_stmt|;
comment|// We do a separate loop up front so we can write the minSegmentVersion before
comment|// any SegmentInfo; this makes it cleaner to throw IndexFormatTooOldExc at read time:
for|for
control|(
name|SegmentCommitInfo
name|siPerCommit
...
argument_list|(
literal|"Segments must record minVersion if they have been created on or after Lucene 7: "
operator|+
name|si
argument_list|)
throw|;
...
expr_stmt|;
block|}
comment|// Carry over generation numbers, and version/counter, from another SegmentInfos
DECL|method|updateGenerationVersionAndCounter
name|void
...
name|IllegalArgumentException
argument_list|(
literal|"All segments must record the minVersion for indices created on or after Lucene 7"
argument_list|)
throw|;
block|}
specifier|final
...
return|;
block|}
comment|/** Returns which Lucene {@link Version} wrote this commit, or null if the    *  version this index was written with did not directly record the version. */
DECL|method|getCommitLuceneVersion
specifier|public
name|Version
...
return|;
block|}
comment|/** Returns the version of the oldest segment, or null if there are no segments. */
DECL|method|getMinSegmentLuceneVersion
specifier|public
name|Version
...
return|;
block|}
comment|/** Return the version major that was used to initially create the index.    *  This version is set when the index is first created and then never    *  changes. This information was added as of version 7.0 so older    *  indices report 6 as a creation version. */
DECL|method|getIndexCreatedVersionMajor
specifier|public
name|int
