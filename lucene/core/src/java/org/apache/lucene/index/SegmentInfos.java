...
decl_stmt|;
comment|/** The Lucene version that was used to create the index. */
...
decl_stmt|;
comment|/** Sole constructor.    *  @param indexCreatedVersion the Lucene version at index creation time, or {@code null} if the index was created before 7.0 */
...
argument_list|(
literal|"indexCreatedVersion may only be non-null if the index was created on or after 7.0, got "
operator|+
name|indexCreatedVersion
argument_list|)
throw|;
...
literal|0
case|:
comment|// version is not known: pre-7.0 index that has been modified since the 7.0 upgrade
name|indexCreatedVersion
operator|=
literal|null
expr_stmt|;
...
literal|1
case|:
comment|// version is known: index has been created on or after 7.0
name|indexCreatedVersion
operator|=
name|Version
operator|.
...
name|CorruptIndexException
argument_list|(
literal|"Illegal byte value for a boolean: "
operator|+
name|b
operator|+
...
block|{
comment|// pre-7.0 index that has not been modified since the 7.0 upgrade
name|indexCreatedVersion
operator|=
literal|null
expr_stmt|;
...
return|;
block|}
comment|/** Return the version that was used to initially create the index. This    *  version is set when the index is first created and then never changes.    *  This returns {@code null} if the index was created before 7.0. */
