...
condition|)
block|{
comment|// TODO: in the future (7.0?  sigh) we can use this to throw IndexFormatTooOldException ... or just rely on the
comment|// minSegmentLuceneVersion check instead:
name|infos
operator|.
name|luceneVersion
operator|=
...
block|{
comment|// else compute the min version down below in the for loop
block|}
name|infos
operator|.
name|version
...
name|input
argument_list|,
literal|"this index contains a too-old segment (version: "
operator|+
name|infos
operator|.
name|minSegmentLuceneVersion
...
block|{
comment|// else we recompute it below as we visit segments; it can't be used for throwing IndexFormatTooOldExc, but consumers of
comment|// SegmentInfos can maybe still use it for other reasons
block|}
name|long
name|totalDocs
init|=
