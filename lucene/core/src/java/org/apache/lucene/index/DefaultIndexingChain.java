...
name|termsHash
decl_stmt|;
comment|// Writes stored fields
DECL|field|storedFieldsConsumer
specifier|final
...
expr_stmt|;
block|}
DECL|method|maybeSortSegment
specifier|private
name|Sorter
operator|.
name|DocMap
name|maybeSortSegment
parameter_list|(
...
block|{
comment|// safe to ignore, sort field with no values or already seen before
block|}
block|}
name|Sorter
name|sorter
...
argument_list|)
decl_stmt|;
comment|// returns null if the documents are already sorted
return|return
name|sorter
operator|.
name|sort
...
operator|)
operator|+
literal|" msec to write docValues"
argument_list|)
expr_stmt|;
block|}
name|t0
...
operator|)
operator|+
literal|" msec to finish stored fields"
argument_list|)
expr_stmt|;
block|}
name|t0
...
operator|)
operator|+
literal|" msec to write postings and finish vectors"
argument_list|)
expr_stmt|;
block|}
...
operator|)
operator|+
literal|" msec to write fieldInfos"
argument_list|)
expr_stmt|;
block|}
return|return
...
condition|)
block|{
comment|// Finish each indexed field name seen in the document:
for|for
control|(
name|int
name|i
...
return|;
block|}
DECL|method|verifyUnIndexedFieldType
specifier|private
specifier|static
name|void
name|verifyUnIndexedFieldType
...
comment|// the dimensions were already set to something different:
if|if
condition|(
name|fp
operator|.
...
name|IllegalArgumentException
argument_list|(
literal|"too many tokens for field \""
operator|+
name|field
operator|.
name|name
