...
begin_comment
comment|/**  * A per-document byte[] with presorted values.  *<p>  * Per-Document values in a SortedDocValues are deduplicated, dereferenced,  * and sorted into a dictionary of unique values. A pointer to the  * dictionary value (ordinal) can be retrieved for each document. Ordinals  * are dense and in increasing sorted order.  *  * @deprecated Use {@link SortedDocValues} instead.  */
end_comment

begin_class
annotation|@
name|Deprecated
DECL|class|LegacySortedDocValues
specifier|public
specifier|abstract
class|class
...
DECL|method|LegacySortedDocValues
specifier|protected
name|LegacySortedDocValues
parameter_list|()
block|{}
