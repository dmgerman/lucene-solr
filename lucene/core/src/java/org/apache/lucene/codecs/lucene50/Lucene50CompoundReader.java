...
argument_list|)
expr_stmt|;
comment|// We also validate length, because e.g. if you strip 16 bytes off the .cfs we otherwise
comment|// would not detect it:
if|if
condition|(
name|handle
operator|.
...
name|CorruptIndexException
argument_list|(
literal|"length should be "
operator|+
name|expectedLength
operator|+
literal|" bytes, but is "
operator|+
name|handle
operator|.
name|length
