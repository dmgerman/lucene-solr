...
condition|)
block|{
comment|// TODO: make a real iterator in this case!
name|docsWithField
operator|=
name|getSparseLiveBits
argument_list|(
name|entry
argument_list|)
expr_stmt|;
...
name|index
decl_stmt|;
comment|// index of docId in docIds
DECL|field|docId
name|long
name|docId
decl_stmt|;
comment|// doc ID at index
DECL|field|nextDocId
name|long
name|nextDocId
decl_stmt|;
comment|// doc ID at (index+1)
DECL|method|SparseBits
name|SparseBits
parameter_list|(
name|long
name|maxDoc
...
argument_list|(
literal|"maxDoc must be> the last element of docIDs"
argument_list|)
throw|;
block|}
name|this
...
expr_stmt|;
block|}
comment|/** Gallop forward and stop as soon as an index is found that is greater than      *  the given docId. {@code index} will store an index that stores a value      *  that is&lt;= {@code docId} while the return value will give an index      *  that stores a value that is&gt; {@code docId}. These indices can then be      *  used to binary search. */
DECL|method|gallop
specifier|private
name|long
name|gallop
...
literal|1
expr_stmt|;
comment|// double the step each time
block|}
return|return
name|hiIndex
return|;
...
parameter_list|)
block|{
comment|// seek forward by doubling the interval on each iteration
specifier|final
name|long
name|hiIndex
init|=
...
argument_list|)
assert|;
comment|// now perform the actual binary search
name|binarySearch
argument_list|(
name|hiIndex
argument_list|,
...
condition|)
block|{
comment|// reading doc IDs backward, go back to the start
name|reset
argument_list|()
expr_stmt|;
block|}
