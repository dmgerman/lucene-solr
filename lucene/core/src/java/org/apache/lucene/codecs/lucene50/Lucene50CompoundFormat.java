...
init|)
block|{
comment|// just copies the index header, verifying that its id matches what we expect
name|CodecUtil
operator|.
...
argument_list|)
expr_stmt|;
comment|// copy all bytes except the footer
name|long
name|numBytesToCopy
init|=
name|in
...
argument_list|)
expr_stmt|;
comment|// verify footer (checksum) matches for the incoming file we are copying
name|long
name|checksum
init|=
name|CodecUtil
...
argument_list|)
decl_stmt|;
comment|// this is poached from CodecUtil.writeFooter, but we need to use our own checksum, not data.getChecksum(), but I think
comment|// adding a public method to CodecUtil to do that is somewhat dangerous:
name|data
operator|.
name|writeInt
argument_list|(
