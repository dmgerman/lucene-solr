...
block|}
decl_stmt|;
comment|/** Compress given int[] into Integer buffer, with PFor format    *    * @param data        uncompressed data    * @param intBuffer   integer buffer to hold compressed data    * @return block header    */
...
comment|// (first exception ranges from -1 ~ 254)
name|long
name|maxChainFirst
init|=
literal|254
...
expr_stmt|;
block|}
comment|/**    * Encode exception values into exception area.    * The width for each exception will be fixed as:    * 1, 2, or 4 byte(s).    */
...
block|}
block|}
comment|/**    * Save only header when the whole block equals to 1    */
...
block|}
comment|/**     * Generate the 4 byte header which contains (from lsb to msb):    *    * 8 bits for encoded block int size (excluding header, this limits DEFAULT_BLOCK_SIZE<= 2^(8-1))    *    * 8 bits for exception num - 1 (when no exceptions, this is undefined)    *    * 8 bits for the index of the first exception + 1 (when no exception, this is 0)    *    * 6 bits for num of frame bits (when 0, values in this block are all the same)    * 2 bits for the exception code: 00: byte, 01: short, 10: int    *    */
