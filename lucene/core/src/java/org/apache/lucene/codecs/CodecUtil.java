...
return|;
block|}
comment|/**    * Expert: verifies the incoming {@link IndexInput} has an index header    * and that its segment ID matches the expected one, and then copies    * that index header into the provided {@link DataOutput}.  This is    * useful when building compound files.    *    * @param in Input stream, positioned at the point where the    *        index header was previously written. Typically this is located    *        at the beginning of the file.    * @param out Output stream, where the header will be copied to.    * @param expectedID Expected segment ID    * @throws CorruptIndexException If the first four bytes are not    *         {@link #CODEC_MAGIC}, or if the<code>expectedID</code>    *         does not match.    * @throws IOException If there is an I/O error reading from the underlying medium.    *    * @lucene.internal     */
DECL|method|verifyAndCopyIndexHeader
specifier|public
specifier|static
name|void
name|verifyAndCopyIndexHeader
parameter_list|(
name|IndexInput
name|in
parameter_list|,
...
name|IOException
block|{
comment|// make sure it's large enough to have a header and footer
if|if
condition|(
name|in
operator|.
...
name|CorruptIndexException
argument_list|(
literal|"compound sub-files must have a valid codec header and footer: file is too small ("
operator|+
name|in
operator|.
name|length
...
name|CorruptIndexException
argument_list|(
literal|"compound sub-files must have a valid codec header and footer: codec header mismatch: actual header="
operator|+
name|actualHeader
operator|+
literal|" vs expected header="
...
throw|;
block|}
comment|// we can't verify these, so we pass-through:
name|String
name|codec
init|=
name|in
...
argument_list|)
expr_stmt|;
comment|// we can't verify extension either, so we pass-through:
name|int
name|suffixLength
init|=
name|in
...
argument_list|)
expr_stmt|;
comment|// now write the header we just verified
name|out
operator|.
name|writeInt
argument_list|(
