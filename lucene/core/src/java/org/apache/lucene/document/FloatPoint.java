...
name|Field
block|{
comment|/**    * Return the least float that compares greater than {@code f} consistently    * with {@link Float#compare}. The only difference with    * {@link Math#nextUp(float)} is that this method returns {@code +0f} when    * the argument is {@code -0f}.    */
DECL|method|nextUp
specifier|public
specifier|static
name|float
...
return|;
block|}
comment|/**    * Return the greatest float that compares less than {@code f} consistently    * with {@link Float#compare}. The only difference with    * {@link Math#nextDown(float)} is that this method returns {@code -0f} when    * the argument is {@code +0f}.    */
