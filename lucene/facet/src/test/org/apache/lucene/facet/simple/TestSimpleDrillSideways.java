...
argument_list|)
decl_stmt|;
comment|//String s = _TestUtil.randomSimpleString(random());
if|if
condition|(
name|s
operator|.
...
argument_list|)
decl_stmt|;
comment|// nocommit subclass& override to use FacetsTestCase.getFacetCounts
name|SimpleDrillSideways
name|ds
decl_stmt|;
if|if
...
block|}
block|}
DECL|class|SimpleTestFacetResult
specifier|private
specifier|static
class|class
name|SimpleTestFacetResult
...
expr_stmt|;
block|}
comment|// nocommit if we add this to SimpleFR then re-enable this:
comment|// assertEquals("dim=" + dim, expected.uniqueCounts[dim], fr.getNumValidDescendants());
block|}
block|}
