...
begin_package
DECL|package|org.apache.lucene.facet.collections
package|package
name|org
operator|.
name|apache
...
block|}
block|}
comment|/** Implements an Iterator, used for iteration over the map's keys. */
...
block|}
block|}
comment|/** Implements an Iterator, used for iteration over the map's values. */
...
literal|16
decl_stmt|;
comment|/**    * Holds the base hash entries. if the capacity is 2^N, than the base hash    * holds 2^(N+1).    */
DECL|field|baseHash
name|int
index|[]
name|baseHash
...
name|prev
decl_stmt|;
comment|/** Number of currently stored objects in the map. */
DECL|field|size
specifier|private
name|int
name|size
decl_stmt|;
comment|/** Holds the values. */
DECL|field|values
name|Object
index|[]
name|values
...
expr_stmt|;
block|}
comment|/**    * Adds a pair to the map. Takes the first empty position from the    * empty-linked-list's head - {@link #firstEmpty}. New pairs are always    * inserted to baseHash, and are followed by the old colliding pair.    */
...
expr_stmt|;
block|}
comment|/** Calculating the baseHash index using the internal internal<code>hashFactor</code>. */
...
expr_stmt|;
block|}
comment|/** Returns true iff the key exists in the map. */
...
return|;
block|}
comment|/** Returns true iff the object exists in the map. */
...
return|;
block|}
comment|/** Returns the index of the given key, or zero if the key wasn't found. */
DECL|method|find
specifier|protected
name|int
name|find
...
return|;
block|}
comment|/**    * Finds the actual index of a given key with it's baseHashIndex. Some methods    * use the baseHashIndex. If those call {@link #find} there's no need to    * re-calculate that hash.    *     * @return the index of the given key, or 0 if the key wasn't found.    */
...
expr_stmt|;
block|}
comment|// If we got thus far, it could only mean we did not find the key we
comment|// were asked for. return 'Ground' index.
return|return
name|prev
operator|=
...
return|;
block|}
comment|/** Returns the object mapped with the given key, or null if the key wasn't found. */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
...
return|;
block|}
comment|/**    * Allocates a new map of double the capacity, and fast-insert the old    * key-value pairs.    */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
...
expr_stmt|;
block|}
comment|/** Returns true iff the map is empty. */
DECL|method|isEmpty
specifier|public
name|boolean
name|isEmpty
...
return|;
block|}
comment|/** Prints the baseHash array, used for debugging purposes. */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unused"
...
return|;
block|}
comment|/**    * Inserts the&lt;key,value&gt; pair into the map. If the key already exists,    * this method updates the mapped value to the given one, returning the old    * mapped value.    *     * @return the old mapped value, or null if the key didn't exist.    */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
...
return|;
block|}
comment|/**    * Removes a&lt;key,value&gt; pair from the map and returns the mapped value,    * or null if the none existed.    *     * @param key used to find the value to remove    * @return the removed value or null if none existed.    */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
...
return|;
block|}
comment|/** Returns number of pairs currently in the map. */
DECL|method|size
specifier|public
name|int
name|size
...
return|;
block|}
comment|/**    * Translates the mapped pairs' values into an array of V    *     * @param a the array into which the elements of the list are to be stored, if    *        it is big enough; otherwise, use as much space as it can.    * @return an array containing the elements of the list    */
DECL|method|toArray
specifier|public
name|V
index|[]
