...
parameter_list|)
block|{
comment|// can't happen - LabelRepository.append() never throws an
comment|// exception
block|}
name|a
operator|.
...
condition|)
block|{
comment|// First component is numcomponents, so we initialize the hash
comment|// to this
name|int
name|ncomponents
init|=
...
name|ncomponents
decl_stmt|;
comment|// If ncomponents is 0, then we are done?
if|if
condition|(
name|ncomponents
operator|!=
...
condition|)
block|{
comment|// usedchars is always the last member of the 'ends' array
comment|// in serialization. Rather than rebuild the entire array,
comment|// assign usedchars to the last value we read in. This will
comment|// be slightly more memory efficient.
name|int
name|usedchars
init|=
literal|0
...
expr_stmt|;
block|}
comment|// Hash the usedchars for this label
for|for
control|(
name|int
name|i
