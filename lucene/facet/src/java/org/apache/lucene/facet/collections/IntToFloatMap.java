...
name|keys
decl_stmt|;
comment|/**    * In case of collisions, we implement a float linked list of the colliding    * hash's with the following next[] and prev[]. Those are also used to store    * the "empty" list.    */
DECL|field|next
name|int
index|[]
name|next
...
return|;
block|}
comment|/**    * Checks if the given value exists in the map.<br>    * This method iterates over the collection, trying to find an equal object.    *     * @param value    *            float value that is checked against the map data.    * @return true if the value exists in the map, false otherwise.    */
...
return|;
block|}
comment|/**    * Returns the value mapped with the given key.    *     * @param key    *            int who's mapped object we're interested in.    * @return a float value mapped by the given key. float.NaN if the key wasn't found.    */
DECL|method|get
specifier|public
name|float
name|get
...
return|;
block|}
comment|/**    * Grows the map. Allocates a new map of float the capacity, and    * fast-insert the old key-value pairs.    */
DECL|method|grow
specifier|protected
name|void
name|grow
...
return|;
block|}
comment|/**    * Returns a new iterator for the mapped float values.    */
...
return|;
block|}
comment|/**    * Translates the mapped pairs' values into an array of Objects    *     * @return a float array of all the values currently in the map.    */
DECL|method|toArray
specifier|public
name|float
index|[]
