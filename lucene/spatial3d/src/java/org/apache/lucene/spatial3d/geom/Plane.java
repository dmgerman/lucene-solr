...
expr_stmt|;
block|}
comment|/**    * Construct a plane through two points and origin.    *    * @param A is the first point (origin based).    * @param BX is the second point X (origin based).    * @param BY is the second point Y (origin based).    * @param BZ is the second point Z (origin based).    */
DECL|method|Plane
specifier|public
name|Plane
parameter_list|(
...
expr_stmt|;
block|}
comment|/** Construct a plane that is parallel to the one provided, but which is just barely numerically    * distinguishable from it, in the direction desired.    * @param basePlane is the starting plane.    * @param above is set to true if the desired plane is in the positive direction from the base plane,    *   or false in the negative direction.    */
DECL|method|Plane
specifier|public
name|Plane
parameter_list|(
...
name|inverse2A
decl_stmt|;
comment|// Maybe we can save ourselves the cost of construction of a point?
specifier|final
name|double
name|pointX
init|=
...
name|inverse2A
decl_stmt|;
comment|// Up to two points being returned.  Do what we can to save on object creation though.
specifier|final
name|double
name|point1X
init|=
...
parameter_list|)
block|{
comment|// This code in this method is very similar to findIntersections(), but eliminates the cases where
comment|// crossings are detected.
comment|// Unnormalized, unchecked...
specifier|final
name|double
name|lineVectorX
...
condition|)
block|{
comment|// One point of intersection: cannot be a crossing.
return|return
name|NO_POINTS
return|;
block|}
...
name|denomYZ
decl_stmt|;
comment|// Each value of D really is two values of D.  That makes 4 combinations.
name|recordLineBounds
argument_list|(
name|planetModel
argument_list|,
name|boundsInfo
...
block|}
block|}
comment|/**    * Accumulate bounds information for this plane, intersected with another plane    * and the world.    * Updates both latitude and longitude information, using max/min points found    * within the specified bounds.  Also takes into account the error envelope for all    * planes being intersected.    *    * @param planetModel is the planet model to use in determining bounds.    * @param boundsInfo is the lat/lon info to update with additional bounding information.    * @param p is the other plane.    * @param bounds     are the surfaces delineating what's inside the shape.    */
DECL|method|recordBounds
specifier|public
name|void
name|recordBounds
...
return|;
block|}
comment|// Save on allocations; do inline instead of calling findIntersections
comment|//System.err.println("Looking for intersection between plane "+this+" and plane "+q+" within bounds");
comment|// Unnormalized, unchecked...
specifier|final
name|double
...
argument_list|(
literal|"Can't find arc distance using plane that doesn't go through origin"
argument_list|)
throw|;
block|}
if|if
...
name|IllegalArgumentException
argument_list|(
literal|"Start point is not on plane"
argument_list|)
throw|;
block|}
assert|assert
...
name|MINIMUM_RESOLUTION_SQUARED
operator|:
literal|"Plane needs to be normalized"
assert|;
comment|// The first step is to rotate coordinates for the point so that the plane lies on the x-y plane.
comment|// To acheive this, there will need to be three rotations:
comment|// (1) rotate the plane in x-y so that the y axis lies in it.
comment|// (2) rotate the plane in x-z so that the plane lies on the x-y plane.
comment|// (3) rotate in x-y so that the starting vector points to (1,0,0).
comment|// This presumes a normalized plane!!
specifier|final
name|double
name|azimuthMagnitude
init|=
name|Math
operator|.
name|sqrt
...
specifier|final
name|double
name|sinPlaneAltitude
init|=
name|azimuthMagnitude
decl_stmt|;
specifier|final
name|double
name|cosPlaneAzimuth
init|=
name|this
operator|.
name|x
...
name|MINIMUM_RESOLUTION
operator|:
literal|"Improper sin/cos of altitude: "
operator|+
operator|(
name|sinPlaneAltitude
operator|*
...
name|MINIMUM_RESOLUTION
operator|:
literal|"Improper sin/cos of azimuth: "
operator|+
operator|(
name|sinPlaneAzimuth
operator|*
...
comment|// xT = xS cos T - yS sin T
comment|// yT = xS sin T + yS cos T
comment|// But we're rotating backwards, so use:
comment|// sin (-T) = -sin (T)
comment|// cos (-T) = cos (T)
comment|// Now, rotate startpoint in x-y
specifier|final
name|double
name|x0
init|=
...
name|MINIMUM_RESOLUTION
operator|:
literal|"Rotation should have put startpoint on x-y plane, instead has value "
operator|+
name|z2
assert|;
comment|// Ok, we have the start point on the x-y plane.  To apply the arc distance, we
comment|// next need to convert to an angle (in radians).
specifier|final
name|double
name|startAngle
init|=
...
argument_list|)
decl_stmt|;
comment|// To apply the arc distance, just add to startAngle.
specifier|final
name|double
name|point1Angle
init|=
...
name|arcDistanceValue
decl_stmt|;
comment|// Convert each point to x-y
specifier|final
name|double
name|point1x2
init|=
...
literal|0.0
decl_stmt|;
comment|// Now, do the reverse rotations for both points
comment|// Altitude...
specifier|final
name|double
name|point1x1
...
argument_list|)
decl_stmt|;
comment|// Figure out what to return
name|boolean
name|isPoint1Inside
init|=
name|meetsAllBounds
