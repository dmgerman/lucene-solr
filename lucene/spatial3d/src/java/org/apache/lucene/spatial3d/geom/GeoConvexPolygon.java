...
literal|null
decl_stmt|;
comment|/** Map from edge to its previous non-coplanar brother */
DECL|field|prevBrotherMap
specifier|protected
name|Map
argument_list|<
name|SidedPlane
...
literal|null
decl_stmt|;
comment|/** Map from edge to its next non-coplanar brother */
DECL|field|nextBrotherMap
specifier|protected
name|Map
argument_list|<
name|SidedPlane
...
argument_list|)
expr_stmt|;
comment|// Also confirm that all interior points are within the bounds
name|int
name|startingIndex
init|=
name|bound2Index
...
name|IllegalArgumentException
argument_list|(
literal|"Convex polygon has a side that is more than 180 degrees"
argument_list|)
throw|;
block|}
block|}
...
argument_list|)
expr_stmt|;
comment|// When we are done with this cycle, we'll need to build the intersection bound for each edge and its brother.
comment|// For now, keep track of the relationships.
name|nextBrotherMap
operator|.
name|put
argument_list|(
...
expr_stmt|;
block|}
comment|// Add all the points and the intersections
for|for
control|(
specifier|final
name|GeoPoint
