...
name|Node
block|{
DECL|field|minimumValue
specifier|public
specifier|final
name|double
name|minimumValue
decl_stmt|;
DECL|field|maximumValue
specifier|public
specifier|final
name|double
name|maximumValue
...
literal|1
decl_stmt|;
DECL|field|OVERLAPS_MINIMUM
specifier|protected
specifier|static
specifier|final
name|int
name|OVERLAPS_MINIMUM
init|=
literal|2
decl_stmt|;
DECL|field|OVERLAPS_MAXIMUM
specifier|protected
specifier|static
specifier|final
name|int
name|OVERLAPS_MAXIMUM
init|=
literal|3
decl_stmt|;
DECL|field|LESS
...
literal|5
decl_stmt|;
comment|/** Add a new edge to the tree.      * @param edge is the edge to add.      */
DECL|method|add
specifier|public
name|void
name|add
...
parameter_list|)
function_decl|;
comment|/** Worker method for adding an edge.      * @param node is the node to add into.      * @param newEdge is the new edge to add.      * @param minimumValue is the minimum limit of the subrange of the edge we'll be adding.      * @param maximumValue is the maximum limit of the subrange of the edge we'll be adding.      * @return the updated node reference.      */
DECL|method|addEdge
specifier|protected
name|Node
name|addEdge
...
name|CONTAINED
case|:
comment|// The node is contained in the range provided.  We need to create a new node and insert
comment|// it into the "within" chain.
specifier|final
name|Node
name|rval
init|=
...
argument_list|(
literal|"Unexpected comparison result: "
operator|+
name|result
argument_list|)
throw|;
...
parameter_list|)
block|{
comment|// This is tricky because edges are duplicated in the tree (where they got split).
comment|// We need to eliminate those duplicate edges as we traverse.  This requires us to keep a set of edges we've seen.
comment|// Luckily, the number of edges we're likely to encounter in a real-world situation is small, so we can get away with it.
return|return
name|traverseEdges
argument_list|(
name|rootNode
...
block|{
comment|// There's overlap with the current node, and there may also be overlap with the lesser side and greater side
if|if
condition|(
name|minValue
operator|<
...
specifier|final
name|double
name|nodeMinimumValue
parameter_list|,
specifier|final
name|double
name|nodeMaximumValue
parameter_list|,
specifier|final
name|double
name|minimumValue
...
name|println
argument_list|(
literal|" Check point is on edge: isWithin = true"
argument_list|)
expr_stmt|;
return|return
literal|false
...
name|println
argument_list|(
literal|" All crossing points processed"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|println
argument_list|(
literal|" Crosses intersection point."
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|println
argument_list|(
literal|"  Already processed on previous leg: exit"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|println
argument_list|(
literal|" Crossing point = edge.startPoint"
argument_list|)
expr_stmt|;
comment|// We have to figure out if this crossing should be counted.
comment|// Does the crossing for this edge go up, or down?  Or can't we tell?
...
name|println
argument_list|(
literal|" Crossing point = edge.endPoint"
argument_list|)
expr_stmt|;
comment|// Figure out if the crossing should be counted.
comment|// Does the crossing for this edge go up, or down?  Or can't we tell?
...
name|println
argument_list|(
literal|" Not a special case: incrementing crossing count"
argument_list|)
expr_stmt|;
