...
return|;
block|}
comment|// Choose our navigation route!
specifier|final
name|double
name|xDelta
init|=
...
block|{
comment|// We need to use two planes to get there.  We can use any two planes, and order doesn't matter.
comment|// The best to pick are the ones with the shortest overall distance.
if|if
condition|(
name|xDelta
operator|+
...
condition|)
block|{
comment|// Travel in X and Y
comment|// We'll do this using the testPointYZPlane, and create a travel plane for the right XZ plane.
specifier|final
name|Plane
name|travelPlane
init|=
...
condition|)
block|{
comment|// Travel in X and Z
comment|// We'll do this using the testPointXYPlane, and create a travel plane for the right YZ plane.
specifier|final
name|Plane
name|travelPlane
init|=
...
condition|)
block|{
comment|// Travel in Y and Z
comment|// We'll do this using the testPointXZPlane, and create a travel plane for the right XY plane.
specifier|final
name|Plane
name|travelPlane
init|=
...
argument_list|)
expr_stmt|;
comment|// Now, find the intersection of the check and test point planes.
specifier|final
name|GeoPoint
index|[]
...
literal|null
operator|:
literal|"couldn't find any intersections"
assert|;
assert|assert
name|intersectionPoints
operator|.
...
literal|1
operator|:
literal|"wrong number of intersection points"
assert|;
name|this
operator|.
name|intersectionPoint
...
literal|1
operator|:
literal|"Can be exactly one inside point"
assert|;
if|if
condition|(
name|aboveAbove
...
expr_stmt|;
block|}
comment|// MHL - this code below is temporary code copied from LinearCrossing above
if|if
condition|(
name|crossingPoint
operator|.
...
name|GeoPoint
index|[]
name|assessInsideIntersections
decl_stmt|;
name|GeoPoint
index|[]
name|assessOutsideIntersections
decl_stmt|;
while|while
condition|(
literal|true
