...
name|testPointInSet
decl_stmt|;
DECL|field|testPointZPlane
specifier|private
specifier|final
name|Plane
name|testPointZPlane
decl_stmt|;
...
parameter_list|)
block|{
comment|// Construct a horizontal plane that goes through the provided z value.  This, along with the
comment|// testPointZPlane, will provide a way of counting intersections between this point and the test point.
comment|// We use z exclusively for this logic at the moment but we may in the future choose x or y based on which
comment|// is bigger.
if|if
condition|(
name|testPoint
...
condition|)
block|{
comment|// The point we are assessing goes through only one plane.
comment|// Compute cutoff planes
specifier|final
name|SidedPlane
...
argument_list|)
decl_stmt|;
comment|// Compute bounds for this part of testZPlane
specifier|final
name|XYZBounds
name|testZPlaneBounds
init|=
operator|new
name|XYZBounds
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Pick which tree to use
specifier|final
name|double
name|xDelta
init|=
...
block|{
comment|// The point requires two planes
specifier|final
name|Plane
name|xyPlane
init|=
...
argument_list|)
decl_stmt|;
comment|// We need to plan a path from the test point to the point to be evaluated.
comment|// This requires finding the intersection point between the xyPlane and the testPointZPlane
comment|// that is closest to our point.
comment|// MHL
block|}
return|return
literal|false
...
DECL|class|CrossingEdgeIterator
specifier|private
class|class
name|CrossingEdgeIterator
implements|implements
...
literal|0
decl_stmt|;
DECL|method|CrossingEdgeIterator
specifier|public
name|CrossingEdgeIterator
parameter_list|(
specifier|final
