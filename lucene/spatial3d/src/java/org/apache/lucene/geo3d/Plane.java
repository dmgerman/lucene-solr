...
expr_stmt|;
block|}
comment|/** Construct the most accurate normalized plane through an x-y point and including the Z axis.    * If none of the points can determine the plane, return null.    * @param planePoints is a set of points to choose from.  The best one for constructing the most precise plane is picked.    * @return the plane    */
DECL|method|constructNormalizedZPlane
specifier|public
specifier|static
name|Plane
...
return|;
block|}
comment|/** Construct the most accurate normalized plane through an x-z point and including the Y axis.    * If none of the points can determine the plane, return null.    * @param planePoints is a set of points to choose from.  The best one for constructing the most precise plane is picked.    * @return the plane    */
DECL|method|constructNormalizedYPlane
specifier|public
specifier|static
name|Plane
...
parameter_list|)
block|{
comment|// Pick the best one (with the greatest x-z distance)
name|double
name|bestDistance
init|=
literal|0.0
...
return|;
block|}
comment|/** Construct the most accurate normalized plane through an y-z point and including the X axis.    * If none of the points can determine the plane, return null.    * @param planePoints is a set of points to choose from.  The best one for constructing the most precise plane is picked.    * @return the plane    */
DECL|method|constructNormalizedXPlane
specifier|public
specifier|static
name|Plane
...
parameter_list|)
block|{
comment|// Pick the best one (with the greatest y-z distance)
name|double
name|bestDistance
init|=
literal|0.0
...
return|;
block|}
comment|/** Construct a normalized plane through an x-y point and including the Z axis.    * If the x-y point is at (0,0), return null.    * @param x is the x value.    * @param y is the y value.    * @return a plane passing through the Z axis and (x,y,0).    */
DECL|method|constructNormalizedZPlane
specifier|public
specifier|static
name|Plane
...
comment|/**    * Accumulate (x,y,z) bounds information for this plane, intersected with the unit sphere.    * Updates min/max information, using max/min points found    * within the specified bounds.    *    * @param planetModel is the planet model to use in determining bounds.    * @param boundsInfo is the xyz info to update with additional bounding information.    * @param bounds     are the surfaces delineating what's inside the shape.    */
...
parameter_list|)
block|{
comment|// Basic plan is to do three intersections of the plane and the planet.
comment|// For min/max x, we intersect a vertical plane such that y = 0.
comment|// For min/max y, we intersect a vertical plane such that x = 0.
comment|// For min/max z, we intersect a vertical plane that is chosen to go through the high point of the arc.
...
condition|)
block|{
comment|//System.err.println("    computing Z bound");
comment|// Compute Z bounds for this arc
...
condition|)
block|{
comment|// NOT a degenerate case
comment|//System.err.println("    not degenerate");
specifier|final
name|Plane
name|normalizedZPlane
init|=
name|constructNormalizedZPlane
argument_list|(
name|A
...
argument_list|)
assert|;
comment|//System.err.println("      Point = "+point+"; this.evaluate(point)="+this.evaluate(point)+"; normalizedZPlane.evaluate(point)="+normalizedZPlane.evaluate(point));
name|addPoint
argument_list|(
name|boundsInfo
argument_list|,
...
block|{
comment|// Since a==b==0, any plane including the Z axis suffices.
comment|//System.err.println("      Perpendicular to z");
specifier|final
name|GeoPoint
index|[]
name|points
...
block|}
block|}
comment|/**    * Accumulate bounds information for this plane, intersected with the unit sphere.    * Updates both latitude and longitude information, using max/min points found    * within the specified bounds.    *    * @param planetModel is the planet model to use in determining bounds.    * @param boundsInfo is the lat/lon info to update with additional bounding information.    * @param bounds     are the surfaces delineating what's inside the shape.    */
DECL|method|recordBounds
specifier|public
name|void
name|recordBounds
...
block|{
comment|// Horizontal circle.  Since a==b, any vertical plane suffices.
specifier|final
name|GeoPoint
index|[]
name|points
