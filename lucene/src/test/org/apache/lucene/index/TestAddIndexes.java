...
argument_list|()
expr_stmt|;
comment|// we should now see segments_X, segments.gen,_Y.cfs,_Y.cfe, _Z.fnx
name|assertEquals
argument_list|(
...
expr_stmt|;
block|}
comment|// LUCENE-3126: tests that if a non-CFS segment is copied, it is converted to
comment|// a CFS, given MP preferences
DECL|method|testCopyIntoCFS
specifier|public
name|void
name|testCopyIntoCFS
parameter_list|()
throws|throws
name|Exception
block|{
comment|// create an index, no CFS (so we can assert that existing segments are not affected)
name|Directory
name|target
init|=
name|newDirectory
...
argument_list|)
expr_stmt|;
comment|// prepare second index, no-CFS too + .del file + separate norms file
name|Directory
name|src
init|=
name|newDirectory
...
argument_list|()
expr_stmt|;
comment|// create separate norms file
name|IndexReader
name|r
init|=
name|IndexReader
...
name|assertTrue
argument_list|(
literal|"separate norms file not found"
argument_list|,
name|src
operator|.
name|fileExists
...
argument_list|)
expr_stmt|;
comment|// Case 1: force 'CFS' on target
name|lmp
operator|.
name|setUseCompoundFile
argument_list|(
...
name|assertFalse
argument_list|(
literal|"existing segments should not be modified by addIndexes"
argument_list|,
name|w
operator|.
name|segmentInfos
...
name|assertTrue
argument_list|(
literal|"segment should have been converted to a CFS by addIndexes"
argument_list|,
name|w
operator|.
name|segmentInfos
...
argument_list|)
expr_stmt|;
comment|// Case 2: LMP disallows CFS
name|lmp
operator|.
name|setUseCompoundFile
argument_list|(
...
name|assertFalse
argument_list|(
literal|"segment should not have been converted to a CFS by addIndexes if MP disallows"
argument_list|,
name|w
operator|.
name|segmentInfos
