...
name|state
decl_stmt|;
comment|// capture test execution state
name|state
operator|=
name|State
operator|.
name|INITIAL
expr_stmt|;
comment|// set the state for subsequent tests
name|Throwable
name|problem
init|=
literal|null
...
name|testCaseThread
decl_stmt|;
comment|/** @see #testCaseThread */
annotation|@
name|Rule
DECL|field|setTestThread
specifier|public
specifier|final
name|MethodRule
name|setTestThread
...
expr_stmt|;
comment|// set the state for subsequent tests
comment|// NOTE: with junit 4.7, we don't get a reproduceWith because our Watchman
comment|// does not know if something fails in tearDown. so we ensure this happens ourselves for now.
comment|// we can remove this if we upgrade to 4.8
name|Throwable
name|problem
init|=
literal|null
...
return|;
block|}
comment|/**    * Returns a new Directory instance, using the specified random. You    * can specify maybeWrap as to whether the directory might be also    * wrapped by NRTCachingDirectory or FileSwitchDirectory    * See {@link #newDirectory()} for more information.    */
DECL|method|newDirectory
specifier|public
specifier|static
name|MockDirectoryWrapper
