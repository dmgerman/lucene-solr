...
name|writer
decl_stmt|;
comment|// TODO: we can save RAM here by using growable packed
comment|// ints...:
...
expr_stmt|;
block|}
comment|/** Expert: creates an FST by packing this one.  This    *  process requires substantial additional RAM (currently    *  ~8 bytes per node), but then should produce a smaller FST. */
DECL|method|pack
specifier|public
name|FST
argument_list|<
...
block|}
comment|// TODO: we can use packed ints:
comment|// +1 because node ords start at 1 (0 is reserved as
comment|// stop node):
specifier|final
name|int
index|[]
