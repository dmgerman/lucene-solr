...
expr_stmt|;
block|}
comment|// transferFrom does not change the position of the channel. Need to change it manually
name|output
operator|.
name|position
argument_list|(
...
name|IOException
block|{
comment|// Optimized copy only if the number of bytes to copy is larger than the
comment|// buffer size, and the given IndexInput supports FileChannel copying ..
comment|// NOTE: the below check relies on NIOIndexInput extending Simple. If that
...
argument_list|)
expr_stmt|;
comment|// corrects the position in super (BufferedIndexOutput), so that calls
comment|// to getFilePointer will return the correct pointer.
comment|// Perhaps a specific method is better?
name|super
operator|.
name|seek
argument_list|(
