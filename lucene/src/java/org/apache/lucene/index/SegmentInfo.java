...
return|;
block|}
comment|/**    * Returns total size in bytes of all of files used by this segment (if    * {@code includeDocStores} is true), or the size of all files except the    * store files otherwise.    *<p>    * NOTE: includeDocStores=false should only be used for debugging.    * Theoretically a codec could combine its files however it wants (after-    * the-fact or something), and this calculation is not particularly    * efficient.    */
DECL|method|sizeInBytes
name|long
name|sizeInBytes
parameter_list|(
...
name|IOException
block|{
comment|// TODO: based on how this is used, can't we just forget about all this docstore crap?
comment|// its really an abstraction violation into the codec
if|if
condition|(
name|includeDocStores
condition|)
...
return|;
block|}
comment|// TODO: a little messy, but sizeInBytes above that uses this is the real problem.
