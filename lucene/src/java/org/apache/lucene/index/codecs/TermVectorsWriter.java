...
block|{
comment|// no terms in this field (hmm why is field present
comment|// then...?)
assert|assert
name|numTerms
operator|==
...
literal|1
decl_stmt|;
comment|// NOTE: we already .next()'d the TermsEnum above, to
comment|// peek @ first term to see if positions/offsets are
comment|// present
while|while
condition|(
literal|true
...
argument_list|)
decl_stmt|;
comment|// TODO: add startOffset()/endOffset() to d&pEnum... this is insanity
if|if
condition|(
name|dp
operator|!=
...
condition|)
block|{
comment|// producer didnt reuse, must re-pull attributes
if|if
condition|(
name|offsetAtt
operator|!=
