...
begin_comment
comment|/** Forked from {@link BKDWriter} and simplified/specialized for SimpleText's usage */
end_comment

begin_class
DECL|class|SimpleTextBKDWriter
specifier|final
class|class
...
literal|0
decl_stmt|;
DECL|field|VERSION_COMPRESSED_DOC_IDS
specifier|public
specifier|static
specifier|final
name|int
...
literal|1
decl_stmt|;
DECL|field|VERSION_COMPRESSED_VALUES
specifier|public
specifier|static
specifier|final
name|int
...
name|scratch2
decl_stmt|;
DECL|field|scratchBytesRef1
specifier|final
name|BytesRef
name|scratchBytesRef1
init|=
operator|new
name|BytesRef
argument_list|()
decl_stmt|;
DECL|field|scratchBytesRef2
specifier|final
name|BytesRef
name|scratchBytesRef2
init|=
operator|new
name|BytesRef
argument_list|()
...
name|maxDoc
decl_stmt|;
DECL|method|SimpleTextBKDWriter
specifier|public
name|SimpleTextBKDWriter
parameter_list|(
name|int
...
block|}
block|}
comment|/* In the 2+D case, we recursively pick the split dimension, compute the    * median value and partition other values around it. */
DECL|method|writeFieldNDims
specifier|private
name|long
name|writeFieldNDims
parameter_list|(
...
name|IllegalStateException
argument_list|(
literal|"cannot mix add and writeField"
argument_list|)
throw|;
block|}
...
index|]
decl_stmt|;
comment|// compute the min/max for this slice
name|Arrays
operator|.
name|fill
argument_list|(
...
return|;
block|}
comment|/* In the 1D case, we can simply sort points in ascending order and use the    * same writing logic as we use at merge time. */
DECL|method|writeField1Dim
specifier|private
name|long
name|writeField1Dim
parameter_list|(
...
return|;
block|}
comment|// TODO: remove this opto: SimpleText is supposed to be simple!
comment|/** More efficient bulk-add for incoming {@link SimpleTextBKDReader}s.  This does a merge sort of the already    *  sorted values and currently only works when numDims==1.  This returns -1 if all documents containing    *  dimensional values were deleted. */
DECL|method|merge
specifier|public
name|long
name|merge
...
return|;
block|}
DECL|class|OneDimensionBKDWriter
specifier|private
class|class
name|OneDimensionBKDWriter
block|{
...
argument_list|()
decl_stmt|;
DECL|field|leafBlockStartValues
specifier|final
name|List
argument_list|<
name|byte
...
name|leafCount
decl_stmt|;
DECL|method|OneDimensionBKDWriter
name|OneDimensionBKDWriter
parameter_list|(
name|IndexOutput
name|out
...
literal|0
assert|;
comment|// only assign when asserts are enabled
block|}
DECL|method|finish
specifier|public
name|long
...
argument_list|)
assert|;
name|writeLeafBlockPackedValues
argument_list|(
name|out
argument_list|,
name|commonPrefixLengths
...
block|}
block|}
DECL|method|writeLeafBlockPackedValues
specifier|protected
name|void
name|writeLeafBlockPackedValues
parameter_list|(
...
block|}
block|}
DECL|method|writeLeafBlockPackedValuesRange
specifier|private
name|void
name|writeLeafBlockPackedValuesRange
parameter_list|(
name|IndexOutput
name|out
parameter_list|,
...
name|maxPointsInLeafNode
assert|;
comment|// Compute common prefixes
name|Arrays
operator|.
name|fill
argument_list|(
...
block|}
block|}
comment|// Find the dimension that has the least number of unique bytes at commonPrefixLengths[dim]
name|FixedBitSet
index|[]
name|usedBytes
init|=
...
block|{
comment|// inner node
comment|// compute the split dimension and partition around it
specifier|final
name|int
name|splitDim
init|=
...
comment|// We can write the block in any order so by default we write it sorted by the dimension that has the
comment|// least number of unique bytes at commonPrefixLengths[dim], which makes compression more efficient
name|int
name|sortedDim
init|=
literal|0
...
begin_function
DECL|method|valuesInOrderAndBounds
specifier|private
name|boolean
name|valuesInOrderAndBounds
parameter_list|(
...
name|AssertionError
argument_list|(
literal|"docs out of order: last doc="
operator|+
name|lastDoc
operator|+
literal|" current doc="
