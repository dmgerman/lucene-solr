...
block|{
return|return
name|makeGridShapeIntersectsQuery
argument_list|(
name|args
operator|.
name|getShape
...
throw|;
block|}
comment|/**    * A quick check of the shape to see if it is perfectly aligned to a grid.    * Points always are as they are indivisible.  It's okay to return false    * if the shape actually is aligned; this is an optimization hint.    */
...
return|;
block|}
comment|/** {@link #makeQuery(SpatialArgs)} specialized for the query being a grid square. */
DECL|method|makeGridShapeIntersectsQuery
specifier|protected
name|Query
name|makeGridShapeIntersectsQuery
parameter_list|(
...
condition|)
block|{
comment|// Awesome; this will be equivalent to a TermQuery.
name|Iterator
argument_list|<
name|Cell
argument_list|>
...
block|{
comment|// Well there could be parent cells. But we can reduce the "scan level" which will be slower for a point query.
comment|// TODO: AVPTQ will still scan the bottom nonetheless; file an issue to eliminate that
return|return
operator|new
name|IntersectsPrefixTreeQuery
argument_list|(
