...
condition|)
block|{
comment|// This path didn't yet see the complete surface form; let's see if it just did with the arc output we just added:
name|BytesRef
name|arcOutput
init|=
name|path
...
condition|)
block|{
comment|// OK this arc that the path was just extended by contains the payloadSep, so we now have a full surface form in this path
name|path
operator|.
name|payload
operator|=
...
condition|)
block|{
comment|// now record that we've seen this surface form:
name|char
index|[]
name|key
init|=
...
condition|)
block|{
comment|// we already collected a higher scoring document with this key, in this segment:
return|return
literal|false
return|;
block|}
...
condition|)
block|{
comment|// Important to cache this, else we have a possibly O(N^2) cost where N is the length of suggestions
name|payload
operator|=
name|j
expr_stmt|;
