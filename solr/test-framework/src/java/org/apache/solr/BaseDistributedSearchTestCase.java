...
literal|8
decl_stmt|;
comment|/**    * When this flag is set, Double values will be allowed a difference ratio of 1E-8    * between the non-distributed and the distributed returned values    */
DECL|field|FUZZY
specifier|public
specifier|static
name|int
...
literal|16
decl_stmt|;
DECL|field|DOUBLE_RATIO_LIMIT
specifier|private
specifier|static
specifier|final
name|double
name|DOUBLE_RATIO_LIMIT
init|=
literal|1E
operator|-
literal|8
...
return|;
block|}
comment|/**    * Returns the QueryResponse from {@link #queryServer}    * @param setDistribParams whether to do a distributed request    * @param user basic auth username (set to null if not in use)    * @param pass basic auth password (set to null if not in use)    * @return the query response    */
DECL|method|query
specifier|protected
name|QueryResponse
name|query
...
return|;
block|}
DECL|method|generateQueryRequest
specifier|private
name|QueryRequest
name|generateQueryRequest
parameter_list|(
...
condition|)
block|{
comment|// so we skip val but otherwise both should be present
assert|assert
operator|(
name|f
operator|&
...
argument_list|)
decl_stmt|;
comment|// When stats computations are done on multiple shards, there may
comment|// be small differences in the results. Allow a small difference
comment|// between the result of the computations.
name|double
name|ratio
init|=
name|Math
...
name|error
argument_list|(
literal|"Error while shutting down servers"
argument_list|,
name|t
argument_list|)
expr_stmt|;
