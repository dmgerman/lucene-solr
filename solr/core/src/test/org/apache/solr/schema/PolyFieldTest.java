...
expr_stmt|;
name|int
name|expectdNumFields
init|=
literal|3
decl_stmt|;
comment|//If DV=false, we expect one field per dimension plus a stored field
if|if
condition|(
name|pt
operator|.
...
literal|2
expr_stmt|;
comment|// If docValues=true, then we expect two more fields
block|}
name|assertEquals
argument_list|(
literal|"Unexpected fields created: "
operator|+
name|Arrays
operator|.
name|toString
...
argument_list|)
expr_stmt|;
comment|//first two/four fields contain the values, last one is just stored and contains the original
for|for
control|(
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|//one less field than with "home", since we aren't storing
name|home
operator|=
name|schema
operator|.
