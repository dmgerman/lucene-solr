...
name|remove
argument_list|(
literal|"tests.solr.useColdSearcher"
argument_list|)
expr_stmt|;
name|super
operator|.
...
name|get
argument_list|(
literal|"SEARCHER.searcher.registeredAt"
argument_list|)
decl_stmt|;
name|Date
...
operator|.
name|registerSlowSearcherListener
operator|=
literal|false
expr_stmt|;
name|MockSearchComponent
...
expr_stmt|;
block|}
DECL|method|testDontUseColdSearcher
specifier|public
name|void
name|testDontUseColdSearcher
parameter_list|()
throws|throws
name|Exception
block|{
...
condition|)
block|{
comment|// even if we wait here, the SearcherListener should not finish
name|Thread
operator|.
name|sleep
argument_list|(
...
expr_stmt|;
block|}
comment|// validate that the searcher warmer didn't finish yet.
name|assertEquals
argument_list|(
literal|0
argument_list|,
...
name|assertFalse
argument_list|(
literal|"Query should be waiting for warming to finish"
argument_list|,
name|querySucceeded
operator|.
name|get
...
argument_list|()
expr_stmt|;
comment|// Validate that the query eventually succeeds
for|for
control|(
name|int
name|i
...
name|fail
argument_list|(
literal|"Query didn't succeed after 10 secoonds"
argument_list|)
expr_stmt|;
block|}
name|Thread
...
block|}
block|}
DECL|method|testUseColdSearcher
specifier|public
name|void
name|testUseColdSearcher
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
expr_stmt|;
comment|// validate that the query runs before the searcher warmer finishes
for|for
control|(
name|int
name|i
...
literal|false
decl_stmt|;
DECL|field|registerSlowSearcherListener
specifier|static
name|boolean
name|registerSlowSearcherListener
init|=
...
block|}
block|}
DECL|class|SlowSearcherListener
specifier|static
class|class
name|SlowSearcherListener
implements|implements
...
literal|null
operator|:
literal|"SlowSearcherListener should only be used as FirstSearcherListener"
assert|;
comment|// simulate a slow searcher listener
name|latch
operator|.
name|await
argument_list|(
