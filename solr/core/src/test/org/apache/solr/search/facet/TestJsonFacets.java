...
argument_list|)
expr_stmt|;
comment|// "${terms}" should be put at the beginning of generic terms facets.
comment|// It may specify "method=..." or "limit:-1", so should not be used if the facet explicitly specifies.
name|MacroExpander
name|m
init|=
operator|new
...
literal|"json.facet"
argument_list|,
literal|"{f1:{terms:{${terms_method} field:${cat_s}, numBuckets:true, limit:1}}}"
...
literal|"{"
operator|+
literal|" f0:{${terms_method} type:terms, field:${multi_ss}, allBuckets:true, limit:0} "
operator|+
literal|",f1:{${terms_method} type:terms, field:${multi_ss}, allBuckets:true, limit:0, offset:1} "
operator|+
comment|// offset with 0 limit
literal|",f2:{${terms_method} type:terms, field:${multi_ss}, allBuckets:true, limit:0, facet:{x:'sum(${num_d})'}, sort:'x desc' } "
operator|+
literal|",f3:{${terms_method} type:terms, field:${multi_ss}, allBuckets:true, limit:0, missing:true, facet:{x:'sum(${num_d})', y:'avg(${num_d})'}, sort:'x desc' } "
operator|+
literal|"}"
argument_list|)
argument_list|,
...
literal|"{"
operator|+
literal|" f0:{${terms_method} type:terms, field:${num_i}, allBuckets:true, limit:0} "
operator|+
literal|",f1:{${terms_method} type:terms, field:${num_i}, allBuckets:true, limit:0, offset:1} "
operator|+
comment|// offset with 0 limit
literal|",f2:{${terms_method} type:terms, field:${num_i}, allBuckets:true, limit:0, facet:{x:'sum(${num_d})'}, sort:'x desc' } "
operator|+
literal|"}"
argument_list|)
argument_list|,
...
block|}
comment|////////////////////////////////////////////////////////////////
comment|// test which phase stats are calculated in
comment|////////////////////////////////////////////////////////////////
if|if
condition|(
name|client
...
name|resets
decl_stmt|;
comment|// NOTE: these test the current implementation and may need to be adjusted to match future optimizations (such as calculating N buckets in parallel in the second phase)
name|creates
operator|=
name|DebugAgg
operator|.
...
literal|"json.facet"
argument_list|,
literal|"{f1:{terms:{${terms_method} field:${super_s}, limit:1, facet:{x:'debug()'}   }}}"
comment|// x should be deferred to 2nd phase
argument_list|)
argument_list|,
literal|"facets=={ 'count':6, "
operator|+
literal|"f1:{  buckets:[{ val:batman, count:1, x:1}]} } "
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// probably "1", but may be special slot for something.  As long as it's not cardinality of the field
name|creates
operator|=
name|DebugAgg
operator|.
...
literal|"json.facet"
argument_list|,
literal|"{f1:{terms:{${terms_method} field:${super_s}, limit:1, facet:{ x:'debug()'} , sort:'x asc'  }}}"
comment|// sorting by x... must be done all at once in first phase
argument_list|)
argument_list|,
literal|"facets=={ 'count':6, "
operator|+
...
argument_list|)
expr_stmt|;
comment|// all slots should be done in a single shot. there may be more than 5 due to special slots or hashing.
comment|// When limit:-1, we should do most stats in first phase (SOLR-10634)
name|creates
operator|=
name|DebugAgg
operator|.
...
expr_stmt|;
comment|// all slots should be done in a single shot. there may be more than 5 due to special slots or hashing.
comment|// Now for a numeric field
comment|// When limit:-1, we should do most stats in first phase (SOLR-10634)
name|creates
operator|=
name|DebugAgg
...
literal|"json.facet"
argument_list|,
literal|"{f1:{terms:{${terms_method} field:${num_d}, limit:-1, facet:{x:'debug()'}  }}}"
argument_list|)
argument_list|,
literal|"facets=="
argument_list|)
...
expr_stmt|;
comment|// all slots should be done in a single shot. there may be more than 5 due to special slots or hashing.
comment|// But if we need to calculate domains anyway, it probably makes sense to calculate most stats in the 2nd phase (along with sub-facets)
name|creates
operator|=
name|DebugAgg
operator|.
...
literal|"json.facet"
argument_list|,
literal|"{f1:{terms:{${terms_method} field:${super_s}, limit:-1, facet:{ x:'debug()' , y:{terms:${where_s}}   }  }}}"
argument_list|)
argument_list|,
literal|"facets=="
argument_list|)
...
argument_list|)
expr_stmt|;
comment|// probably 1, but could be higher
comment|// Now with a numeric field
comment|// But if we need to calculate domains anyway, it probably makes sense to calculate most stats in the 2nd phase (along with sub-facets)
name|creates
operator|=
name|DebugAgg
...
literal|"json.facet"
argument_list|,
literal|"{f1:{terms:{${terms_method} field:${num_d}, limit:-1, facet:{ x:'debug()' , y:{terms:${where_s}}   }  }}}"
argument_list|)
argument_list|,
literal|"facets=="
argument_list|)
