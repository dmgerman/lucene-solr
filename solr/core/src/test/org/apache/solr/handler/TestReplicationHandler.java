...
name|setProperty
argument_list|(
literal|"solr.indexfetcher.sotimeout2"
argument_list|,
literal|"45000"
argument_list|)
expr_stmt|;
...
annotation|@
name|Test
DECL|method|doTestHandlerPathUnchanged
specifier|public
name|void
name|doTestHandlerPathUnchanged
parameter_list|()
throws|throws
name|Exception
block|{
...
name|assertNotNull
argument_list|(
literal|"Expected to see that the slave has replicated"
operator|+
name|i
operator|+
literal|": "
...
argument_list|)
expr_stmt|;
comment|// we can have more replications than we added docs because a replication can legally fail and try
comment|// again (sometimes we cannot merge into a live index and have to try again)
name|assertTrue
argument_list|(
literal|"i:"
operator|+
...
specifier|public
name|void
name|doTestIndexFetchOnMasterRestart
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
expr_stmt|;
comment|// poll interval on slave is 1 second, so we just sleep for a few seconds
name|Thread
operator|.
name|sleep
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//get docs from slave and assert that they are still the same as before
name|slaveQueryRsp
operator|=
name|rQuery
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// details/slave/timesIndexReplicated
name|NamedList
argument_list|<
name|Object
argument_list|>
...
name|PATH
operator|+
literal|"?command=fetchindex&masterUrl="
decl_stmt|;
name|masterUrl
operator|+=
name|buildUrl
...
block|}
block|}
DECL|method|getCachingDirectoryFactory
specifier|private
name|CachingDirectoryFactory
name|getCachingDirectoryFactory
parameter_list|(
name|SolrCore
name|core
parameter_list|)
...
argument_list|()
decl_stmt|;
comment|// one for data, one for the index under data and one for the snapshot metadata.
comment|// we also allow one extra index dir - it may not be removed until the core is closed
if|if
condition|(
name|afterReload
condition|)
...
argument_list|)
decl_stmt|;
comment|// if after reload, there may be 2 index dirs while the reloaded SolrCore closes.
if|if
condition|(
name|afterReload
condition|)
...
name|PATH
operator|+
literal|"?wait=true&command=fetchindex&masterUrl="
operator|+
name|buildUrl
argument_list|(
name|from
...
annotation|@
name|Test
DECL|method|testRateLimitedReplication
specifier|public
name|void
name|testRateLimitedReplication
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
expr_stmt|;
comment|//Start master with the new solrconfig
name|master
operator|.
name|copyConfigFile
argument_list|(
name|CONF_DIR
operator|+
literal|"solrconfig-master-throttled.xml"
argument_list|,
literal|"solrconfig.xml"
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// maxWriteMBPerSec=0.1 in solrconfig
comment|//Start again and replicate the data
name|useFactory
argument_list|(
literal|null
argument_list|)
...
argument_list|()
expr_stmt|;
comment|//Add a few more docs in the master. Just to make sure that we are replicating the correct index point
comment|//These extra docs should not get replicated
operator|new
name|Thread
argument_list|(
operator|new
...
argument_list|()
expr_stmt|;
comment|//Wait and make sure that it actually replicated correctly.
name|NamedList
name|slaveQueryRsp
init|=
name|rQuery
...
annotation|@
name|Test
DECL|method|doTestIllegalFilePaths
specifier|public
name|void
name|doTestIllegalFilePaths
parameter_list|()
throws|throws
name|Exception
block|{
...
name|fail
argument_list|(
literal|"Should have thrown exception on illegal path for param "
operator|+
name|param
operator|+
literal|" and file name "
operator|+
name|filename
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|//Do nothing. Wasn't able to add doc.
block|}
block|}
try|try
block|{
...
parameter_list|)
block|{
comment|//Do nothing. No extra doc got committed.
block|}
block|}
block|}
