...
argument_list|()
expr_stmt|;
comment|//    testBasicGrouping(); // TODO fails due to NOT on java string instead of boolean
comment|//    testBasicGroupingFacets(); // TODO cleanup names and push down facets and fails due to NOT on java string instead of boolean
...
assert|;
comment|//max(field_i)
comment|//assert(tuple.getDouble("avg(field_i)") == 13.5D); //avg(field_i)
name|params
operator|=
operator|new
name|HashMap
...
literal|"stmt"
argument_list|,
literal|"select str_s as myString, count(*) as myCount, sum(field_i) as mySum, min(field_i) as myMin, max(field_i) as myMax, avg(field_i) as myAvg from collection1 where (text='XXXX' AND NOT text='XXXX XXX') group by str_s order by str_s desc"
argument_list|)
expr_stmt|;
name|solrStream
operator|=
...
operator|)
assert|;
comment|//assert(tuple.getDouble("myAvg") == 45);
name|tuple
operator|=
name|tuples
operator|.
...
operator|)
assert|;
comment|//assert(tuple.getDouble("myAvg") == 9.5D);
name|tuple
operator|=
name|tuples
operator|.
...
operator|)
assert|;
comment|//assert(tuple.getDouble("myAvg") == 13.5D);
name|params
operator|=
operator|new
name|HashMap
...
literal|"stmt"
argument_list|,
literal|"select str_s as myString, count(*), sum(field_i) as mySum, min(field_i), max(field_i), avg(field_i) from collection1 where text='XXXX' group by myString having ((sum = 19) AND (min(field_i) = 8))"
argument_list|)
expr_stmt|;
name|solrStream
operator|=
...
literal|"stmt"
argument_list|,
literal|"select str_s as myString, count(*), sum(field_i) as mySum, min(field_i), max(field_i), avg(field_i) from collection1 where (text='XXXX' AND NOT text='XXXX XXX') group by myString order by myString desc"
argument_list|)
expr_stmt|;
name|solrStream
operator|=
