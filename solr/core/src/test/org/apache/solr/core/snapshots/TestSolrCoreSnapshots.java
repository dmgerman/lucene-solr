...
comment|// The IndexWriter (used to cleanup index files) creates an additional commit during closing. Hence we expect 2 commits (instead
comment|// of 1).
block|{
name|List
argument_list|<
...
argument_list|)
expr_stmt|;
comment|// Verify that corresponding index files have been deleted. Ideally this directory should
comment|// be removed immediately. But the current DirectoryFactory impl waits until the
comment|// closing the core (or the directoryFactory) for actual removal. Since the IndexWriter
comment|// (used to cleanup index files) creates an additional commit during closing, we expect a single
comment|// commit (instead of 0).
name|assertEquals
argument_list|(
literal|1
argument_list|,
