...
name|Exception
block|{
comment|// Need to use a disk-based directory because there are tests that close a core after adding documents
comment|// then expect to be able to re-open that core and execute a search
name|useFactory
argument_list|(
...
name|println
argument_list|(
literal|"TestLazyCores.testCreateTransientFromAdmin Thread.run sleeping for "
operator|+
name|sleep_millis
operator|+
literal|" ms"
...
name|println
argument_list|(
literal|"TestLazyCores.testCreateTransientFromAdmin Thread.run caught "
operator|+
name|ie
operator|+
literal|" whilst sleeping for "
operator|+
name|sleep_millis
operator|+
literal|" ms"
...
annotation|@
name|Test
DECL|method|testMidUseUnload
specifier|public
name|void
name|testMidUseUnload
parameter_list|()
throws|throws
name|Exception
block|{
...
name|println
argument_list|(
literal|"TestLazyCores.testMidUseUnload maximumSleepMillis="
operator|+
name|maximumSleepMillis
argument_list|)
expr_stmt|;
...
name|println
argument_list|(
literal|"TestLazyCores.testMidUseUnload Thread.run sleeping for "
operator|+
name|sleep_millis
operator|+
literal|" ms"
...
name|println
argument_list|(
literal|"TestLazyCores.testMidUseUnload Thread.run caught "
operator|+
name|ie
operator|+
literal|" whilst sleeping for "
...
argument_list|)
expr_stmt|;
comment|// not closed since we are still using it and hold a reference
name|core_to_use
operator|.
name|close
argument_list|()
expr_stmt|;
comment|// now give up our reference to the core
block|}
block|}
empty_stmt|;
name|CoreContainer
...
argument_list|)
expr_stmt|;
comment|// freshly-in-use core is not closed
name|thread
operator|.
name|start
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// after unload-ing the core is closed
name|thread
operator|.
name|join
argument_list|()
...
try|try
block|{
comment|// First, go through all the transient cores and add some docs. DO NOT COMMIT!
comment|// The aged-out core should commit the docs when it gets closed.
name|List
argument_list|<
name|SolrCore
argument_list|>
...
expr_stmt|;
block|}
comment|// Just proving that some cores have been aged out.
name|checkNotInCores
argument_list|(
name|cc
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Close our get of all cores above.
for|for
control|(
name|SolrCore
name|core
...
argument_list|()
expr_stmt|;
comment|// We still should have 6, 7, 8, 9 loaded, their reference counts have NOT dropped to zero
name|checkInCores
argument_list|(
name|cc
argument_list|,
...
control|)
block|{
comment|// The point of this test is to insure that when cores are aged out and re-opened
comment|// that the docs are there, so insure that the core we're testing is gone, gone, gone.
name|checkNotInCores
argument_list|(
name|cc
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Load the core up again.
name|SolrCore
name|core
init|=
name|cc
...
argument_list|)
expr_stmt|;
comment|// Insure docs are still there.
name|check10
argument_list|(
name|core
argument_list|)
...
name|assertQ
argument_list|(
literal|"test closing core without committing"
argument_list|,
name|makeReq
argument_list|(
name|core
