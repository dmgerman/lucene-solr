...
name|ArrayList
argument_list|<>
comment|// TODO: SOLR-9314: once all the known bugs are fixed, and this list can be constant
comment|// regardless of single/multi node, change this to Collections.unmodifiableList
comment|// (and adjust jdocs accordingly)
argument_list|(
name|Arrays
operator|.
expr|<
...
name|asList
argument_list|(
comment|// TODO: SOLR-9314: add more of these for other various transformers
comment|//
comment|// TODO: add a [docid] validator (blocked by SOLR-9288& SOLR-9289)
comment|//
operator|new
name|GlobValidator
argument_list|(
...
name|Exception
block|{
comment|// Due to known bugs with some transformers in either multi vs single node, we want
comment|// to test both possible cases explicitly and modify the List of FL_VALIDATORS we use accordingly:
...
block|{
comment|// No-Op
comment|// No known transformers that only work in distrib cloud but fail in singleCoreMode
block|}
comment|// TODO: SOLR-9314: programatically compare FL_VALIDATORS with all known transformers.
comment|// (ala QueryEqualityTest) can't be done until we eliminate the need for "singleCodeMode"
comment|// conditional logic (might still want 'singleCoreMode' on the MiniSolrCloudCluster side,
comment|// but shouldn't have conditional FlValidators
...
name|assertEquals
argument_list|(
literal|"More actual fields then expected"
argument_list|,
name|expectedFieldNames
argument_list|,
name|actualFieldNames
...
name|FieldValueValidator
block|{
comment|/** @see GlobValidator */
DECL|method|getRealFieldName
specifier|public
name|String
