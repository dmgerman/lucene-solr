...
literal|0
decl_stmt|;
comment|// give the update a chance to take effect.
for|for
control|(
name|int
name|idx
...
name|startTimes
init|=
name|getSoftAutocommitInterval
argument_list|(
name|testCollectionName
argument_list|)
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Since it takes some time for this command to complete we need to make sure all the reloads for
comment|// all the cores have been done.
name|boolean
name|allGood
init|=
literal|false
...
condition|)
block|{
comment|// no point in even trying if they're not the same size!
name|allGood
operator|=
literal|true
expr_stmt|;
...
name|assertTrue
argument_list|(
literal|"All cores should have been reloaded within 60 seconds!!!"
argument_list|,
name|allGood
argument_list|)
expr_stmt|;
block|}
comment|// Collect all of the autoSoftCommit intervals.
DECL|method|getSoftAutocommitInterval
specifier|private
name|Map
argument_list|<
name|String
