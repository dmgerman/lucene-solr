...
annotation|@
name|Test
DECL|method|testDistributedQueueCache
specifier|public
name|void
name|testDistributedQueueCache
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
expr_stmt|;
comment|// Wait for watcher being kicked off
while|while
condition|(
operator|!
name|consumer
...
expr_stmt|;
block|}
comment|// DQ still have elements in their queue, so we should not fetch elements path from Zk
name|assertEquals
argument_list|(
literal|1
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// in case of race condition, childWatcher is kicked off after peek()
if|if
condition|(
name|dq
operator|.
