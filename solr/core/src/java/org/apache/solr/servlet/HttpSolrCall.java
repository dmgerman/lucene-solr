...
block|}
block|}
comment|// when we send back an error, we make sure we read
comment|// the full client request so that the client does
comment|// not hit a connection reset and we can reuse the
comment|// connection - see SOLR-8453
DECL|method|consumeInput
specifier|private
name|void
name|consumeInput
parameter_list|(
...
literal|null
decl_stmt|;
comment|//Hitting the leaders is useful when it's an update request.
comment|//For queries it doesn't matter and hence we don't distinguish here.
for|for
control|(
name|Map
operator|.
