...
expr_stmt|;
name|boolean
name|defaultPayloads
init|=
literal|true
decl_stmt|;
comment|//overwritten below
try|try
block|{
comment|// It'd be nice to know if payloads are on the tokenStream but the presence of the attribute isn't a good
comment|// indicator.
specifier|final
name|Terms
name|terms
...
name|error
argument_list|(
literal|"Couldn't check for existence of payloads"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// Lazy container for fvh and fieldQuery
name|IndexReader
name|reader
init|=
operator|new
...
decl_stmt|;
comment|// object type allows flexibility for subclassers
name|fieldHighlights
operator|=
name|doHighlightingOfField
argument_list|(
name|doc
argument_list|,
name|docId
...
return|;
block|}
DECL|method|doHighlightingOfField
specifier|protected
name|Object
name|doHighlightingOfField
parameter_list|(
...
block|{
comment|// standard/default highlighter
name|fieldHighlights
operator|=
...
return|;
block|}
comment|/**    * Determines if we should use the FastVectorHighlighter for this field.    */
...
return|;
block|}
comment|/** Highlights and returns the highlight object for this field -- a String[] by default.  Null if none. */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
...
comment|//  note: offsets are minimally sufficient for this HL.
specifier|final
name|Fields
name|tvFields
init|=
...
literal|null
return|;
comment|//no highlights for this field
block|}
...
return|;
block|}
comment|/** Returns the alternate highlight object for this field -- a String[] by default.  Null if none. */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
...
argument_list|)
expr_stmt|;
comment|// Enforce maxAlternateFieldLength by FRAGSIZE. Minimum 18 due to FVH limitations
name|invariants
operator|.
name|put
argument_list|(
...
block|}
block|}
comment|// Fallback to static non-highlighted
name|IndexableField
index|[]
name|docFields
init|=
...
begin_comment
comment|/** Wraps a DirectoryReader that caches the {@link LeafReader#getTermVectors(int)} so that  * if the next call has the same ID, then it is reused.  */
end_comment
