...
parameter_list|)
block|{
comment|// ignore - someone else already deleted this one
block|}
catch|catch
parameter_list|(
name|Exception
...
argument_list|)
expr_stmt|;
comment|// now register it also under new name
name|String
name|beanName
init|=
name|createBeanName
...
name|warn
argument_list|(
literal|"Failed to register info bean: key="
operator|+
name|key
operator|+
literal|", infoBean="
...
argument_list|(
literal|"SolrInfoMBean.category must never be null: "
operator|+
name|infoBean
argument_list|)
throw|;
...
block|}
block|}
comment|/**    * SolrInfoMBean that provides JMX-specific statistics.  Used, for example,    * if generating full statistics is expensive; the expensive statistics can    * be generated normally for use with the web ui, while an abbreviated version    * are generated for period jmx use.    */
DECL|interface|JmxAugmentedSolrInfoMBean
specifier|public
interface|interface
name|JmxAugmentedSolrInfoMBean
extends|extends
name|SolrInfoMBean
block|{
comment|/**      * JMX-specific statistics      */
