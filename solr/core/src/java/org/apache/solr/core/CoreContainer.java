...
name|authenticationPlugin
decl_stmt|;
DECL|field|backupRepoFactory
specifier|private
name|BackupRepositoryFactory
name|backupRepoFactory
decl_stmt|;
...
name|metricManager
decl_stmt|;
DECL|field|metricsHandler
specifier|protected
name|MetricsHandler
name|metricsHandler
decl_stmt|;
DECL|field|metricsCollectorHandler
specifier|protected
name|MetricsCollectorHandler
name|metricsCollectorHandler
decl_stmt|;
...
specifier|private
enum|enum
name|CoreInitFailedAction
block|{
name|fromleader
block|,
name|none
block|}
comment|/**    * This method instantiates a new instance of {@linkplain BackupRepository}.    *    * @param repositoryName The name of the backup repository (Optional).    *                       If not specified, a default implementation is used.    * @return a new instance of {@linkplain BackupRepository}.    */
DECL|method|newBackupRepository
specifier|public
name|BackupRepository
...
name|debug
argument_list|(
literal|"Authentication plugin class obtained from security.json: "
operator|+
name|pluginClassName
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// may want to add some configuration here in the future
name|metricsCollectorHandler
operator|.
name|init
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// initialize gauges for reporting the number of cores and disk total/free
name|String
name|registryName
init|=
name|SolrMetricManager
...
argument_list|)
expr_stmt|;
comment|// add version information
name|metricManager
operator|.
name|registerGauge
argument_list|(
...
condition|)
block|{
comment|//sort the cores if it is in SolrCloud. In standalone node the order does not matter
name|CoreSorter
name|coreComparator
init|=
operator|new
...
name|info
argument_list|(
literal|"Security node changed, reloading security.json"
argument_list|)
expr_stmt|;
name|reloadSecurityProperties
argument_list|()
...
begin_comment
comment|/**    * Make sure securityConfHandler is initialized    */
end_comment

begin_function
DECL|method|reloadSecurityProperties
specifier|private
name|void
name|reloadSecurityProperties
parameter_list|()
...
argument_list|()
expr_stmt|;
comment|// there is a race we have to protect against
block|}
block|}
else|else
block|{
...
name|core
operator|=
name|processCoreCreateException
argument_list|(
name|e
argument_list|,
name|dcore
...
begin_comment
comment|/**    * Take action when we failed to create a SolrCore. If error is due to corrupt index, try to recover. Various recovery    * strategies can be specified via system properties "-DCoreInitFailedAction={fromleader, none}"    *    * @see CoreInitFailedAction    *    * @param original    *          the problem seen when loading the core the first time.    * @param dcore    *          core descriptor for the core to create    * @param coreConfig    *          core config for the core to create    * @return if possible    * @throws SolrException    *           rethrows the original exception if we will not attempt to recover, throws a new SolrException with the    *           original exception as a suppressed exception if there is a second problem creating the solr core.    */
end_comment

begin_function
DECL|method|processCoreCreateException
specifier|private
name|SolrCore
name|processCoreCreateException
parameter_list|(
...
parameter_list|)
block|{
comment|// Traverse full chain since CIE may not be root exception
name|Throwable
name|cause
init|=
name|original
...
block|}
block|}
comment|// If no CorruptIndexExeption, nothing we can try here
if|if
condition|(
name|cause
operator|==
...
name|debug
argument_list|(
literal|"CorruptIndexException while creating core, will attempt to repair via {}"
argument_list|,
name|action
argument_list|)
expr_stmt|;
...
name|fromleader
case|:
comment|// Recovery from leader on a CorruptedIndexException
if|if
condition|(
name|isZooKeeperAware
argument_list|()
...
name|info
argument_list|(
literal|"Found active leader, will attempt to create fresh core and recover."
argument_list|)
expr_stmt|;
name|resetIndexDirectory
argument_list|(
name|dcore
argument_list|,
name|coreConfig
...
name|warn
argument_list|(
literal|"Failed to create core, and did not recognize specified 'CoreInitFailedAction': [{}]. Valid options are {}."
argument_list|,
name|action
argument_list|,
name|Arrays
...
begin_comment
comment|/**    * Write a new index directory for the a SolrCore, but do so without loading it.    */
end_comment

begin_function
DECL|method|resetIndexDirectory
specifier|private
name|void
name|resetIndexDirectory
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Free the directory object that we had to create for this
name|Directory
name|dir
init|=
literal|null
...
return|return;
block|}
comment|// delete metrics specific to this core
name|metricManager
operator|.
name|removeRegistry
argument_list|(
...
condition|)
block|{
comment|// Stop replication if this is part of a pull/tlog replica before closing the core
name|zkSys
operator|.
name|getZkController
argument_list|()
...
condition|)
block|{
comment|// we aren't going to reload this if we delete the instance dir.
name|solrCores
operator|.
...
argument_list|)
expr_stmt|;
comment|// The old coreDescriptor is obsolete, so remove it. registerCore will put it back.
name|CoreDescriptor
name|cd
init|=
name|core
...
argument_list|)
decl_stmt|;
comment|// If a core is loaded, we're done just return it.
if|if
condition|(
name|core
operator|!=
...
return|;
block|}
comment|// If it's not yet loaded, we can check if it's had a core init failure and "do the right thing"
name|CoreDescriptor
name|desc
init|=
name|solrCores
...
throw|;
block|}
comment|// This is a bit of awkwardness where SolrCloud and transient cores don't play nice together. For transient cores,
comment|// we have to allow them to be created at any time there hasn't been a core load failure (use reload to cure that).
comment|// But for TestConfigSetsAPI.testUploadWithScriptUpdateProcessor, this needs to _not_ try to load the core if
comment|// the core is null and there was an error. If you change this, be sure to run both TestConfiSetsAPI and
comment|// TestLazyCores
if|if
condition|(
name|desc
operator|==
