...
return|;
block|}
DECL|method|createQueryResponseWriter
specifier|private
name|QueryResponseWriter
name|createQueryResponseWriter
parameter_list|(
name|String
name|className
parameter_list|)
...
block|}
block|}
comment|// Open the searcher *before* the update handler so we don't end up
comment|// opening
comment|// one in the middle.
comment|// With lockless commits in Lucene now, this probably shouldn't be an
comment|// issue anymore
try|try
block|{
name|getSearcher
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// This must be being created since there's no file here already. So write out all of the params we were
comment|// created with. This _may_ overwrite the name above, but that's OK.
name|Collection
argument_list|<
name|String
argument_list|>
...
control|)
block|{
comment|// Only preserve things that are legal, and let's just keep instDir right out of the persisted file even
comment|// though it's part of the create properties on the URL.
if|if
condition|(
operator|!
name|CoreDescriptor
...
name|info
argument_list|(
literal|"adding lazy queryResponseWriter: "
operator|+
name|info
operator|.
name|className
...
operator|=
operator|new
name|LazyQueryResponseWriterWrapper
argument_list|(
name|this
argument_list|,
name|info
...
name|warn
argument_list|(
literal|"Multiple queryResponseWriter registered to the same name: "
operator|+
name|info
operator|.
name|name
...
name|SERVER_ERROR
argument_list|,
literal|"QueryResponseWriter init failure"
argument_list|,
name|ex
argument_list|)
decl_stmt|;
...
DECL|method|initTransformerFactories
specifier|private
name|void
name|initTransformerFactories
parameter_list|()
block|{
comment|// Load any transformer factories
name|initPlugins
argument_list|(
name|transformerFactories
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Tell each transformer what its name is
for|for
control|(
name|Map
operator|.
...
argument_list|)
expr_stmt|;
comment|// f.init(null); default ones don't need init
block|}
block|}
catch|catch
parameter_list|(
...
return|;
block|}
DECL|class|LazyQueryResponseWriterWrapper
specifier|public
specifier|final
class|class
name|LazyQueryResponseWriterWrapper
...
name|_writer
decl_stmt|;
DECL|method|LazyQueryResponseWriterWrapper
specifier|public
name|LazyQueryResponseWriterWrapper
parameter_list|(
name|SolrCore
...
expr_stmt|;
block|}
DECL|method|getWrappedWriter
specifier|public
specifier|synchronized
name|QueryResponseWriter
name|getWrappedWriter
parameter_list|()
block|{
if|if
condition|(
