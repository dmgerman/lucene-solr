...
name|schema
decl_stmt|;
DECL|field|configSetProperties
specifier|private
specifier|final
name|NamedList
...
name|directoryFactory
decl_stmt|;
DECL|field|recoveryStrategyBuilder
specifier|private
specifier|final
name|RecoveryStrategy
operator|.
...
name|ruleExpiryLock
decl_stmt|;
DECL|field|snapshotDelLock
specifier|private
specifier|final
name|ReentrantLock
name|snapshotDelLock
decl_stmt|;
comment|// A lock instance to guard against concurrent deletions.
DECL|field|newSearcherTimer
specifier|private
name|Timer
name|newSearcherTimer
decl_stmt|;
DECL|field|newSearcherWarmupTimer
specifier|private
name|Timer
name|newSearcherWarmupTimer
decl_stmt|;
DECL|field|newSearcherCounter
specifier|private
name|Counter
name|newSearcherCounter
decl_stmt|;
DECL|field|newSearcherMaxReachedCounter
specifier|private
name|Counter
name|newSearcherMaxReachedCounter
decl_stmt|;
DECL|field|newSearcherOtherErrorsCounter
specifier|private
name|Counter
name|newSearcherOtherErrorsCounter
decl_stmt|;
...
return|;
block|}
DECL|field|perSegmentFingerprintCache
specifier|private
specifier|final
name|Map
argument_list|<
...
name|IndexFingerprint
argument_list|>
name|perSegmentFingerprintCache
init|=
operator|new
name|MapMaker
argument_list|()
...
name|debug
argument_list|(
literal|"BooleanQuery.maxClauseCount={}, ignoring {}"
argument_list|,
name|boolean_query_max_clause_count
argument_list|,
name|solrConfig
...
return|;
block|}
comment|/**    * Returns the {@link SolrCoreMetricManager} for this core.    *    * @return the {@link SolrCoreMetricManager} for this core    */
DECL|method|getCoreMetricManager
specifier|public
name|SolrCoreMetricManager
...
return|;
block|}
comment|/**    * Returns a Map of name vs SolrInfoBean objects. The returned map is an instance of    * a ConcurrentHashMap and therefore no synchronization is needed for putting, removing    * or iterating over it.    *    * @return the Info Registry map which contains SolrInfoBean objects keyed by name    * @since solr 1.3    */
...
return|;
block|}
DECL|method|initSnapshotMetaDataManager
specifier|private
name|SolrSnapshotMetaDataManager
name|initSnapshotMetaDataManager
parameter_list|()
block|{
try|try
block|{
...
block|}
block|}
comment|/**    * This method deletes the snapshot with the specified name. If the directory    * storing the snapshot is not the same as the *current* core index directory,    * then delete the files corresponding to this snapshot. Otherwise we leave the    * index files related to snapshot as is (assuming the underlying Solr IndexDeletionPolicy    * will clean them up appropriately).    *    * @param commitName The name of the snapshot to be deleted.    * @throws IOException in case of I/O error.    */
...
name|IOException
block|{
comment|// Note this lock is required to prevent multiple snapshot deletions from
comment|// opening multiple IndexWriter instances simultaneously.
name|this
operator|.
name|snapshotDelLock
operator|.
...
name|info
argument_list|(
literal|"Following snapshots exist in the index directory {} : {}"
argument_list|,
name|indexDirPath
argument_list|,
name|snapshots
...
condition|)
block|{
comment|// No snapshots remain in this directory. Can be cleaned up!
name|log
operator|.
name|info
argument_list|(
literal|"Removing index directory {} since all named snapshots are deleted."
argument_list|,
name|indexDirPath
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * This method deletes the index files not associated with any named snapshot only    * if the specified indexDirPath is not the *current* index directory.    *    * @param indexDirPath The path of the directory    * @throws IOException In case of I/O error.    */
...
name|IOException
block|{
comment|// Skip if the specified indexDirPath is the *current* index directory.
if|if
condition|(
name|getIndexDir
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Delete the old index directory only if no snapshot exists in that directory.
if|if
condition|(
name|snapshots
operator|.
...
block|{
comment|// close the new core on any errors that have occurred.
if|if
condition|(
operator|!
name|success
...
return|;
block|}
DECL|method|initRecoveryStrategyBuilder
specifier|private
name|RecoveryStrategy
operator|.
name|Builder
name|initRecoveryStrategyBuilder
parameter_list|()
block|{
specifier|final
name|PluginInfo
...
name|info
argument_list|(
literal|"solr.RecoveryStrategy.Builder"
argument_list|)
expr_stmt|;
name|rsBuilder
operator|=
...
name|configSetProperties
expr_stmt|;
comment|// Initialize the metrics manager
name|this
operator|.
name|coreMetricManager
operator|=
name|initCoreMetricManager
argument_list|(
name|config
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"[{}] Opening new SolrCore at [{}], dataDir=[{}]"
argument_list|,
name|logid
argument_list|,
name|resourceLoader
...
argument_list|)
expr_stmt|;
name|checkVersionFieldExistsInSchema
argument_list|(
name|schema
argument_list|,
name|coreDescriptor
...
argument_list|()
decl_stmt|;
comment|// initialize searcher-related metrics
...
argument_list|()
decl_stmt|;
comment|// this is registered at the CONTAINER level because it's not core-specific - for now we
comment|// also register it here for back-compat
name|solrFieldCacheBean
operator|.
name|initializeMetrics
argument_list|(
...
parameter_list|)
block|{
comment|// release the latch, otherwise we block trying to do the close. This
comment|// should be fine, since counting down on a latch of 0 is still fine
name|latch
operator|.
name|countDown
argument_list|()
...
try|try
block|{
comment|// close down the searcher and any other resources, if it exists, as this
comment|// is not recoverable
name|close
argument_list|()
expr_stmt|;
block|}
...
argument_list|)
expr_stmt|;
comment|// Allow the directory factory to report metrics
if|if
condition|(
name|directoryFactory
operator|instanceof
...
block|}
block|}
comment|/** Set UpdateLog to buffer updates if the slice is in construction. */
DECL|method|bufferUpdatesIfConstructing
specifier|private
name|void
name|bufferUpdatesIfConstructing
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Initializes the core's {@link SolrCoreMetricManager} with a given configuration.    * If metric reporters are configured, they are also initialized for this core.    *    * @param config the given configuration    * @return an instance of {@link SolrCoreMetricManager}    */
DECL|method|initCoreMetricManager
specifier|private
name|SolrCoreMetricManager
name|initCoreMetricManager
parameter_list|(
...
block|}
block|}
comment|// initialize disk total / free metrics
name|Path
name|dataDirPath
init|=
name|Paths
...
expr_stmt|;
block|}
DECL|method|checkVersionFieldExistsInSchema
specifier|private
name|void
name|checkVersionFieldExistsInSchema
parameter_list|(
...
return|;
block|}
comment|/**    * Update the index.properties file with the new index sub directory name    */
...
name|info
argument_list|(
literal|"Updating index properties... index="
operator|+
name|tmpIdxDirName
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Write the index.properties file with the new index sub directory name    * @param dir a data directory (containing an index.properties file)    * @param tmpFileName the file name to write the new index.properties to    * @param tmpIdxDirName new index directory name    */
DECL|method|writeNewIndexProps
specifier|private
specifier|static
name|void
name|writeNewIndexProps
...
argument_list|()
decl_stmt|;
comment|// Read existing properties
try|try
block|{
specifier|final
name|IndexInput
...
parameter_list|)
block|{
comment|// ignore; file does not exist
block|}
name|p
operator|.
name|put
...
return|;
block|}
comment|/**    * Close the core, if it is still in use waits until is no longer in use.    * @see #close()     * @see #isClosed()     */
...
name|info
argument_list|(
literal|"Core {} is not yet closed, waiting {} ms before checking again."
argument_list|,
name|getName
argument_list|()
argument_list|,
...
name|SERVER_ERROR
argument_list|,
literal|"Caught InterruptedException whilst waiting for core "
operator|+
name|getName
argument_list|()
operator|+
...
block|}
block|}
comment|// Close the snapshots meta-data directory.
name|Directory
name|snapshotsDir
init|=
name|snapshotMgr
...
decl_stmt|;
comment|// For testing
DECL|method|areAllSearcherReferencesEmpty
name|boolean
name|areAllSearcherReferencesEmpty
parameter_list|()
block|{
name|boolean
name|isEmpty
...
return|;
block|}
comment|/**    * Computes fingerprint of a segment and caches it only if all the version in segment are included in the fingerprint.    * We can't use computeIfAbsent as caching is conditional (as described above)    * There is chance that two threads may compute fingerprint on the same segment. It might be OK to do so rather than locking entire map.    *    * @param searcher   searcher that includes specified LeaderReaderContext    * @param ctx        LeafReaderContext of a segment to compute fingerprint of    * @param maxVersion maximum version number to consider for fingerprint computation    * @return IndexFingerprint of the segment    * @throws IOException Can throw IOException    */
...
name|debug
argument_list|(
literal|"Cannot cache IndexFingerprint as reader does not support caching. searcher:{} reader:{} readerHash:{} maxVersion:{}"
argument_list|,
name|searcher
argument_list|,
name|ctx
...
argument_list|)
expr_stmt|;
comment|// fingerprint is either not cached or
comment|// if we want fingerprint only up to a version less than maxVersionEncountered in the segment, or
comment|// documents were deleted from segment for which fingerprint was cached
comment|//
if|if
condition|(
name|f
...
name|debug
argument_list|(
literal|"IndexFingerprint cache miss for searcher:{} reader:{} readerHash:{} maxVersion:{}"
argument_list|,
name|searcher
argument_list|,
name|ctx
...
argument_list|)
expr_stmt|;
comment|// cache fingerprint for the segment only if all the versions in the segment are included in the fingerprint
if|if
condition|(
name|f
operator|.
...
name|info
argument_list|(
literal|"Caching fingerprint for searcher:{} leafReaderContext:{} mavVersion:{}"
argument_list|,
name|searcher
argument_list|,
name|ctx
...
name|debug
argument_list|(
literal|"IndexFingerprint cache hit for searcher:{} reader:{} readerHash:{} maxVersion:{}"
argument_list|,
name|searcher
argument_list|,
name|ctx
...
name|debug
argument_list|(
literal|"Cache Size: {}, Segments Size:{}"
argument_list|,
name|perSegmentFingerprintCache
operator|.
name|size
...
init|)
block|{
comment|// Check if the core is closed again inside the lock in case this method is racing with a close. If the core is
comment|// closed, clean up the new searcher and bail.
if|if
condition|(
name|isClosed
argument_list|()
...
argument_list|()
expr_stmt|;
comment|// once for caller since we're not returning it
name|newSearcher
operator|.
name|decref
argument_list|()
expr_stmt|;
comment|// once for ourselves since it won't be "replaced"
throw|throw
operator|new
name|SolrException
argument_list|(
...
literal|null
decl_stmt|;
comment|// if the underlying searcher has not changed, no warming is needed
if|if
condition|(
name|newSearcher
operator|!=
...
return|;
block|}
DECL|method|initDefaultPlugin
specifier|public
name|void
name|initDefaultPlugin
parameter_list|(
name|Object
name|plugin
parameter_list|,
...
return|;
block|}
comment|/**    * @return A reference of {@linkplain SolrSnapshotMetaDataManager}    *         managing the persistent snapshots for this Solr core.    */
DECL|method|getSnapshotMetaDataManager
specifier|public
name|SolrSnapshotMetaDataManager
...
block|}
comment|/////////////////////////////////////////////////////////////////////
comment|// SolrInfoBean stuff: Statistics and Module Info
comment|/////////////////////////////////////////////////////////////////////
annotation|@
name|Override
...
name|error
argument_list|(
literal|"Failed to cleanup old index directories for core "
operator|+
name|coreName
argument_list|,
name|exc
...
block|}
argument_list|,
literal|"OldIndexDirectoryCleanupThreadForCore-"
operator|+
name|coreName
argument_list|)
decl_stmt|;
...
block|}
block|}
DECL|field|implicitPluginsInfo
specifier|private
specifier|static
specifier|final
name|Map
name|implicitPluginsInfo
init|=
operator|(
name|Map
operator|)
...
argument_list|(
literal|"ImplicitPlugins.json"
argument_list|)
decl_stmt|;
DECL|method|getImplicitHandlers
specifier|public
name|List
argument_list|<
name|PluginInfo
...
return|;
block|}
comment|/**    * Convenience method to load a blob. This method minimizes the degree to which component and other code needs     * to depend on the structure of solr's object graph and ensures that a proper close hook is registered. This method     * should normally be called in {@link SolrCoreAware#inform(SolrCore)}, and should never be called during request    * processing. The Decoder will only run on the first invocations, subsequent invocations will return the     * cached object.     *     * @param key A key in the format of name/version for a blob stored in the .system blob store via the Blob Store API    * @param decoder a decoder with which to convert the blob into a Java Object representation (first time only)    * @return a reference to the blob that has already cached the decoded version.    */
DECL|method|loadDecodeAndCacheBlob
specifier|public
name|BlobRepository
operator|.
name|BlobContentRef
...
parameter_list|)
block|{
comment|// make sure component authors don't give us oddball keys with no version...
if|if
condition|(
operator|!
name|BlobRepository
...
argument_list|(
literal|"invalid key format, must end in /N where N is the version number"
argument_list|)
throw|;
block|}
comment|// define the blob
name|BlobRepository
operator|.
name|BlobContentRef
name|blobRef
