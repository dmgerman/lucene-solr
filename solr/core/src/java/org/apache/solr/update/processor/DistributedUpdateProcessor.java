...
decl_stmt|;
DECL|field|DISTRIB_INPLACE_PREVVERSION
specifier|public
specifier|static
specifier|final
name|String
...
literal|null
decl_stmt|;
comment|// should we clone the document before sending it to replicas?
comment|// this is set to true in the constructor if the next processors in the chain
comment|// are custom and may modify the SolrInputDocument racing with its serialization for replication
DECL|field|cloneRequiredOnLeader
specifier|private
specifier|final
name|boolean
name|cloneRequiredOnLeader
decl_stmt|;
...
DECL|method|setupRequestForDBQ
specifier|private
name|List
argument_list|<
name|Node
argument_list|>
name|setupRequestForDBQ
parameter_list|()
block|{
name|List
argument_list|<
...
block|}
block|}
comment|// If we were sent a previous version, set this to the AddUpdateCommand (if not already set)
if|if
condition|(
operator|!
name|cmd
...
argument_list|()
expr_stmt|;
comment|//just in case anyone is waiting let them know that we have a new update
...
condition|)
block|{
comment|// this was checked for (in waitForDependentUpdates()) before entering the synchronized block.
comment|// So we shouldn't be here, unless what must've happened is:
comment|// by the time synchronization block was entered, the prev update was deleted by DBQ. Since
comment|// now that update is not in index, the vinfo.lookupVersion() is possibly giving us a version
comment|// from the deleted list (which might be older than the prev update!)
name|UpdateCommand
name|fetchedFromLeader
init|=
name|fetchFullUpdateFromLeader
argument_list|(
name|cmd
argument_list|,
name|versionOnUpdate
...
name|info
argument_list|(
literal|"In-place update of {} failed to find valid lastVersion to apply to, and the document"
operator|+
literal|" was deleted at the leader subsequently."
argument_list|,
name|idBytes
operator|.
name|utf8ToString
...
name|AddUpdateCommand
assert|;
comment|// Newer document was fetched from the leader. Apply that document instead of this current in-place update.
name|log
operator|.
name|info
argument_list|(
literal|"In-place update of {} failed to find valid lastVersion to apply to, forced to fetch full doc from leader: {}"
argument_list|,
name|idBytes
operator|.
name|utf8ToString
...
condition|)
block|{
comment|// this means we got a newer full doc update and in that case it makes no sense to apply the older
comment|// inplace update. Drop this update
name|log
operator|.
name|info
argument_list|(
literal|"Update was applied on version: "
operator|+
name|prev
operator|+
literal|", but last version I have is: "
operator|+
name|lastVersion
operator|+
literal|". Dropping current update."
argument_list|)
expr_stmt|;
return|return
literal|true
...
block|{
comment|// We're good, we should apply this update. First, update the bucket's highest.
if|if
condition|(
name|bucketVersion
operator|!=
...
return|;
block|}
comment|/**    * This method checks the update/transaction logs and index to find out if the update ("previous update") that the current update    * depends on (in the case that this current update is an in-place update) has already been completed. If not,    * this method will wait for the missing update until it has arrived. If it doesn't arrive within a timeout threshold,    * then this actively fetches from the leader.    *     * @return -1 if the current in-place should be dropped, or last found version if previous update has been indexed.    */
DECL|method|waitForDependentUpdates
specifier|private
name|long
name|waitForDependentUpdates
parameter_list|(
...
name|debug
argument_list|(
literal|"Re-ordered inplace update. version={}, prevVersion={}, lastVersion={}, replayOrPeerSync={}, id={}"
argument_list|,
operator|(
name|cmd
operator|.
...
condition|)
block|{
comment|// wait(0) waits forever until notified, but we don't want that.
name|bucket
operator|.
name|wait
argument_list|(
...
condition|)
block|{
comment|// This must've been the case due to a higher version full update succeeding concurrently, while we were waiting or
comment|// trying to index this partial update. Since a full update more recent than this partial update has succeeded,
comment|// we can drop the current update.
if|if
condition|(
name|log
operator|.
...
name|debug
argument_list|(
literal|"Update was applied on version: {}, but last version I have is: {}"
operator|+
literal|". Current update should be dropped. id={}"
argument_list|,
name|cmd
operator|.
name|prevVersion
...
name|debug
argument_list|(
literal|"Dependent update found. id={}"
argument_list|,
name|cmd
operator|.
name|getPrintableId
...
return|;
block|}
comment|// We have waited enough, but dependent update didn't arrive. Its time to actively fetch it from leader
name|log
operator|.
name|info
argument_list|(
literal|"Missing update, on which current in-place update depends on, hasn't arrived. id={}, looking for version={}, last found version={}"
argument_list|,
name|cmd
operator|.
name|getPrintableId
...
name|info
argument_list|(
literal|"Tried to fetch document {} from the leader, but the leader says document has been deleted. "
operator|+
literal|"Deleting the document here and skipping this update: Last found version: {}, was looking for: {}"
argument_list|,
name|cmd
operator|.
name|getPrintableId
...
name|info
argument_list|(
literal|"Added the fetched document, id="
operator|+
operator|(
operator|(
name|AddUpdateCommand
...
return|;
block|}
comment|/**    * This method is used when an update on which a particular in-place update has been lost for some reason. This method    * sends a request to the shard leader to fetch the latest full document as seen on the leader.    * @return AddUpdateCommand containing latest full doc at shard leader for the given id, or null if not found.    */
DECL|method|fetchFullUpdateFromLeader
specifier|private
name|UpdateCommand
name|fetchFullUpdateFromLeader
parameter_list|(
...
condition|)
block|{
comment|// An update we're dependent upon didn't arrive! This is unexpected. Perhaps likely our leader is
comment|// down or partitioned from us for some reason. Lets force refresh cluster state, and request the
comment|// leader for the update.
if|if
condition|(
name|zkController
operator|==
...
condition|)
block|{
comment|// we should be in cloud mode, but wtf? could be a unit test
throw|throw
operator|new
name|SolrException
argument_list|(
...
name|SERVER_ERROR
argument_list|,
literal|"Can't find document with id="
operator|+
name|id
operator|+
literal|", but fetching from leader "
operator|+
literal|"failed since we're not in cloud mode."
argument_list|)
throw|;
block|}
name|Replica
...
name|SERVER_ERROR
argument_list|,
literal|"Exception during fetching from leader."
argument_list|,
name|e
argument_list|)
throw|;
...
condition|)
block|{
comment|// this doc was not found (deleted) on the leader. Lets delete it here as well.
name|DeleteUpdateCommand
name|del
init|=
operator|new
...
condition|)
block|{
comment|// non-empty means this is suitable for in-place updates
if|if
condition|(
name|docMerger
operator|.
...
block|{
comment|// in-place update failed, so fall through and re-try the same with a full atomic update
block|}
block|}
comment|// full (non-inplace) atomic update
name|SolrInputDocument
name|sdoc
init|=
name|cmd
...
argument_list|)
decl_stmt|;
comment|// DBQ forwarded to NRT and TLOG replicas
name|List
argument_list|<
name|ZkCoreNodeProps
argument_list|>
...
condition|)
block|{
comment|// TLOG replica not leader, don't write the DBQ to IW
name|cmd
operator|.
name|setFlags
argument_list|(
...
condition|)
block|{
comment|// This could happen if there are only pull replicas
throw|throw
operator|new
name|SolrException
argument_list|(
...
name|SERVER_ERROR
argument_list|,
literal|"Unable to distribute commit operation. No replicas available of types "
operator|+
name|Replica
operator|.
name|Type
...
argument_list|()
operator|+
literal|" not in sync with leader"
assert|;
block|}
block|}
catch|catch
...
name|warn
argument_list|(
literal|"Commit not supported on replicas of type "
operator|+
name|Replica
operator|.
name|Type
...
block|{
comment|// NRT replicas will always commit
if|if
condition|(
name|vinfo
operator|!=
