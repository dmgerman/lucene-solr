...
block|}
block|}
comment|// NOTE: when adding new states make sure to keep existing numbers, because external metrics
comment|// monitoring may depend on these values being stable.
DECL|enum|State
...
comment|/**    * The index of the flags value in an entry from the transaction log.    */
DECL|field|FLAGS_IDX
specifier|public
specifier|static
specifier|final
name|int
...
literal|0
decl_stmt|;
comment|/**    * The index of the _version_ value in an entry from the transaction log.    */
DECL|field|VERSION_IDX
specifier|public
specifier|static
specifier|final
name|int
...
literal|1
decl_stmt|;
comment|/**    * The index of the previous pointer in an entry from the transaction log.    * This is only relevant if flags (indexed at FLAGS_IDX) includes UPDATE_INPLACE.    */
DECL|field|PREV_POINTER_IDX
specifier|public
specifier|static
specifier|final
name|int
...
literal|2
decl_stmt|;
comment|/**    * The index of the previous version in an entry from the transaction log.    * This is only relevant if flags (indexed at FLAGS_IDX) includes UPDATE_INPLACE.    */
DECL|field|PREV_VERSION_IDX
specifier|public
specifier|static
specifier|final
name|int
...
DECL|field|maxVersionFromIndex
specifier|protected
name|Long
...
decl_stmt|;
DECL|field|replayLogsCountGauge
specifier|protected
name|Gauge
argument_list|<
name|Integer
argument_list|>
name|replayLogsCountGauge
decl_stmt|;
DECL|field|replayBytesGauge
specifier|protected
name|Gauge
argument_list|<
name|Long
argument_list|>
name|replayBytesGauge
decl_stmt|;
DECL|field|stateGauge
specifier|protected
name|Gauge
argument_list|<
name|Integer
...
name|stateGauge
decl_stmt|;
DECL|field|applyingBufferedOpsMeter
specifier|protected
name|Meter
name|applyingBufferedOpsMeter
decl_stmt|;
DECL|field|replayOpsMeter
specifier|protected
name|Meter
name|replayOpsMeter
decl_stmt|;
...
name|version
decl_stmt|;
DECL|field|previousPointer
specifier|final
name|long
name|previousPointer
decl_stmt|;
comment|// used for entries that are in-place updates and need a pointer to a previous update command
comment|/**      * Creates an object that contains the position and version of an update. In this constructor,      * the effective value of the previousPointer is -1.      *       * @param pointer Position in the transaction log of an update      * @param version Version of the update at the given position      */
DECL|method|LogPtr
specifier|public
name|LogPtr
parameter_list|(
...
expr_stmt|;
block|}
comment|/**      *       * @param pointer Position in the transaction log of an update      * @param version Version of the update at the given position      * @param previousPointer Position, in the transaction log, of an update on which the current update depends       */
DECL|method|LogPtr
specifier|public
name|LogPtr
parameter_list|(
...
return|;
block|}
DECL|method|getNumRecordsToKeep
specifier|public
name|int
...
return|;
block|}
DECL|method|getMaxNumLogsToKeep
specifier|public
name|int
...
condition|)
block|{
comment|// numRecords counts header as a record
return|return
name|tlog
operator|.
name|numRecords
...
block|}
block|}
comment|/**    * @return If cmd is an in-place update, then returns the pointer (in the tlog) of the previous    *        update that the given update depends on.    *        Returns -1 if this is not an in-place update, or if we can't find a previous entry in    *        the tlog. Upon receiving a -1, it should be clear why it was -1: if the command's    *        flags|UpdateLog.UPDATE_INPLACE is set, then this command is an in-place update whose    *        previous update is in the index and not in the tlog; if that flag is not set, it is    *        not an in-place update at all, and don't bother about the prevPointer value at    *        all (which is -1 as a dummy value).)    */
DECL|method|getPrevPointerForUpdate
specifier|private
specifier|synchronized
name|long
name|getPrevPointerForUpdate
...
parameter_list|)
block|{
comment|// note: sync required to ensure maps aren't changed out form under us
if|if
condition|(
name|cmd
operator|.
...
name|log
argument_list|,
literal|"Error opening realtime searcher"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
parameter_list|,
name|SolrDocumentBase
name|latestPartialDoc
parameter_list|)
block|{
name|SolrInputDocument
name|partialUpdateDoc
init|=
literal|null
decl_stmt|;
name|List
...
condition|)
block|{
comment|//go through each partial update and apply it on the incoming doc one after another
name|List
name|entry
decl_stmt|;
name|entry
operator|=
name|getEntryFromTLog
argument_list|(
name|prevPointer
argument_list|,
name|prevVersion
...
name|prevPointer
return|;
comment|// a previous update was supposed to be found, but wasn't found (due to log rotation)
block|}
name|int
name|flags
init|=
...
argument_list|)
decl_stmt|;
comment|// since updates can depend only upon ADD updates or other UPDATE_INPLACE updates, we assert that we aren't
comment|// getting something else
if|if
condition|(
operator|(
name|flags
...
name|entry
operator|+
literal|" should've been either ADD or UPDATE_INPLACE update"
operator|+
literal|", while looking for id="
operator|+
operator|new
name|String
argument_list|(
...
throw|;
block|}
comment|// if this is an ADD (i.e. full document update), stop here
if|if
condition|(
operator|(
name|flags
...
literal|0
return|;
comment|// Full document was found in the tlog itself
block|}
if|if
condition|(
name|entry
...
throw|;
block|}
comment|// This update is an inplace update, get the partial doc. The input doc is always at last position.
name|partialUpdateDoc
operator|=
operator|(
name|SolrInputDocument
...
literal|0
return|;
comment|// all the onlyTheseFields have been resolved, safe to abort now.
block|}
block|}
return|return
operator|-
literal|1
return|;
comment|// last full document is not supposed to be in tlogs, but it must be in the index
block|}
comment|/**    * Add all fields from olderDoc into newerDoc if not already present in newerDoc    */
DECL|method|applyOlderUpdates
specifier|private
name|void
name|applyOlderUpdates
parameter_list|(
...
control|)
block|{
comment|// if the newerDoc has this field, then this field from olderDoc can be ignored
if|if
condition|(
operator|!
name|newerDoc
...
block|}
block|}
comment|/***    * Get the entry that has the given lookupVersion in the given lookupLogs at the lookupPointer position.    *    * @return The entry if found, otherwise null    */
DECL|method|getEntryFromTLog
specifier|private
specifier|synchronized
name|List
name|getEntryFromTLog
...
parameter_list|)
block|{
comment|// This can happen when trying to deserialize the entry at position lookupPointer,
comment|// but from a different tlog than the one containing the desired entry.
comment|// Just ignore the exception, so as to proceed to the next tlog.
name|log
operator|.
name|debug
argument_list|(
literal|"Exception reading the log (this is expected, don't worry)="
operator|+
name|lookupLog
operator|+
literal|", for version="
...
return|;
block|}
comment|/**    * Block updates, append a commit at current tlog,    * then copy over buffer updates to new tlog and bring back ulog to active state.    * So any updates which hasn't made it to the index is preserved in the current tlog,    * this also make RTG work    * @param cuc any updates that have version larger than the version of cuc will be copied over    */
DECL|method|copyOverBufferingUpdates
specifier|public
name|void
name|copyOverBufferingUpdates
parameter_list|(
name|CommitUpdateCommand
name|cuc
parameter_list|)
...
block|}
block|}
comment|/**    * Block updates, append a commit at current tlog, then copy over updates to a new tlog.    * So any updates which hasn't made it to the index is preserved in the current tlog    * @param cuc any updates that have version larger than the version of cuc will be copied over    */
DECL|method|copyOverOldUpdates
specifier|public
name|void
...
block|}
block|}
DECL|method|copyAndSwitchToNewTlog
specifier|protected
name|void
name|copyAndSwitchToNewTlog
parameter_list|(
...
block|}
block|}
comment|/**    * Copy over updates from prevTlog or last tlog (in tlog folder) to a new tlog    * @param commitVersion any updates that have version larger than the commitVersion will be copied over    */
DECL|method|copyOverOldUpdates
specifier|public
name|void
name|copyOverOldUpdates
...
block|}
block|}
comment|// Prev tlog will be closed, so nullify prevMap
if|if
condition|(
name|prevTlog
operator|==
...
name|version
decl_stmt|;
DECL|field|previousVersion
name|long
name|previousVersion
decl_stmt|;
comment|// for in-place updates
DECL|field|pointer
name|long
name|pointer
decl_stmt|;
...
block|}
block|}
DECL|method|getMaxRecentVersion
specifier|public
name|long
...
condition|)
block|{
comment|// after replay, update the max from the index
name|log
operator|.
name|info
argument_list|(
literal|"Re-computing max version from index after log re-play."
argument_list|)
expr_stmt|;
name|maxVersionFromIndex
operator|=
...
name|cmd
init|=
name|convertTlogEntryToAddUpdateCommand
argument_list|(
name|req
argument_list|,
name|entry
...
block|}
block|}
comment|/**    * Given a entry from the transaction log containing a document, return a new AddUpdateCommand that     * can be applied to ADD the document or do an UPDATE_INPLACE.    *    * @param req The request to use as the owner of the new AddUpdateCommand    * @param entry Entry from the transaction log that contains the document to be added    * @param operation The value of the operation flag; this must be either ADD or UPDATE_INPLACE --     *        if it is UPDATE_INPLACE then the previous version will also be read from the entry    * @param version Version already obtained from the entry.    */
DECL|method|convertTlogEntryToAddUpdateCommand
specifier|public
specifier|static
name|AddUpdateCommand
name|convertTlogEntryToAddUpdateCommand
...
argument_list|(
literal|"No searcher available to lookup max version from index!"
argument_list|)
throw|;
try|try
block|{
...
return|;
block|}
comment|/**    * Used to seed all version buckets with the max value of the version field in the index.    */
...
expr_stmt|;
block|}
comment|// seed all version buckets with the highest value from recent and index
name|versions
operator|.
name|seedBucketsWithHighestVersion
argument_list|(
...
name|warn
argument_list|(
literal|"Failed to determine the max value of the version field due to: "
operator|+
name|ioExc
argument_list|,
name|ioExc
