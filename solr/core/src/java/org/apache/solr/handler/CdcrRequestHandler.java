...
name|info
argument_list|(
literal|"Found maxVersionFromRecent {} maxVersionFromIndex {}"
argument_list|,
name|maxVersionFromRecent
argument_list|,
name|maxVersionFromIndex
argument_list|)
expr_stmt|;
comment|// there is no race with ongoing bootstrap because we don't expect any updates to come from the source
name|long
name|maxVersion
init|=
name|Math
...
name|SHARDCHECKPOINT
operator|+
literal|"' could not read max version"
argument_list|)
throw|;
block|}
block|}
...
decl_stmt|;
DECL|field|bootstrapCallable
specifier|private
specifier|volatile
name|BootstrapCallable
name|bootstrapCallable
decl_stmt|;
DECL|method|handleBootstrapAction
specifier|private
name|void
name|handleBootstrapAction
parameter_list|(
...
name|warn
argument_list|(
literal|"Bootstrap was interrupted"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Bootstrap operation failed"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Action {} sent to non-leader replica @ {}:{}. Aborting bootstrap."
argument_list|,
name|CdcrParams
operator|.
name|CdcrAction
...
parameter_list|)
block|{
comment|// no problem, we're probably shutting down
name|rsp
operator|.
name|add
argument_list|(
...
block|}
block|}
DECL|method|handleCancelBootstrap
specifier|private
name|void
name|handleCancelBootstrap
parameter_list|(
...
expr_stmt|;
block|}
DECL|method|handleBootstrapStatus
specifier|private
name|void
name|handleBootstrapStatus
parameter_list|(
...
name|RESPONSE_MESSAGE
argument_list|,
literal|"No bootstrap found in running, completed or failed states"
argument_list|)
expr_stmt|;
block|}
elseif|else
...
condition|)
block|{
comment|// could be a normal termination or an exception
try|try
block|{
name|Boolean
name|result
...
name|RESPONSE_MESSAGE
argument_list|,
literal|"Bootstrap was cancelled"
argument_list|)
expr_stmt|;
block|}
block|}
...
argument_list|()
decl_stmt|;
comment|// we start buffering updates as a safeguard however we do not expect
comment|// to receive any updates from the source during bootstrap
name|ulog
operator|.
name|bufferUpdates
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// we do not want the raw tlog files from the source
name|solrParams
operator|.
name|set
argument_list|(
...
comment|// Applying buffered updates fails without the following line because LogReplayer
comment|// also tries to set request info and fails with AssertionError
name|SolrRequestInfo
operator|.
...
condition|)
block|{
comment|// we cannot apply the buffer in this case because it will introduce newer versions in the
comment|// update log and then the source cluster will get those versions via collectioncheckpoint
comment|// causing the versions in between to be completely missed
name|boolean
name|dropped
init|=
name|ulog
