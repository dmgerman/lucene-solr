...
condition|)
block|{
comment|//System.out.println( ev + "["+JSONParser.getEventString(ev)+"] "+parser.wasKey() ); //+ parser.getString() );
switch|switch
condition|(
name|ev
condition|)
...
argument_list|()
expr_stmt|;
comment|/*** if we wanted to remove the field from the document now...           if (!inArray) {             Object o = stack.peek();             // if null was only value in the field, then remove the field             if (o instanceof SolrInputField) {               SolrInputField sif = (SolrInputField)o;               if (sif.getValueCount() == 0) {                 sdoc.remove(sif.getName());               }             }           }           ***/
name|addValToField
argument_list|(
name|stack
argument_list|,
