...
argument_list|()
decl_stmt|;
comment|/**    *<p>    * Undocumented expert level system property to prevent doing a reverse lookup of our hostname.    * This property ill be logged as a suggested workaround if any probems are noticed when doing reverse     * lookup.    *</p>    *    *<p>    * TODO: should we refactor this (and the associated logic) into a helper method for any other places    * where DNS is used?    *</p>    * @see #initHostname    */
DECL|field|PREVENT_REVERSE_DNS_OF_LOCALHOST_SYSPROP
specifier|private
specifier|static
specifier|final
name|String
name|PREVENT_REVERSE_DNS_OF_LOCALHOST_SYSPROP
init|=
literal|"solr.dns.prevent.reverse.lookup"
decl_stmt|;
...
name|info
argument_list|(
literal|"Resolving canonical hostname for local host prevented due to '{}' sysprop"
argument_list|,
name|PREVENT_REVERSE_DNS_OF_LOCALHOST_SYSPROP
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Unable to resolve canonical hostname for local host, possible DNS misconfiguration. "
operator|+
literal|"Set the '"
operator|+
name|PREVENT_REVERSE_DNS_OF_LOCALHOST_SYSPROP
operator|+
literal|"' sysprop to true on startup to "
operator|+
literal|"prevent future lookups if DNS can not be fixed."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Resolving canonical hostname for local host took {} seconds, possible DNS misconfiguration. "
operator|+
literal|"Set the '{}' sysprop to true on startup to prevent future lookups if DNS can not be fixed."
argument_list|,
name|readableTime
argument_list|,
name|PREVENT_REVERSE_DNS_OF_LOCALHOST_SYSPROP
...
comment|// also those from JVM implementation-specific classes
name|MetricUtils
operator|.
...
argument_list|,
name|getInputArgumentsRedacted
argument_list|(
name|mx
argument_list|)
argument_list|)
...
return|;
block|}
DECL|method|getInputArgumentsRedacted
specifier|private
specifier|static
name|List
argument_list|<
