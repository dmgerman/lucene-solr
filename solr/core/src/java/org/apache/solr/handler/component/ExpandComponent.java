...
argument_list|()
decl_stmt|;
comment|// Since the expand component depends on the operation of the collapse component,
comment|// which validates that numeric field types are 32-bit,
comment|// we don't need to handle invalid 64-bit field types here.
if|if
condition|(
name|defaultValue
operator|!=
...
condition|)
block|{
comment|// Integer case already handled by nullValue defaulting to 0
name|nullValue
operator|=
name|Float
operator|.
...
condition|)
block|{
comment|//Get The Top Level SortedDocValues again so we can re-iterate:
if|if
condition|(
name|CollapsingQParserPlugin
operator|.
...
return|;
block|}
DECL|method|getPointGroupQuery
specifier|private
name|Query
name|getPointGroupQuery
parameter_list|(
...
argument_list|(
literal|"FieldType must be INT,LONG,FLOAT,DOUBLE found "
operator|+
name|fieldType
argument_list|)
throw|;
