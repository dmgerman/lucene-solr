...
name|replicationHandler
decl_stmt|;
DECL|field|replicationStartTimeStamp
specifier|private
specifier|volatile
name|Date
name|replicationStartTimeStamp
decl_stmt|;
DECL|field|replicationTimer
specifier|private
name|RTimer
name|replicationTimer
decl_stmt|;
DECL|field|solrCore
specifier|private
specifier|final
...
decl_stmt|;
DECL|field|tlogFilesToDownload
specifier|private
specifier|volatile
name|List
argument_list|<
...
decl_stmt|;
DECL|field|tlogFilesDownloaded
specifier|private
specifier|volatile
name|List
argument_list|<
...
name|soTimeout
decl_stmt|;
DECL|field|INTERRUPT_RESPONSE_MESSAGE
specifier|private
specifier|static
specifier|final
name|String
name|INTERRUPT_RESPONSE_MESSAGE
init|=
literal|"Interrupted while waiting for modify lock"
decl_stmt|;
DECL|class|IndexFetchResult
specifier|public
specifier|static
class|class
...
name|exception
decl_stmt|;
DECL|field|FAILED_BY_INTERRUPT_MESSAGE
specifier|public
specifier|static
specifier|final
name|String
name|FAILED_BY_INTERRUPT_MESSAGE
init|=
literal|"Fetching index failed by interrupt"
decl_stmt|;
DECL|field|FAILED_BY_EXCEPTION_MESSAGE
specifier|public
specifier|static
specifier|final
name|String
name|FAILED_BY_EXCEPTION_MESSAGE
init|=
literal|"Fetching index failed by exception"
decl_stmt|;
comment|/** pre-defined results */
DECL|field|ALREADY_IN_SYNC
specifier|public
specifier|static
specifier|final
name|IndexFetchResult
name|ALREADY_IN_SYNC
init|=
operator|new
name|IndexFetchResult
argument_list|(
literal|"Local index commit is already in sync with peer"
argument_list|,
literal|true
argument_list|,
literal|null
argument_list|)
decl_stmt|;
DECL|field|INDEX_FETCH_FAILURE
specifier|public
specifier|static
specifier|final
name|IndexFetchResult
name|INDEX_FETCH_FAILURE
init|=
operator|new
name|IndexFetchResult
argument_list|(
literal|"Fetching lastest index is failed"
argument_list|,
literal|false
argument_list|,
literal|null
argument_list|)
decl_stmt|;
DECL|field|INDEX_FETCH_SUCCESS
specifier|public
specifier|static
specifier|final
name|IndexFetchResult
name|INDEX_FETCH_SUCCESS
init|=
operator|new
name|IndexFetchResult
argument_list|(
literal|"Fetching latest index is successful"
argument_list|,
literal|true
argument_list|,
literal|null
argument_list|)
decl_stmt|;
DECL|field|LOCK_OBTAIN_FAILED
specifier|public
specifier|static
specifier|final
name|IndexFetchResult
name|LOCK_OBTAIN_FAILED
init|=
operator|new
name|IndexFetchResult
argument_list|(
literal|"Obtaining SnapPuller lock failed"
argument_list|,
literal|false
argument_list|,
literal|null
...
specifier|final
name|IndexFetchResult
name|MASTER_VERSION_ZERO
init|=
operator|new
name|IndexFetchResult
argument_list|(
literal|"Index in peer is empty and never committed yet"
argument_list|,
literal|true
argument_list|,
literal|null
argument_list|)
decl_stmt|;
DECL|field|NO_INDEX_COMMIT_EXIST
specifier|public
specifier|static
specifier|final
name|IndexFetchResult
name|NO_INDEX_COMMIT_EXIST
init|=
operator|new
name|IndexFetchResult
argument_list|(
literal|"No IndexCommit in local index"
argument_list|,
literal|false
argument_list|,
literal|null
argument_list|)
decl_stmt|;
DECL|field|PEER_INDEX_COMMIT_DELETED
specifier|public
specifier|static
specifier|final
name|IndexFetchResult
name|PEER_INDEX_COMMIT_DELETED
init|=
operator|new
name|IndexFetchResult
argument_list|(
literal|"No files to download because IndexCommit in peer was deleted"
argument_list|,
literal|false
argument_list|,
literal|null
argument_list|)
decl_stmt|;
DECL|field|LOCAL_ACTIVITY_DURING_REPLICATION
specifier|public
specifier|static
specifier|final
name|IndexFetchResult
name|LOCAL_ACTIVITY_DURING_REPLICATION
init|=
operator|new
name|IndexFetchResult
argument_list|(
literal|"Local index modification during replication"
argument_list|,
literal|false
argument_list|,
literal|null
argument_list|)
decl_stmt|;
DECL|field|EXPECTING_NON_LEADER
specifier|public
specifier|static
specifier|final
name|IndexFetchResult
...
expr_stmt|;
block|}
comment|/*      * @return exception thrown if failed by exception or interrupt, otherwise null      */
...
return|;
block|}
comment|/*      * @return true if index fetch was successful, false otherwise      */
...
argument_list|)
expr_stmt|;
comment|// allow a master override for tests - you specify this in /replication slave section of solrconfig and some
comment|// test don't want to define this
name|soTimeout
operator|=
name|Integer
operator|.
name|getInteger
argument_list|(
literal|"solr.indexfetcher.sotimeout"
argument_list|,
operator|-
literal|1
argument_list|)
...
name|masterUrl
operator|+
literal|" is not available. Index fetch failed by interrupt. Exception: "
operator|+
name|errorMsg
argument_list|)
expr_stmt|;
...
name|masterUrl
operator|+
literal|" is not available. Index fetch failed by exception: "
operator|+
name|errorMsg
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|// TODO: Should we be comparing timestamps (across machines) here?
if|if
condition|(
operator|!
name|forceReplication
...
name|info
argument_list|(
literal|"Number of tlog files in master: "
operator|+
name|tlogFilesToDownload
operator|.
name|size
...
argument_list|)
expr_stmt|;
comment|// tmp dir for tlog files
if|if
condition|(
name|tlogFilesToDownload
operator|!=
...
condition|)
block|{
comment|// move tlog files and refresh ulog only if we successfully installed a new index
name|successfulInstall
operator|&=
name|moveTlogFiles
argument_list|(
...
parameter_list|)
block|{
comment|// this can happen on shutdown, a fetch may be running in a thread after DirectoryFactory is closed
name|LOG
operator|.
name|warn
argument_list|(
literal|"Could not log failed replication details"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|{
comment|// order below is important
try|try
block|{
if|if
condition|(
...
name|reason
operator|=
literal|"Need currentTimeMillis for debugging/stats"
argument_list|)
...
block|}
block|}
comment|/**    * Download all the tlog files to the temp tlog directory.    */
...
name|info
argument_list|(
literal|"Starting download of tlog files from master: "
operator|+
name|tlogFilesToDownload
argument_list|)
expr_stmt|;
...
name|SERVER_ERROR
argument_list|,
literal|"Failed to create temporary tlog folder: "
operator|+
name|tmpTlogDir
operator|.
name|getName
...
name|debug
argument_list|(
literal|"Downloading file={} size={} checksum={} alwaysDownload={}"
argument_list|,
name|filename
argument_list|,
name|size
...
return|;
block|}
comment|/**    *<p>    *   Copy all the tlog files from the temp tlog dir to the actual tlog dir, and reset    *   the {@link UpdateLog}. The copy will try to preserve the original tlog directory    *   if the copy fails.    *</p>    *<p>    *   This assumes that the tlog files transferred from the leader are in synch with the    *   index files transferred from the leader. The reset of the update log relies on the version    *   of the latest operations found in the tlog files. If the tlogs are ahead of the latest commit    *   point, it will not copy all the needed buffered updates for the replay and it will miss    *   some operations.    *</p>    */
DECL|method|moveTlogFiles
specifier|private
name|boolean
name|moveTlogFiles
parameter_list|(
...
argument_list|()
expr_stmt|;
comment|// block updates until the new update log is initialised
try|try
block|{
...
argument_list|()
decl_stmt|;
comment|// try to move the temp tlog files to the tlog directory
if|if
condition|(
operator|!
...
literal|false
return|;
comment|// reinitialise the update log and copy the buffered updates
if|if
condition|(
name|bufferedUpdates
operator|.
...
condition|)
block|{
comment|// map file path to its new backup location
name|File
name|parentDir
init|=
name|FileSystems
...
expr_stmt|;
block|}
comment|// init the update log with the new set of tlog files, and copy the buffered updates
operator|(
operator|(
name|CdcrUpdateLog
operator|)
...
name|error
argument_list|(
literal|"Unable to copy tlog files"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * The tlog files are moved from the tmp dir to the tlog dir as an atomic filesystem operation.    * A backup of the old directory is maintained. If the directory move fails, it will try to revert back the original    * tlog directory.    */
DECL|method|copyTmpTlogFiles2Tlog
specifier|private
name|boolean
name|copyTmpTlogFiles2Tlog
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// In case of error, try to revert back the original tlog directory
try|try
block|{
name|Files
operator|.
...
parameter_list|)
block|{
comment|// bad, we were not able to revert back the original tlog directory
throw|throw
operator|new
name|SolrException
argument_list|(
...
literal|"Need currentTimeMillis for debugging/stats"
argument_list|)
DECL|method|markReplicationStart
specifier|private
name|void
name|markReplicationStart
parameter_list|()
...
expr_stmt|;
block|}
DECL|method|markReplicationStop
specifier|private
name|void
name|markReplicationStop
parameter_list|()
...
expr_stmt|;
block|}
DECL|method|getReplicationStartTimeStamp
name|Date
name|getReplicationStartTimeStamp
parameter_list|()
block|{
return|return
name|replicationStartTimeStamp
...
return|;
block|}
DECL|method|getTlogFilesToDownload
name|List
argument_list|<
name|Map
argument_list|<
...
return|;
block|}
DECL|method|getTlogFilesDownloaded
name|List
argument_list|<
name|Map
argument_list|<
...
name|saveAs
decl_stmt|;
DECL|field|solrParamOutput
specifier|private
specifier|final
name|String
name|solrParamOutput
decl_stmt|;
DECL|field|indexGen
specifier|private
specifier|final
...
name|LOG
argument_list|,
literal|"Error fetching file, doing one retry..."
argument_list|,
name|e
argument_list|)
expr_stmt|;
