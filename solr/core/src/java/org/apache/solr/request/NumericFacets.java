...
name|docIDs
decl_stmt|;
comment|//Will be null if HashTable is created with needsDocId=false
DECL|field|mask
name|int
name|mask
decl_stmt|;
...
condition|)
block|{
comment|// TODO: evaluate using getCountsMultiValued for singleValued numerics with SingletonSortedNumericDocValues
return|return
name|getCountsMultiValued
argument_list|(
name|searcher
argument_list|,
name|docs
...
return|;
block|}
DECL|method|getCountsSingleValue
specifier|private
specifier|static
name|NamedList
argument_list|<
...
argument_list|()
expr_stmt|;
comment|// We don't return zeros when using PointFields or when index=false
specifier|final
name|List
argument_list|<
name|LeafReaderContext
...
return|;
block|}
DECL|method|getCountsMultiValued
specifier|private
specifier|static
name|NamedList
argument_list|<
...
name|IOException
block|{
comment|// If facet.mincount=0 with PointFields the only option is to get the values from DocValues
comment|// not currently supported. See SOLR-10033
name|mincount
operator|=
name|Math
operator|.
...
argument_list|()
decl_stmt|;
comment|// This document must have at least one value
name|hashTable
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// Skip the value if it's equal to the last one, we don't want to double-count it
name|hashTable
operator|.
name|add
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// TODO: convert to correct value
block|}
comment|// Once facet.mincount=0 is supported we'll need to add logic similar to the SingleValue case, but obtaining values
comment|// with count 0 from DocValues
if|if
condition|(
name|missing
condition|)
...
name|AssertionError
argument_list|(
literal|"Unsupported NumberType: "
operator|+
name|fieldType
operator|.
name|getNumberType
