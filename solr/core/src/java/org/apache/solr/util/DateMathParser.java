...
argument_list|)
decl_stmt|;
comment|/**    * A mapping from (uppercased) String labels identifying time units,    * to the corresponding {@link ChronoUnit} enum (e.g. "YEARS") used to    * set/add/roll that unit of measurement.    *    *<p>    * A single logical unit of time might be represented by multiple labels    * for convenience (ie:<code>DATE==DAYS</code>,    *<code>MILLI==MILLIS</code>)    *</p>    *    * @see Calendar    */
...
argument_list|)
expr_stmt|;
comment|// NOTE: Maybe eventually support NANOS
return|return
name|units
return|;
block|}
comment|/**    * Returns a modified time by "adding" the specified value of units    *    * @exception IllegalArgumentException if unit isn't recognized.    * @see #CALENDAR_UNITS    */
DECL|method|add
specifier|private
specifier|static
name|LocalDateTime
...
return|;
block|}
comment|/**    * Returns a modified time by "rounding" down to the specified unit    *    * @exception IllegalArgumentException if unit isn't recognized.    * @see #CALENDAR_UNITS    */
DECL|method|round
specifier|private
specifier|static
name|LocalDateTime
...
throw|;
block|}
comment|// note: OffsetDateTime.truncatedTo does not support>= DAYS units so we handle those
switch|switch
condition|(
name|uu
condition|)
...
name|now
decl_stmt|;
comment|/**    * Default constructor that assumes UTC should be used for rounding unless     * otherwise specified in the SolrRequestInfo    *     * @see SolrRequestInfo#getClientTimeZone    */
...
expr_stmt|;
block|}
comment|/**    * @param tz The TimeZone used for rounding (to determine when hours/days begin).  If null, then this method defaults    *           to the value dictated by the SolrRequestInfo if it exists -- otherwise it uses UTC.    * @see #DEFAULT_MATH_TZ    * @see Calendar#getInstance(TimeZone,Locale)    * @see SolrRequestInfo#getClientTimeZone    */
DECL|method|DateMathParser
specifier|public
name|DateMathParser
parameter_list|(
...
expr_stmt|;
block|}
comment|/**     * Returns a clone of this instance's concept of "now" (never null).    *    * If setNow was never called (or if null was specified) then this method     * first defines 'now' as the value dictated by the SolrRequestInfo if it     * exists -- otherwise it uses a new Date instance at the moment getNow()     * is first called.    * @see #setNow    * @see SolrRequestInfo#getNOW    */
DECL|method|getNow
specifier|public
name|Date
name|getNow
...
argument_list|()
decl_stmt|;
comment|// localDateTime is a date and time local to the timezone specified
name|LocalDateTime
name|localDateTime
init|=
name|ZonedDateTime
