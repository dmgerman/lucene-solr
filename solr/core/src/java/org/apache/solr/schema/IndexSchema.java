...
name|SERVER_ERROR
argument_list|,
literal|"Setting default operator in schema (solrQueryParser/@defaultOperator) not supported"
argument_list|)
throw|;
block|}
...
argument_list|()
expr_stmt|;
comment|// we fail on init if the ROOT field is *explicitly* defined as incompatible with uniqueKey
comment|// we don't want ot fail if there happens to be a dynamicField matching ROOT, (ie: "*")
comment|// because the user may not care about child docs at all.  The run time code
comment|// related to child docs can catch that if it happens
if|if
condition|(
name|fields
operator|.
...
name|ROOT_FIELD_NAME
operator|+
literal|" field must be defined using the exact same fieldType as the "
operator|+
name|UNIQUE_KEY
operator|+
literal|" field ("
...
begin_comment
comment|/**    * Helper method that returns<code>true</code> if the {@link #ROOT_FIELD_NAME} uses the exact     * same 'type' as the {@link #getUniqueKeyField()}    *    * @lucene.internal    */
end_comment

begin_function
DECL|method|isUsableForChildDocs
specifier|public
name|boolean
name|isUsableForChildDocs
parameter_list|()
