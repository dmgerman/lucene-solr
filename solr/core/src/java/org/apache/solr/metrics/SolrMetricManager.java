...
argument_list|()
decl_stmt|;
comment|/**      * Create a filter that uses the provided prefix.      * @param prefix prefix to use, must not be null. If empty then any      *               name will match.      */
...
block|}
block|}
comment|/**    * Move all matching metrics from one registry to another. This is useful eg. during    * {@link org.apache.solr.core.SolrCore} rename or swap operations.    * @param fromRegistry source registry    * @param toRegistry target registry    * @param filter optional {@link MetricFilter} to select what metrics to move. If null    *               then all metrics will be moved.    */
DECL|method|moveMetrics
specifier|public
name|void
name|moveMetrics
parameter_list|(
