...
argument_list|)
expr_stmt|;
comment|// always 100ms - can take a moment for our cores to come up
block|}
name|System
operator|.
name|clearProperty
...
name|Exception
block|{
comment|// we try and do a bunch of extra stop stuff because
comment|// jetty doesn't like to stop if it started
comment|// and ended up in a failure state (like when it cannot get the port)
if|if
condition|(
name|server
operator|.
