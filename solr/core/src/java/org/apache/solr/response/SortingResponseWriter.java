...
comment|// That causes the totalHits and export entries in the context to _not_ get set.
comment|// The only time that really matters is when we search against an _empty_ set. That's too obscure
comment|// a condition to handle as part of this patch, if someone wants to pursue it it can be reproduced with:
comment|// ant test  -Dtestcase=StreamingTest -Dtests.method=testAllValidExportTypes -Dtests.seed=10F13879D0D1D6AD -Dtests.slow=true -Dtests.locale=es-PA -Dtests.timezone=America/Bahia_Banderas -Dtests.asserts=true -Dtests.file.encoding=ISO-8859-1
comment|// You'll have to uncomment the if below to hit the null pointer exception.
comment|// This is such an unusual case (i.e. an empty index) that catching this concdition here is probably OK.
comment|// This came to light in the very artifical case of indexing a single doc to Cloud.
name|int
name|totalHits
init|=
literal|0
...
name|write
argument_list|(
literal|"{\"responseHeader\": {\"status\": 400}, \"response\":{\"numFound\":0, \"docs\":["
argument_list|)
expr_stmt|;
name|out
operator|.
...
name|IOException
argument_list|(
literal|"Export fields must either be one of the following types: int,float,long,double,string,date,boolean"
argument_list|)
throw|;
block|}
block|}
...
condition|)
block|{
comment|// This is a bit of a hack, but since the boolean field stores ByteRefs, just like Strings
comment|// _and_ since "F" happens to sort before "T" (thus false sorts "less" than true)
comment|// we can just use the existing StringValue here.
name|LeafReader
name|reader
init|=
name|searcher
...
name|IOException
argument_list|(
literal|"Sort fields must be one of the following types: int,float,long,double,string,date,boolean"
argument_list|)
throw|;
block|}
block|}
...
condition|)
block|{
comment|// TODO: can we enforce caller to go in order instead?
name|this
operator|.
name|vals
operator|=
