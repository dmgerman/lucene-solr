...
name|SIMPLE
block|;
comment|// NONE is distinct from null since we may want to know if refinement was explicitly turned off.
DECL|method|fromObj
specifier|public
specifier|static
name|FacetRequest
...
name|BAD_REQUEST
argument_list|,
literal|"Unknown RefineMethod method "
operator|+
name|method
argument_list|)
throw|;
...
comment|// True if a starting set of documents can be mapped onto a different set of documents not originally in the starting set.
DECL|method|canTransformDomain
specifier|public
name|boolean
name|canTransformDomain
parameter_list|()
block|{
return|return
name|toParent
...
return|;
block|}
comment|// Can this domain become non-empty if the input domain is empty?  This does not check any sub-facets (see canProduceFromEmpty for that)
DECL|method|canBecomeNonEmpty
specifier|public
name|boolean
name|canBecomeNonEmpty
parameter_list|()
block|{
return|return
name|excludeTags
...
return|;
block|}
comment|/** Are we doing a query time join across other documents */
DECL|class|JoinField
specifier|public
specifier|static
class|class
name|JoinField
...
expr_stmt|;
block|}
comment|/**        * Given a<code>Domain</code>, and a (JSON) map specifying the configuration for that Domain,        * validates if a '<code>join</code>' is specified, and if so creates a<code>JoinField</code>         * and sets it on the<code>Domain</code>.        *        * (params must not be null)        */
...
condition|)
block|{
comment|// TODO: maybe allow simple string (instead of map) to mean "self join on this field name" ?
if|if
condition|(
operator|!
operator|(
...
name|BAD_REQUEST
argument_list|,
literal|"'join' domain change requires a map containing the 'from' and 'to' fields"
argument_list|)
throw|;
block|}
specifier|final
...
name|BAD_REQUEST
argument_list|,
literal|"'join' domain change requires non-null 'from' and 'to' field names"
argument_list|)
throw|;
block|}
if|if
...
name|BAD_REQUEST
argument_list|,
literal|"'join' domain change contains unexpected keys, only 'from' and 'to' supported: "
operator|+
name|join
operator|.
name|toString
...
block|}
block|}
comment|/**        * Creates a Query that can be used to recompute the new "base" for this domain, realtive to the         * current base of the FacetContext.        */
DECL|method|createDomainQuery
specifier|public
name|Query
name|createDomainQuery
parameter_list|(
name|FacetContext
name|fcontext
parameter_list|)
...
name|IOException
block|{
comment|// NOTE: this code lives here, instead of in FacetProcessor.handleJoin, in order to minimize
comment|// the number of classes that have to know about the number of possible settings on the join
comment|// (ie: if we add a score mode, or some other modifier to how the joins are done)
specifier|final
name|SolrConstantScoreQuery
name|fromQuery
init|=
...
argument_list|)
decl_stmt|;
comment|// this shouldn't matter once we're wrapped in a join query, but just in case it ever does...
name|fromQuery
operator|.
name|setCache
argument_list|(
...
return|;
block|}
comment|/** Returns true if this facet can return just some of the facet buckets that match all the criteria.    * This is normally true only for facets with a limit.    */
...
return|;
block|}
comment|/** Returns true if this facet, or any sub-facets can produce results from an empty domain. */
DECL|method|canProduceFromEmpty
specifier|public
name|boolean
...
argument_list|)
expr_stmt|;
comment|// TODO: pull up to higher level?
name|facet
operator|.
name|refine
operator|=
