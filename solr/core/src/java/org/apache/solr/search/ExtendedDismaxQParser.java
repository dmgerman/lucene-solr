...
argument_list|)
expr_stmt|;
comment|// Respect the q.op parameter before mm will be applied later
name|SolrParams
name|defaultParams
init|=
name|SolrParams
...
block|}
comment|/**      * Delegates to the super class unless the field has been specified      * as an alias -- in which case we recurse on each of      * the aliased fields, and the results are composed into a      * DisjunctionMaxQuery.  (so yes: aliases which point at other      * aliases should work)      */
DECL|method|getAliasedMultiTermQuery
specifier|protected
name|Query
name|getAliasedMultiTermQuery
parameter_list|()
...
argument_list|()
expr_stmt|;
comment|// Make a dismax query for each clause position in the boolean per-field queries.
for|for
control|(
name|int
name|n
...
block|}
block|}
comment|/**      * Recursively examines the given query list for identical structure in all queries.      * Boosts on BoostQuery-s are ignored, and the contained queries are instead used as the basis for comparison.      **/
DECL|method|allSameQueryStructure
specifier|private
name|boolean
name|allSameQueryStructure
parameter_list|(
...
argument_list|()
expr_stmt|;
comment|// ignore boost; compare contained query
block|}
for|for
control|(
name|int
...
parameter_list|)
block|{
comment|// We might have been passed a null query; the terms might have been filtered away by the analyzer.
if|if
condition|(
name|q
operator|==
...
condition|)
block|{
comment|// getAliasedMultiTermQuery() constructed a BooleanQuery containing only SHOULD DisjunctionMaxQuery-s.
comment|// Unwrap the query and add a clause for each contained DisMax query.
for|for
control|(
name|BooleanClause
name|c
...
return|;
block|}
DECL|method|getMultiTermQueries
specifier|protected
name|List
argument_list|<
name|Query
