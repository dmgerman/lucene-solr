...
name|wrap
argument_list|(
name|wrapUninvertingReaderPerSegment
argument_list|(
name|core
argument_list|,
name|reader
...
return|;
block|}
comment|/**    * If docvalues are enabled or disabled after data has already been indexed for a field, such that    * only some segments have docvalues, uninverting on the top level reader will cause     * IllegalStateException to be thrown when trying to use a field with such mixed data. This is because    * the {@link IndexSchema#getUninversionMap(IndexReader)} method decides to put a field     * into the uninverteding map only if *NO* segment in the index contains docvalues for that field.    *     * Therefore, this class provides a uninverting map per segment such that for any field,     * DocValues are used from segments if they exist and uninversion of the field is performed on the rest    * of the segments.    */
DECL|method|wrapUninvertingReaderPerSegment
specifier|private
specifier|static
name|DirectoryReader
name|wrapUninvertingReaderPerSegment
...
return|;
block|}
comment|/**    * Not recommended to call this method unless there is some particular reason due to internally calling {@link SlowCompositeReaderWrapper}.    * Use {@link IndexSearcher#leafContexts} to get the sub readers instead of using this method.    */
DECL|method|getSlowAtomicReader
specifier|public
specifier|final
name|LeafReader
...
return|;
block|}
comment|/**    * Register sub-objects such as caches and our own metrics    */
...
comment|// }
comment|// }
comment|/**    * Retrieve the {@link Document} instance corresponding to the document id.    *    * @see SolrDocumentFetcher    */
annotation|@
name|Override
DECL|method|doc
specifier|public
...
return|;
block|}
comment|/**    * Visit a document's fields using a {@link StoredFieldVisitor}.    * This method does not currently add to the Solr document cache.    *     * @see IndexReader#document(int, StoredFieldVisitor)    * @see SolrDocumentFetcher    */
annotation|@
name|Override
DECL|method|doc
specifier|public
...
expr_stmt|;
block|}
comment|/**    * Retrieve the {@link Document} instance corresponding to the document id.    *<p>    *<b>NOTE</b>: the document will have all fields accessible, but if a field filter is provided, only the provided    * fields will be loaded (the remainder will be available lazily).    *    * @see SolrDocumentFetcher    */
annotation|@
name|Override
DECL|method|doc
specifier|public
...
return|;
block|}
comment|/** Returns a weighted sort spec according to this searcher */
DECL|method|weightSortSpec
specifier|public
name|SortSpec
name|weightSortSpec
parameter_list|(
name|SortSpec
name|originalSortSpec
parameter_list|,
name|Sort
name|nullEquivalent
parameter_list|)
...
block|}
comment|/** Returns a weighted sort spec according to this searcher */
DECL|method|implWeightSortSpec
specifier|private
name|SortSpec
name|implWeightSortSpec
parameter_list|(
...
parameter_list|)
block|{
comment|// TODO: this should be implemented in DocSet, most likely with a getBits method that takes a maxDoc argument
comment|// or make DocSet instances remember maxDoc
name|FixedBitSet
name|bs
init|=
operator|new
...
name|liveDocs
decl_stmt|;
comment|/** @lucene.internal the type of DocSet returned may change in the future */
...
DECL|method|isLiveDocsInstantiated
specifier|public
name|boolean
name|isLiveDocsInstantiated
parameter_list|()
block|{
return|return
name|liveDocs
...
parameter_list|)
block|{
comment|// a few places currently expect BitDocSet
assert|assert
name|docs
operator|.
name|size
