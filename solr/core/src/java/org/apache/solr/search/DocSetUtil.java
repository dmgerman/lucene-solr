...
block|}
block|}
comment|/**    * This variant of getDocSet will attempt to do some deduplication    * on certain DocSets such as DocSets that match numDocs.  This means it can return    * a cached version of the set, and the returned set should not be modified.    * @lucene.experimental    */
...
parameter_list|)
block|{
comment|// should be impossible... liveDocs should exist, so no IO should be necessary
throw|throw
operator|new
name|SolrException
argument_list|(
...
return|;
block|}
comment|/**    * This variant of getDocSet maps all sets with size numDocs to searcher.getLiveDocs.    * The returned set should not be modified.    * @lucene.experimental    */
DECL|method|getDocSet
specifier|public
specifier|static
name|DocSet
...
try|try
block|{
comment|// if this docset has the same cardinality as liveDocs, return liveDocs instead
comment|// so this set will be short lived garbage.
return|return
name|searcher
operator|.
name|getLiveDocs
...
return|;
block|}
DECL|method|collectSortedDocSet
specifier|public
specifier|static
name|void
...
name|IOException
block|{
comment|// TODO add SortedDocSet sub-interface and take that.
comment|// TODO collectUnsortedDocSet: iterate segment, then all docSet per segment.
specifier|final
name|List
argument_list|<
name|LeafReaderContext
...
argument_list|(
literal|"algorithm expects sorted DocSet but wasn't: "
operator|+
name|docs
operator|.
name|getClass
