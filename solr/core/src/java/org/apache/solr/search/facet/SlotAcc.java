...
begin_comment
comment|/**  * Accumulates statistics separated by a slot number.   * There is a separate statistic per slot. The slot is usually an ordinal into a set of values, e.g. tracking a count  * frequency<em>per term</em>.  * Sometimes there doesn't need to be a slot distinction, in which case there is just one nominal slot.  */
end_comment
...
name|count
return|;
comment|// returns 0 instead of NaN.. todo - make configurable? if NaN, we need to
comment|// handle comparisons though...
block|}
DECL|method|avg
specifier|private
name|double
...
parameter_list|)
block|{
comment|// TODO: count arrays can use fewer bytes based on the number of docs in
comment|// the base set (that's the upper bound for single valued) - look at ttf?
name|result
index|[
name|slotNum
index|]
