...
argument_list|)
expr_stmt|;
comment|// this just saves space... the presence of FACET_STATE really control the faceting
name|sreq
operator|.
name|params
operator|.
...
argument_list|)
expr_stmt|;
comment|// The presence of FACET_STATE (_facet_) turns on json faceting
block|}
else|else
block|{
comment|// turn off faceting on other requests
comment|/*** distributedProcess will need to use other requests for refinement       sreq.params.remove("json.facet");  // this just saves space... the presence of FACET_STATE really control the faceting       sreq.params.remove(FACET_STATE);        **/
block|}
block|}
annotation|@
name|Override
...
parameter_list|)
block|{
comment|// TODO nocommit - partial results should turn off refining!!!
name|boolean
name|parentMissing
init|=
name|mcontext
