...
condition|)
block|{
comment|// TODO, this is the same in Trie and Point now
switch|switch
condition|(
name|ft
operator|.
...
control|)
block|{
comment|// we don't skip these buckets based on mincount
name|Range
name|range
init|=
name|otherList
...
block|}
block|}
comment|// this refineFacets method is patterned after FacetFieldProcessor.refineFacets and should
comment|// probably be merged when range facet becomes more like field facet in it's ability to sort and limit
...
comment|// currently, only _s should be present for range facets.  In the future, range facets will
comment|// be more like field facets and will have the same refinement cases.  When that happens, we should try to unify the refinement code more
assert|assert
name|leaves
operator|.
name|size
...
expr_stmt|;
block|}
comment|/*** special buckets     if (freq.missing) {       Map<String,Object> bucketFacetInfo = (Map<String,Object>)fcontext.facetInfo.get("missing");        if (bucketFacetInfo != null || !skipThisFacet) {         SimpleOrderedMap<Object> missingBucket = new SimpleOrderedMap<>();         fillBucket(missingBucket, getFieldMissingQuery(fcontext.searcher, freq.field), null, skipThisFacet, bucketFacetInfo);         res.add("missing", missingBucket);       }     }      **********/
...
name|IOException
block|{
comment|// TODO: refactor this repeated code from above
name|Comparable
name|start
init|=
name|calc
