...
literal|false
expr_stmt|;
comment|// we don't use the boost on wrapped queries
block|}
else|else
block|{
name|out
...
return|;
block|}
comment|/**      * Sort direction or null if current position does not inidcate a       * sort direction. (True is desc, False is asc).        * Position is advanced to after the comma (or end) when result is non null       */
