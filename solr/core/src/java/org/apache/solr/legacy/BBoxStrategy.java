...
name|type
expr_stmt|;
comment|// Legacy default: legacyNumerics + docValues
name|LegacyFieldType
name|legacyType
init|=
operator|new
...
DECL|field|optionsFieldType
specifier|private
specifier|final
name|FieldType
name|optionsFieldType
decl_stmt|;
comment|//from constructor; aggregate field type used to express all options
...
name|xdlFieldType
decl_stmt|;
comment|/**    * Creates a new {@link BBoxStrategy} instance that uses {@link DoublePoint} and {@link DoublePoint#newRangeQuery}    */
...
return|;
block|}
comment|/**    * Creates a new {@link BBoxStrategy} instance that uses {@link LegacyDoubleField} for backwards compatibility    * @deprecated LegacyNumerics will be removed    */
annotation|@
name|Deprecated
...
return|;
block|}
comment|/**    * Creates this strategy.    * {@code fieldType} is used to customize the indexing options of the 4 number fields, and to a lesser degree the XDL    * field too. Search requires pointValues (or legacy numerics), and relevancy requires docValues. If these features    * aren't needed then disable them.    */
...
expr_stmt|;
block|}
comment|/** Returns a field type representing the set of field options. This is identical to what was passed into the    * constructor.  It's frozen. */
...
block|}
comment|//---------------------------------
comment|// Query Building
comment|//---------------------------------
comment|//  Utility on SpatialStrategy?
comment|//  public Query makeQueryWithValueSource(SpatialArgs args, ValueSource valueSource) {
comment|//    return new CustomScoreQuery(makeQuery(args), new FunctionQuery(valueSource));
comment|//or...
comment|//  return new BooleanQuery.Builder()
comment|//      .add(new FunctionQuery(valueSource), BooleanClause.Occur.MUST)//matches everything and provides score
comment|//      .add(filterQuery, BooleanClause.Occur.FILTER)//filters (score isn't used)
comment|//  .build();
comment|//  }
annotation|@
name|Override
...
throw|;
block|}
comment|/**    * Returns a numeric range query based on FieldType    * {@link LegacyNumericRangeQuery} is used for indexes created using {@code FieldType.LegacyNumericType}    * {@link DoublePoint#newRangeQuery} is used for indexes created using {@link DoublePoint} fields    *    * @param fieldname field name. must not be<code>null</code>.    * @param min minimum value of the range.    * @param max maximum value of the range.    * @param minInclusive include the minimum value if<code>true</code>.    * @param maxInclusive include the maximum value if<code>true</code>    */
DECL|method|makeNumericRangeQuery
specifier|private
name|Query
name|makeNumericRangeQuery
parameter_list|(
