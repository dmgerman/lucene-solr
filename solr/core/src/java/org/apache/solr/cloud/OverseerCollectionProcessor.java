...
parameter_list|)
block|{
comment|// Just catch and do nothing. The runningMap.contains(..) check ensures that this is the only
comment|// entry point into the runningMap.
comment|// NOTE: Make sure to handle it as soon as OCP gets distributed/multi-threaded.
block|}
name|log
operator|.
name|info
...
argument_list|)
decl_stmt|;
comment|// even if operation is async, it is sync!
name|SolrResponse
name|response
init|=
literal|null
...
expr_stmt|;
block|}
comment|/**    * Get collection status from cluster state.    * Can return collection status by given shard name.    */
...
name|info
argument_list|(
literal|"Request status invoked"
argument_list|)
expr_stmt|;
name|String
name|requestId
