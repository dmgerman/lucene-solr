...
expr_stmt|;
block|}
comment|// let's record the ephemeralOwner of the parent leader node
name|Stat
...
condition|)
block|{
comment|// we just got to know the leader but its live node is gone already!
throw|throw
operator|new
name|SolrException
argument_list|(
...
condition|)
block|{
comment|// refresh the locally cached cluster state
comment|// we know we have the latest because otherwise deleteshard would have failed
name|clusterState
operator|=
name|zkStateReader
operator|.
...
argument_list|()
decl_stmt|;
comment|// compare against the ephemeralOwner of the parent leader node
name|leaderZnodeStat
operator|=
name|zkStateReader
operator|.
...
condition|)
block|{
comment|// put sub-shards in recovery_failed state
name|DistributedQueue
name|inQueue
init|=
name|Overseer
...
condition|)
block|{
comment|// the leader is not live anymore, fail the split!
throw|throw
operator|new
name|SolrException
argument_list|(
...
condition|)
block|{
comment|// there's a new leader, fail the split!
throw|throw
operator|new
name|SolrException
argument_list|(
...
name|SERVER_ERROR
argument_list|,
literal|"The zk session id for the shard leader node: "
operator|+
name|parentShardLeader
operator|.
name|getNodeName
...
argument_list|()
operator|+
literal|". This can cause data loss so we must abort the split"
argument_list|)
throw|;
block|}
block|}
