...
name|info
argument_list|(
literal|"Will not register as leader because collection appears to be gone."
argument_list|)
expr_stmt|;
return|return;
block|}
...
argument_list|)
decl_stmt|;
comment|// only if /collections/{collection} exists already do we succeed in creating this path
name|zcmd
operator|.
name|ensureExists
argument_list|(
...
condition|)
block|{
comment|// stop replicate from old leader
name|zkController
operator|.
...
name|info
argument_list|(
literal|"Replaying tlog before become new leader"
argument_list|)
expr_stmt|;
name|future
operator|.
...
name|info
argument_list|(
literal|"New leader does not have old tlog to replay"
argument_list|)
expr_stmt|;
block|}
block|}
