...
name|info
argument_list|(
literal|"Shard: {} - All replicas across all fellow sub-shards are now ACTIVE."
argument_list|,
name|sliceName
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// now lets see if the parent leader is still the same or else there's a chance of data loss
comment|// see SOLR-9438 for details
name|String
name|shardParentZkSession
init|=
operator|(
name|String
operator|)
...
name|remove
argument_list|(
literal|"shard_parent_zk_session"
argument_list|)
decl_stmt|;
name|String
name|shardParentNode
init|=
operator|(
name|String
operator|)
...
name|info
argument_list|(
literal|"Checking whether sub-shard leader node is still the same one at {} with ZK session id {}"
argument_list|,
name|shardParentNode
argument_list|,
name|shardParentZkSession
...
name|error
argument_list|(
literal|"The shard leader node: {} is not live anymore!"
argument_list|,
name|shardParentNode
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"The zk session id for shard leader node: {} has changed from {} to {}"
argument_list|,
name|shardParentNode
argument_list|,
name|shardParentZkSession
...
name|warn
argument_list|(
literal|"Error occurred while checking if parent shard node is still live with the same zk session id. "
operator|+
literal|"We cannot switch shard states at this time."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|collection
return|;
comment|// we aren't going to make any changes right now
block|}
block|}
if|if
condition|(
...
name|info
argument_list|(
literal|"Sub-shard leader node is still the same one at {} with ZK session id {}. Preparing to switch shard states."
argument_list|,
name|shardParentNode
argument_list|,
name|shardParentZkSession
...
block|{
comment|// we must mark the shard split as failed by switching sub-shards to recovery_failed state
name|Map
argument_list|<
name|String
argument_list|,
