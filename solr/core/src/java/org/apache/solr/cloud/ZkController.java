...
name|info
argument_list|(
literal|"This conf directory is no more watched {}"
argument_list|,
name|configLocation
argument_list|)
expr_stmt|;
...
name|SERVER_ERROR
argument_list|,
literal|"Failed to update data to 'down' for znode: "
operator|+
name|znodePath
operator|+
literal|" because the zookeeper leader sequence for leader: "
operator|+
name|leaderCoreNodeName
operator|+
literal|" is null"
...
argument_list|)
expr_stmt|;
comment|// version doesn't matter, the seq path is unique
name|ops
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|//a race condition where a we missed an even fired
