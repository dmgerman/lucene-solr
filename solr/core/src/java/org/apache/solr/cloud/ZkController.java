...
argument_list|()
decl_stmt|;
comment|// key is the local core name
DECL|field|coreStatesVersion
specifier|private
name|long
name|coreStatesVersion
decl_stmt|;
comment|// bumped by 1 each time we serialize coreStates... sync on  coreStates
DECL|field|coreStatesPublishedVersion
specifier|private
name|long
name|coreStatesPublishedVersion
decl_stmt|;
comment|// last version published to ZK... sync on coreStatesPublishLock
DECL|field|coreStatesPublishLock
specifier|private
name|Object
name|coreStatesPublishLock
init|=
operator|new
name|Object
argument_list|()
decl_stmt|;
comment|// only publish one at a time
...
name|cc
decl_stmt|;
comment|/**    * Bootstraps the current configs for all collections in solr.xml.    * Takes two params - the zkhost to connect to and the solrhome location    * to find solr.xml.    *    * If you also pass a solrPort, it will be used to start    * an embedded zk useful for single machine, multi node tests.    *     * @param args    * @throws Exception    */
DECL|method|main
specifier|public
specifier|static
name|void
...
block|}
block|}
comment|/*    * sync internal state with zk on startup    */
DECL|method|syncNodeState
specifier|private
name|void
name|syncNodeState
parameter_list|()
...
name|debug
argument_list|(
literal|"Syncing internal state with zk. Current: "
operator|+
name|coreStates
argument_list|)
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|// TODO: should we do this?
for|for
control|(
name|CoreState
name|coreState
...
expr_stmt|;
block|}
DECL|method|publishAsRecoveryFailed
name|void
...
condition|)
block|{
comment|// publish with no shard id so we are assigned one, and then look for it
name|doPublish
argument_list|(
name|shardZkNodeName
argument_list|,
...
expr_stmt|;
block|}
comment|// if multiple threads are trying to publish state, make sure that we never write
comment|// an older version after a newer version.
synchronized|synchronized
init|(
name|coreStatesPublishLock
init|)
...
name|info
argument_list|(
literal|"Another thread already published a newer coreStates: ours="
operator|+
name|version
operator|+
...
name|version
expr_stmt|;
comment|// put it after so it won't be set if there's an exception
block|}
block|}
catch|catch
parameter_list|(
...
name|SERVER_ERROR
argument_list|,
literal|"could not publish node state"
argument_list|,
name|e
argument_list|)
throw|;
