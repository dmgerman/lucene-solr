...
decl_stmt|;
DECL|field|replicateFromLeaders
specifier|private
name|Map
argument_list|<
name|String
...
comment|// ref is held as a HashSet since we clone the set before notifying to avoid synchronizing too long
...
name|listener
decl_stmt|;
DECL|method|OnReconnectNotifyAsync
name|OnReconnectNotifyAsync
parameter_list|(
name|OnReconnect
name|listener
...
expr_stmt|;
block|}
comment|// the OnReconnect operation can be expensive per listener, so do that async in the background
for|for
control|(
name|OnReconnect
name|listener
...
argument_list|)
expr_stmt|;
name|checkForExistingEphemeralNode
argument_list|()
expr_stmt|;
comment|// start the overseer first as following code may need it's processing
if|if
...
block|}
block|}
DECL|method|checkForExistingEphemeralNode
specifier|private
name|void
name|checkForExistingEphemeralNode
parameter_list|()
...
condition|)
block|{
comment|// znode suddenly disappeared but that's okay
return|return;
block|}
name|boolean
name|deleted
...
name|SERVER_ERROR
argument_list|,
literal|"A previous ephemeral live node still exists. "
operator|+
literal|"Solr cannot continue. Please ensure that no other Solr process using the same port is running already."
argument_list|)
throw|;
block|}
block|}
...
name|String
argument_list|>
name|collectionsWithLocalReplica
init|=
name|ConcurrentHashMap
operator|.
name|newKeySet
...
control|(
name|String
name|collectionWithLocalReplica
range|:
name|collectionsWithLocalReplica
control|)
block|{
...
expr_stmt|;
block|}
comment|//TODO WHy would replica be null?
if|if
condition|(
name|replica
operator|==
...
name|warn
argument_list|(
literal|"Replica {} was designated as preferred leader but it's type is {}, It won't join election"
argument_list|,
name|coreZkNodeName
argument_list|,
name|Type
...
name|debug
argument_list|(
literal|"Replica {} skipping election because it's type is {}"
argument_list|,
name|coreZkNodeName
argument_list|,
name|Type
...
argument_list|)
expr_stmt|;
name|startReplicationFromLeader
argument_list|(
name|coreName
argument_list|,
literal|false
...
operator|)
operator|:
literal|"Pull replica became leader!"
assert|;
try|try
init|(
name|SolrCore
...
decl_stmt|;
name|boolean
name|isTlogReplicaAndNotLeader
init|=
name|replicaType
operator|==
name|Replica
...
block|}
block|}
DECL|method|startReplicationFromLeader
specifier|public
name|void
name|startReplicationFromLeader
parameter_list|(
...
name|info
argument_list|(
literal|"{} starting background replication from leader"
argument_list|,
name|coreName
argument_list|)
expr_stmt|;
...
init|)
block|{
comment|// synchronize to prevent any stop before we finish the start
if|if
condition|(
name|replicateFromLeaders
operator|.
...
name|warn
argument_list|(
literal|"A replicate from leader instance already exists for core {}"
argument_list|,
name|coreName
argument_list|)
expr_stmt|;
...
block|}
block|}
DECL|method|stopReplicationFromLeader
specifier|public
name|void
name|stopReplicationFromLeader
parameter_list|(
name|String
name|coreName
parameter_list|)
...
name|info
argument_list|(
literal|"{} stopping background replication from leader"
argument_list|,
name|coreName
argument_list|)
expr_stmt|;
...
name|PULL
operator|:
literal|"LIR should not happen for pull replicas!"
assert|;
if|if
condition|(
name|state
...
literal|null
condition|?
literal|"Collection {} not visible yet, but flagging it so a watch is registered when it becomes visible"
else|:
...
try|try
block|{
comment|// make sure we don't create /collections/{collection} if they do not exist with 2 param
name|zkClient
operator|.
name|makePath
argument_list|(
...
block|}
block|}
comment|/**    * Removed a previously registered OnReconnect listener, such as when a core is removed or reloaded.    */
DECL|method|removeOnReconnectListener
specifier|public
name|void
name|removeOnReconnectListener
parameter_list|(
name|OnReconnect
name|listener
parameter_list|)
...
name|warn
argument_list|(
literal|"Was asked to remove OnReconnect listener "
operator|+
name|listener
operator|+
literal|", but remove operation did not find it in the list of registered listeners."
argument_list|)
expr_stmt|;
block|}
block|}
block|}
DECL|method|getCurrentOnReconnectListeners
name|Set
argument_list|<
name|OnReconnect
argument_list|>
