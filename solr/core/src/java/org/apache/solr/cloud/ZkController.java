...
name|error
argument_list|(
literal|"Error closing updateShardHandler"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|//assert cd.getCloudDescriptor().getShardId() != null;
name|ZkNodeProps
name|m
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// shardState and shardRange are for one-time use only, thereafter the actual values in the Slice should be used
if|if
condition|(
name|Slice
operator|.
...
return|;
block|}
comment|// may return null if not in zk mode
...
block|}
block|}
comment|/**    * utilitiy method fro trimming and leading and/or trailing slashes from     * it's input.  May return the empty string.  May return null if and only     * if the input is null.    */
