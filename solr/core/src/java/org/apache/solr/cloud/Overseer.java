...
condition|)
block|{
comment|// it must be the default then
name|coreNodeName
operator|=
name|message
operator|.
...
condition|)
block|{
comment|//String nodeName = message.getStr(ZkStateReader.NODE_NAME_PROP);
...
name|info
argument_list|(
literal|"Assigning new node to shard="
operator|+
name|sliceName
argument_list|)
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// throw an exception if the slice is not yet active.
comment|//if(!sliceState.equals(Slice.ACTIVE)) {
comment|//  throw new SolrException(ErrorCode.BAD_REQUEST, "Can not assign core to a non-active slice [" + slice.getName() + "]");
comment|//}
name|Replica
name|oldReplica
init|=
...
return|;
block|}
DECL|method|defaultCollectionProps
specifier|private
name|Map
argument_list|<
name|String
...
name|info
argument_list|(
literal|"Create collection {} with numShards {}"
argument_list|,
name|collectionName
argument_list|,
name|numShards
