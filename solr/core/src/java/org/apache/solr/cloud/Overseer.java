...
decl_stmt|;
comment|/**    * @deprecated use {@link org.apache.solr.common.params.CollectionParams.CollectionAction#DELETESHARD}    */
annotation|@
name|Deprecated
...
argument_list|)
decl_stmt|;
comment|// we batch updates for the main cluster state together (stateFormat=1)
comment|// but if we encounter a message for a collection with a stateFormat different than the last
comment|// then we stop batching at that point
name|String
name|collection
init|=
name|message
...
argument_list|()
expr_stmt|;
comment|// we don't want to mix batching of different state formats together because that makes
comment|// it harder to guarantee atomicity of ZK writes
break|break;
block|}
if|if
condition|(
...
block|{
comment|// merely for back-compat where overseer action names were different from the ones
comment|// specified in CollectionAction. See SOLR-6115. Remove this in 5.0
switch|switch
condition|(
name|operation
condition|)
