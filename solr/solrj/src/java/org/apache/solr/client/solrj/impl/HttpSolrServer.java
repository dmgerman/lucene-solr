...
argument_list|)
expr_stmt|;
comment|// Increase default max connection per route to 32
name|ccm
operator|.
...
argument_list|)
expr_stmt|;
comment|// Increase max total connection to 128
name|ccm
operator|.
name|setMaxTotal
argument_list|(
...
condition|)
block|{
comment|// XXX this retry thing seems noop to me
name|Collection
argument_list|<
name|ContentStream
argument_list|>
...
throw|;
block|}
comment|// TODO: move to a interceptor?
name|method
operator|.
name|getParams
argument_list|()
...
expr_stmt|;
block|}
comment|/**    * Sets HttpConnectionParams.setSoTimeout (read timeout). This is desirable    * for queries, but probably not for indexing.    *     * @param timeout    *          Timeout in milliseconds    **/
...
block|}
block|}
comment|/**    * Allow server->client communication to be compressed. Currently gzip and    * deflate are supported. If the server supports compression the response will    * be compressed.    */
...
block|}
block|}
comment|/**    * Set the maximum number of connections that can be open at any given time.    */
