...
name|closed
decl_stmt|;
DECL|field|pollInterrupts
name|AtomicInteger
name|pollInterrupts
decl_stmt|;
DECL|field|pollExits
name|AtomicInteger
name|pollExits
decl_stmt|;
DECL|field|blockLoops
name|AtomicInteger
name|blockLoops
decl_stmt|;
DECL|field|emptyQueueLoops
name|AtomicInteger
name|emptyQueueLoops
decl_stmt|;
...
name|update
decl_stmt|;
name|notifyQueueAndRunnersIfEmptyQueue
argument_list|()
expr_stmt|;
try|try
block|{
...
block|}
block|}
DECL|method|notifyQueueAndRunnersIfEmptyQueue
specifier|private
name|void
name|notifyQueueAndRunnersIfEmptyQueue
parameter_list|()
...
init|)
block|{
comment|// we notify runners too - if there is a high queue poll time and this is the update
comment|// that emptied the queue, we make an attempt to avoid the 250ms timeout in blockUntilFinished
name|runners
operator|.
name|notifyAll
argument_list|()
...
argument_list|()
expr_stmt|;
name|interruptRunnerThreadsPolling
argument_list|()
expr_stmt|;
synchronized|synchronized
init|(
...
argument_list|()
expr_stmt|;
comment|// try to avoid the worst case wait timeout
comment|// without bad spin
name|int
name|timeout
decl_stmt|;
if|if
...
name|warn
argument_list|(
literal|"The task queue still has elements but the update scheduler {} is terminated. Can't process any more tasks. "
operator|+
literal|"Queue size: {}, Runners: {}. Current thread Interrupted? {}"
argument_list|,
name|scheduler
argument_list|,
name|queue
...
parameter_list|)
block|{
comment|// If we set the thread as interrupted again, the next time the wait it's called i t's going to return immediately
name|threadInterrupted
operator|=
literal|true
expr_stmt|;
...
name|warn
argument_list|(
literal|"Thread interrupted while waiting for update queue to be empty. There are still {} elements in the queue."
argument_list|,
name|queue
operator|.
name|size
...
name|debug
argument_list|(
literal|"STATS pollInteruppts={} pollExists={} blockLoops={} emptyQueueLoops={}"
argument_list|,
name|pollInterrupts
operator|.
name|get
...
block|}
block|}
DECL|method|interruptRunnerThreadsPolling
specifier|private
name|void
name|interruptRunnerThreadsPolling
parameter_list|()
...
name|streamDeletes
decl_stmt|;
comment|/**      * Create a Builder object, based on the provided Solr URL.      *       * Two different paths can be specified as a part of this URL:      *       * 1) A path pointing directly at a particular core      *<pre>      *   SolrClient client = new ConcurrentUpdateSolrClient.Builder("http://my-solr-server:8983/solr/core1").build();      *   QueryResponse resp = client.query(new SolrQuery("*:*"));      *</pre>      * Note that when a core is provided in the base URL, queries and other requests can be made without mentioning the      * core explicitly.  However, the client can only send requests to that core.      *       * 2) The path of the root Solr path ("/solr")      *<pre>      *   SolrClient client = new ConcurrentUpdateSolrClient.Builder("http://my-solr-server:8983/solr").build();      *   QueryResponse resp = client.query("core1", new SolrQuery("*:*"));      *</pre>      * In this case the client is more flexible and can be used to send requests to any cores.  This flexibility though      * requires that the core be specified on all requests.       */
DECL|method|Builder
specifier|public
name|Builder
parameter_list|(
