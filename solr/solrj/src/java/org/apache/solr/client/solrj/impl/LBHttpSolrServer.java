...
parameter_list|)
block|{
comment|// we retry on 404 or 403 or 503 - you can see this on solr shutdown
if|if
condition|(
name|e
operator|.
...
throw|;
block|}
comment|// TODO: consider using below above - currently does cause a problem with distrib updates:
comment|// seems to match up against a failed forward to leader exception as well...
comment|//     || e.getMessage().contains("java.net.SocketException")
comment|//     || e.getMessage().contains("java.net.ConnectException")
block|}
catch|catch
parameter_list|(
name|SocketException
