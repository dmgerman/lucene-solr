...
decl_stmt|;
DECL|field|directUpdatesToLeadersOnly
specifier|private
specifier|final
name|boolean
name|directUpdatesToLeadersOnly
decl_stmt|;
...
literal|"_stateVer_"
decl_stmt|;
DECL|field|retryExpiryTime
specifier|private
name|long
name|retryExpiryTime
init|=
name|TimeUnit
operator|.
name|NANOSECONDS
...
argument_list|)
decl_stmt|;
comment|//3 seconds or 3 million nanos
...
condition|)
block|{
comment|// a new collection is likely to be added now.
comment|//check if there are stale items and remove them
name|evictStale
argument_list|()
expr_stmt|;
return|return
...
block|}
block|}
comment|/**    * This is the time to wait to refetch the state    * after getting the same state version from ZK    *<p>    * secs    */
DECL|method|setRetryExpiryTime
specifier|public
name|void
name|setRetryExpiryTime
parameter_list|(
name|int
name|secs
parameter_list|)
...
name|cachedAt
decl_stmt|;
comment|//This is the time at which the collection is retried and got the same old version
DECL|field|retriedAt
name|long
name|retriedAt
init|=
operator|-
literal|1
decl_stmt|;
comment|//flag that suggests that this is potentially to be rechecked
DECL|field|maybeStale
name|boolean
name|maybeStale
init|=
literal|false
...
condition|)
block|{
comment|// we are not sure if it is stale so check with retry time
if|if
condition|(
operator|(
name|retriedAt
...
literal|true
return|;
comment|// we retried a while back. and we could not get anything new.
comment|//it's likely that it is not going to be available now also.
block|}
block|}
return|return
literal|false
...
argument_list|(
literal|"Both zkHost(s)& solrUrl(s) have been specified. Only specify one."
argument_list|)
throw|;
block|}
if|if
...
argument_list|(
literal|"Couldn't initialize a HttpClusterStateProvider (is/are the "
operator|+
literal|"Solr server(s), "
operator|+
name|solrUrls
operator|+
literal|", down?)"
...
name|IllegalArgumentException
argument_list|(
literal|"Both zkHosts and solrUrl cannot be null."
argument_list|)
throw|;
block|}
block|}
...
expr_stmt|;
block|}
comment|/**    * @param zkHost    *          A zookeeper client endpoint.    * @param updatesToLeaders    *          If true, sends updates only to shard leaders.    * @see #CloudSolrClient(String) for full description and details on zkHost    * @deprecated use {@link CloudSolrClient.Builder} instead.    */
annotation|@
name|Deprecated
DECL|method|CloudSolrClient
specifier|public
...
expr_stmt|;
block|}
comment|/**    * @param zkHost    *          A zookeeper client endpoint.    * @param updatesToLeaders    *          If true, sends updates only to shard leaders.    * @param httpClient    *          the {@link HttpClient} instance to be used for all requests. The provided httpClient should use a    *          multi-threaded connection manager.    * @see #CloudSolrClient(String) for full description and details on zkHost    * @deprecated use {@link CloudSolrClient.Builder} instead.    */
annotation|@
name|Deprecated
DECL|method|CloudSolrClient
specifier|public
...
expr_stmt|;
block|}
comment|/**    * @param zkHost    *          A zookeeper client endpoint.    * @param lbClient    *          LBHttpSolrServer instance for requests.    * @see #CloudSolrClient(String) for full description and details on zkHost    * @deprecated use {@link CloudSolrClient.Builder} instead.    */
annotation|@
name|Deprecated
DECL|method|CloudSolrClient
specifier|public
...
expr_stmt|;
block|}
comment|/**    * @param zkHost    *          A zookeeper client endpoint.    * @param lbClient    *          LBHttpSolrServer instance for requests.    * @param updatesToLeaders    *          If true, sends updates only to shard leaders.    * @see #CloudSolrClient(String) for full description and details on zkHost    * @deprecated use {@link Builder} instead.    */
annotation|@
name|Deprecated
DECL|method|CloudSolrClient
specifier|public
...
argument_list|(
literal|"This has no Zk stateReader"
argument_list|)
throw|;
block|}
...
expr_stmt|;
block|}
comment|/**    * Connect to a cluster.  If the cluster is not ready, retry connection up to a given timeout.    * @param duration the timeout    * @param timeUnit the units of the timeout    * @throws TimeoutException if the cluster is not ready after the timeout    * @throws InterruptedException if the wait is interrupted    */
DECL|method|connect
specifier|public
name|void
name|connect
...
name|info
argument_list|(
literal|"Waiting for {} {} for cluster at {} to be ready"
argument_list|,
name|duration
argument_list|,
name|timeUnit
...
name|TimeoutException
argument_list|(
literal|"Timed out waiting for cluster"
argument_list|)
throw|;
block|}
...
expr_stmt|;
block|}
DECL|method|assertZKStateProvider
specifier|private
name|ZkClientClusterStateProvider
name|assertZKStateProvider
parameter_list|()
...
name|IllegalArgumentException
argument_list|(
literal|"This client does not use ZK"
argument_list|)
throw|;
block|}
...
expr_stmt|;
block|}
comment|/**    * Register a CollectionStateWatcher to be called when the cluster state for a collection changes    *    * Note that the watcher is unregistered after it has been called once.  To make a watcher persistent,    * it should re-register itself in its {@link CollectionStateWatcher#onStateChanged(Set, DocCollection)}    * call    *    * @param collection the collection to watch    * @param watcher    a watcher that will be called when the state changes    */
...
condition|)
block|{
comment|// we have info (documents with ids and/or ids to delete) with
comment|// which to find the leaders but we could not find (all of) them
throw|throw
operator|new
name|SolrException
argument_list|(
...
argument_list|,
literal|"directUpdatesToLeadersOnly==true but could not find leader(s)"
argument_list|)
throw|;
block|}
else|else
block|{
comment|// we could not find a leader or routes yet - use unoptimized general path
return|return
literal|null
return|;
block|}
...
literal|null
decl_stmt|;
comment|// TolerantUpdateProcessor
name|List
argument_list|<
name|SimpleOrderedMap
argument_list|<
...
argument_list|>
argument_list|>
name|toleratedErrors
init|=
literal|null
decl_stmt|;
name|int
name|maxToleratedErrors
init|=
name|Integer
operator|.
name|MAX_VALUE
decl_stmt|;
comment|// For "adds", "deletes", "deleteByQuery" etc.
name|Map
argument_list|<
name|String
argument_list|,
...
name|shardMaxToleratedErrors
operator|:
literal|"TolerantUpdateProcessor reported errors but not maxErrors"
assert|;
comment|// if we get into some weird state where the nodes disagree about the effective maxErrors,
comment|// assume the min value seen to decide if we should fail.
name|maxToleratedErrors
operator|=
name|Math
operator|.
...
condition|)
block|{
comment|// cumulative errors are too high, we need to throw a client exception w/correct metadata
comment|// NOTE: it shouldn't be possible for 1 == toleratedErrors.size(), because if that were the case
comment|// then at least one shard should have thrown a real error before this, so we don't worry
comment|// about having a more "singular" exception msg for that situation
name|StringBuilder
name|msgBuf
init|=
operator|new
...
name|append
argument_list|(
literal|" Async failures during distributed update: "
argument_list|)
decl_stmt|;
name|NamedList
name|metadata
...
name|routes
expr_stmt|;
comment|// create a merged copy of the metadata from all wrapped exceptions
name|NamedList
argument_list|<
name|String
argument_list|>
...
condition|)
block|{
comment|// don't do _stateVer_ checking for admin, v2 api requests
name|Set
argument_list|<
name|String
argument_list|>
...
argument_list|)
decl_stmt|;
comment|// don't do retry support for admin requests
comment|// or if the request doesn't have a collection specified
comment|// or request is v2 api and its method is not GET
if|if
condition|(
name|collection
operator|==
...
condition|)
block|{
comment|// it was a communication error. it is likely that
comment|// the node to which the request to be sent is down . So , expire the state
comment|// so that the next attempt would fetch the fresh state
comment|// just re-read state for all of them, if it has not been retired
comment|// in retryExpiryTime time
for|for
control|(
name|DocCollection
name|ext
...
condition|)
block|{
comment|//if it is a communication error , we must try again
comment|//may be, we have a stale version of the collection state
comment|// and we could not get any information from the server
comment|//it is probably not worth trying again and again because
comment|// the state would not have been updated
return|return
name|requestWithRetryOnStaleState
argument_list|(
name|request
...
return|;
block|}
comment|/**    * @return true if direct updates are sent to shard leaders only    */
DECL|method|isDirectUpdatesToLeadersOnly
specifier|public
name|boolean
name|isDirectUpdatesToLeadersOnly
parameter_list|()
block|{
return|return
name|directUpdatesToLeadersOnly
...
return|;
block|}
comment|// We are going to fetch a new version
comment|// we MUST try to get a new version
name|fetchedCol
operator|=
name|ref
operator|.
...
literal|null
return|;
comment|// this collection no more exists
if|if
condition|(
name|col
operator|!=
...
argument_list|()
expr_stmt|;
comment|//we retried and found that it is the same version
name|cacheEntry
operator|.
name|maybeStale
operator|=
...
condition|)
block|{
comment|// no documents and no delete-by-id, so no info to find leader(s)
return|return
literal|false
return|;
block|}
...
condition|)
block|{
comment|// a document with no id field value, so can't find leader for it
return|return
literal|false
return|;
block|}
...
return|;
block|}
DECL|method|createLBHttpSolrClient
specifier|private
specifier|static
name|LBHttpSolrClient
name|createLBHttpSolrClient
...
return|;
block|}
comment|/**    * Constructs {@link CloudSolrClient} instances from provided configuration.    */
DECL|class|Builder
specifier|public
specifier|static
class|class
...
name|zkChroot
decl_stmt|;
DECL|field|loadBalancedSolrClient
specifier|private
name|LBHttpSolrClient
name|loadBalancedSolrClient
decl_stmt|;
DECL|field|lbClientBuilder
specifier|private
name|LBHttpSolrClient
operator|.
name|Builder
name|lbClientBuilder
decl_stmt|;
DECL|field|shardLeadersOnly
specifier|private
name|boolean
name|shardLeadersOnly
decl_stmt|;
DECL|field|directUpdatesToLeadersOnly
specifier|private
name|boolean
...
expr_stmt|;
block|}
comment|/**      * Provide a ZooKeeper client endpoint to be used when configuring {@link CloudSolrClient} instances.      *       * Method may be called multiple times.  All provided values will be used.      *       * @param zkHost      *          The client endpoint of the ZooKeeper quorum containing the cloud      *          state.      */
DECL|method|withZkHost
specifier|public
name|Builder
name|withZkHost
parameter_list|(
...
return|;
block|}
comment|/**      * Provide a Solr URL to be used when configuring {@link CloudSolrClient} instances.      *      * Method may be called multiple times. One of the provided values will be used to fetch      * the list of live Solr nodes that the underlying {@link HttpClusterStateProvider} would be maintaining.      */
DECL|method|withSolrUrl
specifier|public
name|Builder
name|withSolrUrl
parameter_list|(
...
return|;
block|}
comment|/**      * Provide a list of Solr URL to be used when configuring {@link CloudSolrClient} instances.      * One of the provided values will be used to fetch the list of live Solr      * nodes that the underlying {@link HttpClusterStateProvider} would be maintaining.      */
DECL|method|withSolrUrl
specifier|public
name|Builder
name|withSolrUrl
...
DECL|method|withLBHttpSolrClientBuilder
specifier|public
name|Builder
...
return|;
block|}
comment|/**      * Provide a series of ZooKeeper client endpoints for the builder to use when creating clients.      *       * Method may be called multiple times.  All provided values will be used.      *       * @param zkHosts      *          A Java Collection (List, Set, etc) of HOST:PORT strings, one for      *          each host in the ZooKeeper ensemble. Note that with certain      *          Collection types like HashSet, the order of hosts in the final      *          connect string may not be in the same order you added them.      */
DECL|method|withZkHost
specifier|public
name|Builder
name|withZkHost
...
return|;
block|}
comment|/**      * Provides a ZooKeeper chroot for the builder to use when creating clients.      */
DECL|method|withZkChroot
specifier|public
name|Builder
name|withZkChroot
parameter_list|(
...
return|;
block|}
comment|/**      * Provides a {@link LBHttpSolrClient} for the builder to use when creating clients.      */
DECL|method|withLBHttpSolrClient
specifier|public
name|Builder
...
return|;
block|}
comment|/**      * Tells {@link Builder} that created clients should send updats only to shard leaders.      */
DECL|method|sendUpdatesOnlyToShardLeaders
specifier|public
name|Builder
...
return|;
block|}
comment|/**      * Tells {@link Builder} that created clients should send updates to all replicas for a shard.      */
DECL|method|sendUpdatesToAllReplicasInShard
specifier|public
name|Builder
name|sendUpdatesToAllReplicasInShard
parameter_list|()
block|{
name|shardLeadersOnly
operator|=
...
return|;
block|}
comment|/**      * Tells {@link Builder} that created clients should send direct updates to shard leaders only.      */
DECL|method|sendDirectUpdatesToShardLeadersOnly
specifier|public
name|Builder
...
return|;
block|}
comment|/**      * Tells {@link Builder} that created clients can send updates      * to any shard replica (shard leaders and non-leaders).      */
DECL|method|sendDirectUpdatesToAnyShardReplica
specifier|public
name|Builder
name|sendDirectUpdatesToAnyShardReplica
parameter_list|()
block|{
name|directUpdatesToLeadersOnly
operator|=
...
return|;
block|}
DECL|method|withClusterStateProvider
specifier|public
name|Builder
...
return|;
block|}
comment|/**      * Create a {@link CloudSolrClient} based on the provided configuration.      */
DECL|method|build
specifier|public
name|CloudSolrClient
name|build
