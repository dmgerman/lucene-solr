...
begin_comment
comment|/** * Connects to Zookeeper to pick replicas from a specific collection to send the query to. * SolrStream instances are used to send the query to the replicas. * SolrStreams are opened using a Thread pool, but a single thread is used to iterate through each stream's tuples.* * **/
end_comment
