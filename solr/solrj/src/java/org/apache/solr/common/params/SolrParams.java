...
parameter_list|)
block|{
comment|// always use MultiMap for easier processing further down the chain
return|return
operator|new
name|MultiMapSolrParams
argument_list|(
...
return|;
block|}
comment|/**Copy all params to the given map or if the given map is null    * create a new one    */
DECL|method|getAll
specifier|public
name|Map
argument_list|<
...
return|;
block|}
comment|/** Returns this SolrParams as a properly URL encoded string, starting with {@code "?"}, if not empty. */
...
block|}
block|}
comment|/**    * Generates a local-params string of the form<pre>{! name=value name2=value2}</pre>.    */
DECL|method|toLocalParamsString
specifier|public
name|String
name|toLocalParamsString
parameter_list|()
block|{
specifier|final
name|StringBuilder
...
argument_list|)
expr_stmt|;
comment|//TODO perhaps look for 'type' and add here?  but it doesn't matter.
for|for
control|(
specifier|final
name|Iterator
...
argument_list|)
expr_stmt|;
comment|// do so even the first time; why not.
name|sb
operator|.
name|append
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// no escaping for name; it must follow "Java Identifier" rules.
name|sb
operator|.
name|append
argument_list|(
...
return|;
block|}
comment|/** Like {@link #toQueryString()}, but only replacing enough chars so that    * the URL may be unambiguously pasted back into a browser.    * This method can be used to properly log query parameters without    * making them unreadable.    *<p>    * Characters with a numeric value less than 32 are encoded.    *&amp;,=,%,+,space are encoded.    */
annotation|@
name|Override
