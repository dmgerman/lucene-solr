...
name|PULL_REPLICAS
init|=
literal|"pullReplicas"
decl_stmt|;
DECL|field|NRT_REPLICAS
specifier|public
specifier|static
specifier|final
name|String
...
expr_stmt|;
block|}
comment|/**    * Construct the total state view from all sources.    * Must hold {@link #getUpdateLock()} before calling this.    *    * @param changedCollections collections that have changed since the last call,    *                           and that should fire notifications    */
...
condition|)
block|{
comment|// check that we haven't just migrated
name|newState
operator|=
name|watchedCollectionStates
operator|.
...
parameter_list|)
block|{
comment|//TODO: We don't need all these getReplicaProps method overloading. Also, it's odd that the default is to return replicas of type TLOG and NRT only
return|return
name|getReplicaProps
argument_list|(
name|collection
...
begin_decl_stmt
DECL|field|clusterPropertiesWatcher
specifier|private
specifier|final
name|Watcher
name|clusterPropertiesWatcher
init|=
name|event
lambda|->
block|{
...
literal|"unchecked"
argument_list|)
DECL|method|loadClusterProperties
specifier|private
name|void
name|loadClusterProperties
parameter_list|()
...
argument_list|)
expr_stmt|;
comment|// set an exists watch, and if the node has been created since the last call,
comment|// read the data again
if|if
condition|(
name|zkClient
operator|.
...
name|error
argument_list|(
literal|"Error reading cluster properties from zookeeper"
argument_list|,
name|SolrZkClient
operator|.
name|checkInterrupted
...
begin_function
DECL|method|getCollectionPathRoot
specifier|public
specifier|static
name|String
...
begin_comment
comment|/**    * Register a CollectionStateWatcher to be called when the state of a collection changes    */
end_comment
...
condition|)
block|{
comment|// no change to state, but we might have been triggered by the addition of a
comment|// state watcher, so run notifications
name|updated
operator|=
literal|true
expr_stmt|;
