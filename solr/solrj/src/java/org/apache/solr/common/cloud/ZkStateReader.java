...
argument_list|()
decl_stmt|;
comment|/**These are collections which are actively watched by this  instance .    *    */
...
argument_list|()
expr_stmt|;
comment|//on reconnect of SolrZkClient re-add watchers for the watched external collections
synchronized|synchronized
init|(
name|this
init|)
...
block|}
block|}
DECL|method|updateCollectionNames
specifier|public
name|void
name|updateCollectionNames
parameter_list|()
...
name|warn
argument_list|(
literal|"Error reading collections nodes"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    *<b>Advance usage</b>    * This method can be used to fetch a collection object and control whether it hits    * the cache only or if information can be looked up from ZooKeeper.    *    * @param coll the collection name    * @param cachedCopyOnly whether to fetch data from cache only or if hitting Zookeeper is acceptable    * @return the {@link org.apache.solr.common.cloud.DocCollection}    */
...
condition|)
block|{
comment|//this collection resides in clusterstate.json. So it's always up-to-date
return|return
name|clusterState
operator|.
name|getCollectionStates
...
return|;
block|}
DECL|field|ephemeralCollectionData
specifier|private
name|Map
name|ephemeralCollectionData
decl_stmt|;
comment|/**    * this is only set by Overseer not to be set by others and only set inside the Overseer node. If Overseer has    unfinished external collections which are yet to be persisted to ZK    this map is populated and this class can use that information    @param map  The map reference    */
DECL|method|setEphemeralCollectionData
specifier|public
name|void
name|setEphemeralCollectionData
parameter_list|(
name|Map
name|map
parameter_list|)
...
name|info
argument_list|(
literal|"Updating collection state at {} from ZooKeeper... "
argument_list|,
name|fullpath
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"No value set for collection state : {}"
argument_list|,
name|coll
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Updating data for {} to ver {} "
argument_list|,
name|coll
argument_list|,
name|newState
