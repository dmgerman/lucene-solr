...
argument_list|)
decl_stmt|;
comment|// first load all collections in /clusterstate.json (i.e. stateFormat=1)
name|Map
argument_list|<
name|String
argument_list|,
...
name|s
range|:
name|getStateFormat2CollectionNames
argument_list|()
control|)
block|{
synchronized|synchronized
...
return|;
block|}
DECL|method|getStateFormat2CollectionNames
specifier|private
name|Set
argument_list|<
name|String
...
try|try
block|{
comment|// this exists call is necessary because we only want to return
comment|// those collections which have their own state.json.
comment|// The getCollectionPath() calls returns the complete path to the
comment|// collection's state.json
if|if
condition|(
name|zkClient
operator|.
