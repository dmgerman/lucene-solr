...
expr_stmt|;
block|}
name|refreshLazyFormat2Collections
argument_list|(
literal|true
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Search for any lazy-loadable state format2 collections.    */
DECL|method|refreshLazyFormat2Collections
specifier|private
name|void
name|refreshLazyFormat2Collections
parameter_list|(
...
condition|)
block|{
comment|// We will create an eager collection for any interesting collections.
continue|continue;
block|}
if|if
condition|(
...
condition|)
block|{
comment|// Try to use an already-created lazy collection if it's not a full refresh.
name|ClusterState
operator|.
name|CollectionRef
name|existing
...
operator|.
name|CollectionRef
name|lazyCollectionState
init|=
name|tryMakeLazyCollectionStateFormat2
argument_list|(
name|collName
argument_list|)
decl_stmt|;
...
block|}
block|}
DECL|method|tryMakeLazyCollectionStateFormat2
specifier|private
name|ClusterState
operator|.
name|CollectionRef
...
argument_list|)
expr_stmt|;
comment|// Changes to clusterstate.json signal global state changes.
comment|// TODO: get rid of clusterstate.json as a signaling mechanism.
name|refreshLazyFormat2Collections
argument_list|(
literal|false
argument_list|)
...
operator|.
name|CollectionRef
name|lazyCollectionStateFormat2
init|=
name|tryMakeLazyCollectionStateFormat2
argument_list|(
name|coll
