...
name|contains
argument_list|(
literal|"Remote error message: Document contains multiple values for uniqueKey"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
name|rawJson
init|=
literal|"{ \"raw\": 1.234, \"id\":\"111\" }"
decl_stmt|;
name|String
name|rawXml
init|=
literal|"<hello>this is<some/><xml/></hello>"
decl_stmt|;
name|SolrClient
name|client
init|=
...
name|FL
argument_list|,
literal|"id,json_s:[json],xml_s:[xml]"
argument_list|)
expr_stmt|;
name|QueryRequest
name|req
...
argument_list|)
expr_stmt|;
comment|// Check that the 'raw' fields are unchanged using the standard formats
name|assertEquals
argument_list|(
name|rawJson
argument_list|,
...
argument_list|)
expr_stmt|;
comment|//    // Check that unknown augmenters throw an error
comment|//    query.set( CommonParams.FL, "id,[asdkgjahsdgjka]" );
comment|//    try {
comment|//      rsp = client.query( query );
comment|//      fail("Should throw an exception for unknown transformer: "+query.get(CommonParams.FL));
comment|//    }
comment|//    catch(SolrException ex) {
comment|//      assertEquals(ErrorCode.BAD_REQUEST.code, ex.code());
comment|//    }
if|if
condition|(
name|client
...
block|{
return|return;
comment|// the EmbeddedSolrServer ignores the configured parser
block|}
comment|// Check raw JSON Output
name|query
operator|.
name|set
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// Check that the response parses as JSON
name|JSONParser
name|parser
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// Check that we get raw xml and json is escaped
name|assertTrue
argument_list|(
name|raw
operator|.
...
annotation|@
name|Test
DECL|method|testPivotFacetsQueries
specifier|public
name|void
name|testPivotFacetsQueries
parameter_list|()
throws|throws
name|Exception
block|{
...
name|addFacetPivotField
argument_list|(
literal|"{!query=s1}features,manu"
argument_list|)
expr_stmt|;
name|query
operator|.
name|addFacetQuery
argument_list|(
literal|"{!key=highPrice tag=s1}price:[100 TO *]"
argument_list|)
expr_stmt|;
name|query
operator|.
name|addFacetQuery
argument_list|(
literal|"{!tag=s1 key=lowPrice}price:[0 TO 50]"
argument_list|)
expr_stmt|;
name|query
operator|.
...
annotation|@
name|Test
DECL|method|testPivotFacetsRanges
specifier|public
name|void
name|testPivotFacetsRanges
parameter_list|()
throws|throws
name|Exception
block|{
...
name|addFacetPivotField
argument_list|(
literal|"{!range=s1}features,manu"
argument_list|)
expr_stmt|;
name|query
operator|.
...
name|FACET_RANGE
argument_list|,
literal|"{!key=price1 tag=s1}price"
argument_list|)
expr_stmt|;
name|query
operator|.
...
name|FACET_RANGE
argument_list|,
literal|"{!key=price2 tag=s1}price"
argument_list|)
expr_stmt|;
name|query
operator|.
...
annotation|@
name|Test
DECL|method|testExpandComponent
specifier|public
name|void
...
name|q
operator|.
name|setMoreLikeThisMinTermFreq
argument_list|(
literal|0
argument_list|)
expr_stmt|;
...
name|id
operator|+
literal|" had not returned exactly 2 documents"
argument_list|,
name|mltResp
operator|.
name|size
