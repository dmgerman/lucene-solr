...
argument_list|(
literal|"Won't Happen for UTF-8"
argument_list|)
throw|;
block|}
name|System
...
operator|+
literal|"If you are not using --go-live, pass the --shards argument. If you are building shards for "
operator|+
literal|"a Non-SolrCloud cluster, pass the --shard-url argument one or more times. To build indexes for"
operator|+
literal|" a replicated cluster with --shard-url, pass replica urls consecutively and also pass --shards. "
operator|+
literal|"If you are building shards for a SolrCloud cluster, pass the --zk-host argument. "
operator|+
literal|"Using --go-live requires either --shard-url or --zk-host."
argument_list|)
decl_stmt|;
name|Argument
name|shardUrlsArg
...
argument_list|)
expr_stmt|;
comment|// FIXME: handle more than 99999 shards (need numeric sort rather than lexicographical sort)
return|return
name|dirs
return|;
block|}
