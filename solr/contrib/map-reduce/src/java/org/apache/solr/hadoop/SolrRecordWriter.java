...
name|IOException
argument_list|(
literal|"Unable to find solr home setting"
argument_list|)
throw|;
block|}
name|LOG
...
argument_list|()
decl_stmt|;
comment|// FIXME note this is odd (no scheme) given Solr doesn't currently
comment|// support uris (just abs/relative path)
name|Path
name|solrDataDir
init|=
operator|new
...
control|)
block|{
comment|// Only logged if debugging
if|if
condition|(
name|LOG
operator|.
...
name|ENGLISH
argument_list|,
literal|"Examining unpack directory %s for %s"
argument_list|,
name|unpackedDir
argument_list|,
name|SolrOutputFormat
