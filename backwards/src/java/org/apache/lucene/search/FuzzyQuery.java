...
condition|)
block|{
comment|// record the current bottom item
if|if
condition|(
name|bottomSt
operator|==
...
expr_stmt|;
block|}
comment|// add to PQ, as it is not yet filled up
name|stQueue
operator|.
name|offer
argument_list|(
...
literal|null
assert|;
comment|// only add to PQ, if the ScoreTerm is greater than the current bottom,
comment|// as all entries will be enqueued after the current bottom and will never be visible
if|if
condition|(
name|st
operator|.
...
block|}
block|}
comment|//System.out.println("current: "+st.term+"("+st.score+"), bottom: "+bottomSt.term+"("+bottomSt.score+")");
block|}
do|while
condition|(
name|enumerator
