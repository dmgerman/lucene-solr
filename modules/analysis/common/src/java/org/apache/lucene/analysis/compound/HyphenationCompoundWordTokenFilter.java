...
argument_list|)
decl_stmt|;
comment|// we need this to load the DTD in very old parsers (like the one in JDK 1.4).
comment|// The DTD itsself is provided via EntityResolver, so it should always load, but
comment|// some parsers still want to have a base URL (Crimson).
name|is
operator|.
name|setSystemId
argument_list|(
