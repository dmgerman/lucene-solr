...
return|;
block|}
comment|// params for the "nutch" query optimizer
...
expr_stmt|;
block|}
comment|/**    * Returns the set of document ids matching a query.    * This method is cache-aware and attempts to retrieve the answer from the cache if possible.    * If the answer was not cached, it may have been inserted into the cache as a result of this call.    *<p>    * The DocSet returned should<b>not</b> be modified.    */
...
return|;
block|}
comment|// TODO: do a more efficient version that starts with the
comment|// smallest DocSet and drives the intersection off that
comment|// or implement an intersection() function that takes multiple
comment|// DocSets (prob the better way)
DECL|method|getDocSet
specifier|protected
name|DocSet
name|getDocSet
...
argument_list|)
return|;
comment|/******* OLD VERSION that did a filtered query instead of      * an intersection if the query docset wasn't found in the cache.      * It made misses != inserts (even if no evictions)     DocSet first=null;     if (filterCache != null) {       first = (DocSet)filterCache.get(query);       if (first != null) {         return first.intersection(filter);       }     }      DocSet answer = getDocSetNC(query, filter);     // nothing is inserted into the cache, because we don't cache materialized filters.     // Hmmm, we *could* make a hitcollector that made a DocSet out of the query at the     // same time it was running the filter though...      // Q: we could call getDocSet(query) and then take the intersection instead of running     // the query as a filter.  Then it could be cached.     return answer;     ****************/
block|}
...
argument_list|)
expr_stmt|;
comment|// OPT... if getDocListNC can get the set at the same time (later version)
comment|// then set it as out.docSet.
name|out
operator|.
name|docList
operator|=
...
expr_stmt|;
block|}
comment|// TODO: OPT: Hmmm, but if docList.size() == docList.matches() then
comment|// we actually already have all the ids (the set)!  We could simply
comment|// return the docList as the docSet also, or hash the ids into
comment|// a HashDocSet, etc... no need to run the query again!
comment|//
assert|assert
operator|(
name|ret
...
name|IOException
block|{
comment|// reverse: do the query on filter and filter using docs...
comment|// or if filter is a term query, can get the freq and
comment|// drive things off that.
comment|//  this higher level API leaves open more optimization possibilities.
comment|// prob only worth it if cacheHitRatio is bad...
return|return
name|b
operator|.
...
block|{
return|return
literal|"the searcher that handles all index queries"
return|;
block|}
...
parameter_list|()
block|{
comment|/***     NamedList lst = new NamedList();     lst.add("requests", numRequests);     lst.add("errors", numErrors);     return lst;     ***/
return|return
operator|new
name|NamedList
argument_list|()
...
begin_comment
comment|// Todo: counting only hit collector (for speed comparison w/ caching filters)
end_comment

begin_comment
comment|// todo: fast term query
end_comment

begin_comment
comment|// todo: do a both hit collector that can get a DocList and DocSet at the same time
end_comment

begin_class
DECL|class|SetHitCollector
specifier|final
class|class
name|SetHitCollector
extends|extends
...
name|filter
decl_stmt|;
comment|// should we bother with filters at this point?
comment|// how much faster would it be to take the check for
comment|// filter!=null out of the loop??? depends on HotSpot... it may
comment|// optimize it anyway.
DECL|field|bits
specifier|final
name|BitSet
name|bits
...
condition|)
return|return;
comment|// OPTIMIZATION: should I only set bits *after* I have run out of
comment|// room in scratch?  (then at the end I could add all of the docs
comment|// in scratch to the bitset.
name|bits
operator|.
name|set
argument_list|(
...
comment|// It is assumed that storing in this array will be quicker to convert
...
comment|// we could have a SortedListDocSet() and use the collected array directly.
if|if
condition|(
name|pos
operator|<
