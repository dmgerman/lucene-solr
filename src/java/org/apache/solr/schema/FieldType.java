...
return|;
block|}
comment|/**    * Create multiple fields from a single field and multiple values.  Fields are named as SchemaField.getName() + {@link #POLY_FIELD_SEPARATOR} + i, where    * i starts at 0.    *<p/>    * If the field is stored, then an extra field gets created that contains the storageVal.  It is this field that also    *    * @param field The {@link org.apache.solr.schema.SchemaField}    * @param props The properties to use    * @param delegatedType An optional type to use.  If null, then field.getType() is used.  Useful for poly fields.    * @param storageVal If the field stores, then this value will be used for the stored field    * @param boost The boost to apply to all fields    * @param externalVals The values to use    * @return The fields    */
...
condition|)
block|{
comment|//if the type isn't being overriden, then just use the base one
name|delegatedType
operator|=
name|field
operator|.
...
index|]
decl_stmt|;
comment|//Field.Store.NO,Field.Index.NOT_ANALYZED_NO_NORMS, Field.TermVector.NO, true, true
if|if
condition|(
name|externalVals
operator|.
...
control|)
block|{
comment|//SchemaField is final, as is name, so we need to recreate each time
comment|//put the counter before the separator, b/c dynamic fields can't be asterisks on both the front and the end of the String
name|bldr
operator|.
name|append
argument_list|(
...
argument_list|)
decl_stmt|;
comment|//schema.getDynamicField(name  + "_" + j + POLY_FIELD_SEPARATOR + delegatedType.typeName);
comment|/**/
comment|//new SchemaField(name, ft, p, defaultValue )
comment|//QUESTION: should we allow for vectors, etc?  Not sure that it makes sense
name|results
index|[
name|j
index|]
...
argument_list|)
expr_stmt|;
comment|//cut the builder back to just the length of the prefix, but keep the capacity
block|}
block|}
name|Field
operator|.
