...
begin_comment
comment|/**  * Implements {@link LockFactory} using native OS file locks  * (available through java.nio.*).  Note that for certain  * filesystems native locks are possible but must be  * explicity configured and enabled (and may be disabled by  * default).  For example, for NFS servers there sometimes  * must be a separate lockd process running, and other  * configuration may be required such as running the server  * in kernel mode.  Other filesystems may not even support  * native OS locks in which case you must use a different  * {@link LockFactory} implementation.  *  *<p>The advantage of this lock factory over  * SimpleFSLockFactory is that the locks should be  * "correct", whereas SimpleFSLockFactory uses  * java.io.File.createNewFile which has warnings about not  * using it for locking.  Furthermore, if the JVM crashes,  * the OS will free any held locks, whereas  * SimpleFSLockFactory will keep the locks held, requiring  * manual removal before re-running Lucene.</p>  *  *<p>Note that, unlike SimpleFSLockFactory, the existence of  * leftover lock files in the filesystem on exiting the JVM  * is fine because the OS will free the locks held against  * these files even though the files still remain.</p>  *  *<p>Native locks file names have the substring "-n-", which  * you can use to differentiate them from lock files created  * by SimpleFSLockFactory.</p>  *  * @see LockFactory  */
end_comment
...
name|lockDir
decl_stmt|;
comment|/*    * The javadocs for FileChannel state that you should have    * a single instance of a FileChannel (per JVM) for all    * locking against a given file.  To do this, we ensure    * there's a single instance of NativeFSLockFactory per    * canonical lock directory, and then we always use a    * single lock instance (per lock name) if it's present.    */
DECL|field|LOCK_FACTORIES
specifier|private
specifier|static
name|Hashtable
name|LOCK_FACTORIES
...
expr_stmt|;
block|}
comment|/**    * Returns a NativeFSLockFactory instance, storing lock    * files into the default LOCK_DIR:    *<code>org.apache.lucene.lockDir</code> system property,    * or (if that is null) then<code>java.io.tmpdir</code>.    */
...
return|;
block|}
comment|/**    * Returns a NativeFSLockFactory instance, storing lock    * files into the specified lockDirName:    *    * @param lockDirName where lock files are created.    */
DECL|method|getLockFactory
specifier|public
specifier|static
name|NativeFSLockFactory
...
return|;
block|}
comment|/**    * Returns a NativeFSLockFactory instance, storing lock    * files into the specified lockDir:    *     * @param lockDir where lock files are created.    */
DECL|method|getLockFactory
specifier|public
specifier|static
name|NativeFSLockFactory
