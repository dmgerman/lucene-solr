...
comment|// this case we have to update the FieldInfo of the particular field.
name|invertDocument
argument_list|(
name|doc
argument_list|)
...
name|fieldBoosts
decl_stmt|;
comment|// If any of the tokens of a paticular field carry a payload
comment|// then we enable payloads for that field.
DECL|field|fieldStoresPayloads
specifier|private
name|BitSet
name|fieldStoresPayloads
decl_stmt|;
...
condition|)
block|{
comment|// enable payloads for this field
name|fieldStoresPayloads
operator|.
name|set
argument_list|(
...
block|}
block|}
comment|// update fieldInfos for all fields that have one or more tokens with payloads
for|for
control|(
name|int
name|i
...
condition|)
block|{
comment|// lazily allocate payload array
name|ti
operator|.
name|payloads
operator|=
...
literal|1
decl_stmt|;
comment|// The following encoding is being used for positions and payloads:
comment|// Case 1: current field does not store payloads
comment|//           Positions     -><PositionDelta>^freq
comment|//           PositionDelta -> VInt
comment|//         The PositionDelta is the difference between the current
comment|//         and the previous position
comment|// Case 2: current field stores payloads
comment|//           Positions     -><PositionDelta, Payload>^freq
comment|//           Payload       -><PayloadLength?, PayloadData>
comment|//           PositionDelta -> VInt
comment|//           PayloadLength -> VInt
comment|//           PayloadData   -> byte^PayloadLength
comment|//         In this case PositionDelta/2 is the difference between
comment|//         the current and the previous position. If PositionDelta
comment|//         is odd, then a PayloadLength encoded as VInt follows,
comment|//         if PositionDelta is even, then it is assumed that the
comment|//         length of the current Payload equals the length of the
...
condition|)
block|{
comment|// the length of the current payload equals the length
comment|// of the previous one. So we do not have to store the length
comment|// again and we only shift the position delta by one bit
name|prox
operator|.
name|writeVInt
argument_list|(
...
block|{
comment|// the length of the current payload is different from the
comment|// previous one. We shift the position delta, set the lowest
comment|// bit and store the current payload length as VInt.
name|prox
operator|.
name|writeVInt
argument_list|(
...
block|{
comment|// field does not store payloads, just write position delta as VInt
name|prox
operator|.
name|writeVInt
argument_list|(
...
name|payloads
decl_stmt|;
comment|// the payloads of the terms
DECL|field|offsets
name|TermVectorOffsetInfo
index|[]
name|offsets
