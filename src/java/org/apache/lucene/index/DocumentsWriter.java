...
name|infoStream
decl_stmt|;
comment|// Holds buffered deletes, by docID, term or query.  We
comment|// hold two instances of this class: one for the deletes
comment|// prior to the last flush, the other for deletes after
comment|// the last flush.  This is so if we need to abort
comment|// (discard all buffered docs) we can also discard the
comment|// buffered deletes yet keep the deletes done during
comment|// previously flushed segments.
