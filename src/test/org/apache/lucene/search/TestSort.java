...
expr_stmt|;
block|}
comment|// test sorts where the type of field is specified and a custom field parser is used, that
comment|// uses a simple char encoding. The sorted string contains a character beginning from 'A' that
comment|// is mapped to a numeric value using some "funny" algorithm to be different for each data type.
